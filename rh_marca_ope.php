...
if($_POST['rh_modo'] == 'PAGINA'){
	...
	$sqlcount = "SELECT COUNT(nombre) AS paginas FROM rh_marca WHERE nombre LIKE '%".$_POST['buscacion']."%'";
	...
	$sql = "SELECT * FROM rh_marca WHERE nombre LIKE '%".$_POST['buscacion']."%' ORDER BY ".$_POST['ordenado']." ".$_POST['ad']." LIMIT ".(($_POST['pagina']-1)*20).",20";
...
if($_POST['rh_modo'] == 'EDITA'){
	...
	include('includes/session.inc');
	include('includes/SQL_CommonFunctions.inc');
	$sql = "SELECT * FROM rh_marca WHERE id = ".$_POST['id']."";
	$result = DB_query($sql,$db);
	$myrow = DB_fetch_array($result);
	echo $myrow['id']."||".$myrow['codigo']."||".$myrow['nombre'];
}
//GRABAR
if($_POST['rh_modo'] == 'GRABA'){
	...
	$checkSql = "SELECT count(*) FROM rh_marca WHERE codigo = '".str_replace(' ','',strtoupper($_POST['codigo']))."'";
	...
	if ($checkrow[0] > 0) {
		...
		$sql = "INSERT INTO rh_marca (codigo,nombre) VALUES ('".str_replace(' ','',strtoupper($_POST['codigo']))."','".$_POST['nombre']."')";
		$result = DB_query($sql,$db);
	}
	echo "||<FORM name='forma'>
	<input type='hidden' id='actulpagina' value='".$_POST['pagina']."'><input type='hidden' id='totalpagina' value='".$_POST['tpagina']."'><input type='hidden' id='ordenado' value='".$_POST['ordenado']."'><input type='hidden' id='ad' value='".$_POST['ad']."'>
...
if($_POST['rh_modo'] == 'ACTUALIZA'){
	...
	echo "<TABLE width=100%><TR><TD style='background:#b9ecb4;color:darkgreen;border:1px solid darkgreen'>MENSAJE: La Marca ".$_POST['nombre']." ha sido actualizada</TD></TR></TABLE>";
	$sql = "UPDATE rh_marca SET nombre = '".$_POST['nombre']."' WHERE id = '".$_POST['id']."'";
	$result = DB_query($sql,$db);
	echo "||<FORM name='forma'>
	<input type='hidden' id='actulpagina' value='".$_POST['pagina']."'><input type='hidden' id='totalpagina' value='".$_POST['tpagina']."'><input type='hidden' id='ordenado' value='".$_POST['ordenado']."'><input type='hidden' id='ad' value='".$_POST['ad']."'>
	<input type='text' id='buscacion' value='".$_POST['buscacion']."'><INPUT TYPE='button' VALUE='Buscar' onClick=buscararticulo()>
...
if($_POST['rh_modo'] == 'BORRA'){
	...
	$sql= "SELECT COUNT(*) FROM stockmaster WHERE rh_marca = '".$_POST['id']."'";
	$ErrMsg = _('The number of transactions using this customer/sales/pricelist type could not be retrieved');
	$result = DB_query($sql,$db,$ErrMsg);
	$myrow = DB_fetch_row($result);
	if ($myrow[0]>0) {
		echo "<TABLE width=100%><TR><TD style='background:#fddbdb;color:red;border:1px solid red'>ERROR: La Marca no pude ser borrada porque esta asignada a un producto</TD></TR></TABLE>";
	} else {
		...
		$sql2 = "DELETE FROM rh_marca WHERE id = '".$_POST['id']."'";
