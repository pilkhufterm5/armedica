...
/************************ BEGIN SQL TRANSACTIONS ************************/
	...
	// bowikaxu realhost - este costo debe de ser select case rh_type = 0 (variable * qty despues del select)
	// al final dividir entre el total de productos generados.
	
	// bowikaxu realhost march 2008 - divide costing
		if ($WORow['qtyrecd']==0){
			$CostResult = DB_query("SELECT 
									SUM((materialcost+labourcost+overheadcost)*bom.quantity)
									...
									$db); 
			$CostRow = DB_fetch_row($CostResult); // variable
			$CostResult = DB_query("SELECT 
									SUM((materialcost+labourcost+overheadcost)*bom.quantity)
									FROM stockmaster INNER JOIN bom
									ON stockmaster.stockid=bom.component
									...
									AND bom.loccode='" . DB_escape_string($WORow['loccode']) . "'",
									$db);
			$CostRow2 = DB_fetch_row($CostResult); // fijo
			if (is_null($CostRow[0]+$CostRow2[0]) OR $CostRow2[0]+$CostRow[0]==0){
					$Cost =0;
			} else {
					// bowikaxu realhost March 2008 - dividir costo entre la cantidad del articulo padre
					$Cost = $CostRow[0]*$WORow['qtyreqd']; // variable por la cantidad
					$Cost += $CostRow2[0]; // fijo entre la cantidad
					$Cost = $Cost/$WORow['qtyreqd'];
			}
		...
		while ($AutoIssueCompRow = DB_fetch_array($AutoIssueCompsResult)){
			...
			*/
			// bowikaxu realhost March 2008 - get item type
			$sql = "SELECT rh_type FROM bom WHERE component = '".$AutoIssueCompRow['stockid']."' AND parent = '".$_POST['StockID']."'";
			$res = DB_query($sql,$db);
			$rh_type = DB_fetch_array($res);
			if($rh_type['rh_type']==1){
			...
			$SQL = "UPDATE locstock
						SET quantity = quantity - " . ($AutoIssueCompRow['qtypu']). "
						WHERE locstock.stockid = '" . DB_escape_string($AutoIssueCompRow['stockid']) . "'
						AND loccode = '" . DB_escape_string($WORow['loccode']) . "'";
			...
			$SQL = "INSERT INTO stockmoves (stockid,
						...
						VALUES ('" . DB_escape_string($AutoIssueCompRow['stockid']) . "',
							...
							" . -($AutoIssueCompRow['qtypu']) . ",
							" . $AutoIssueCompRow['cost'] . ",
							" . ($QtyOnHandPrior - ($AutoIssueCompRow['qtypu'])) . ")";
			...
			$SQL = "UPDATE workorders SET
						costissued = costissued+" . ($AutoIssueCompRow['qtypu'] * $AutoIssueCompRow['cost']) ."
					WHERE wo=" . $_POST['WO'];
			...
			if ($_SESSION['CompanyRecord']['gllink_stock']==1 AND ($AutoIssueCompRow['qtypu'] * $AutoIssueCompRow['cost'])!=0){
			//if GL linked then do the GL entries to DR wip and CR stock
				...
				$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (28,
								...
								" . $PeriodNo . ",
								" . $StockGLCode['wipact'] . ",
								'" . $_POST['WO'] . ' - ' . DB_escape_string($_POST['StockID']) . ' ' . _('Component') . ': ' . DB_escape_string($AutoIssueCompRow['stockid']) . ' x ' . DB_escape_string($AutoIssueCompRow['qtypu']) . ' @ ' . number_format($AutoIssueCompRow['cost'],2) . "',
								" . ($AutoIssueCompRow['qtypu'] * $AutoIssueCompRow['cost']) . ")";
				...
				$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (28,
								...
								'" . $_POST['WO'] . ' - ' . DB_escape_string($_POST['StockID']) . ' -> ' . DB_escape_string($AutoIssueCompRow['stockid']) . ' x ' . DB_escape_string($AutoIssueCompRow['qtypu']) . ' @ ' . number_format($AutoIssueCompRow['cost'],2) . "',
								" . -($AutoIssueCompRow['qtypu'] * $AutoIssueCompRow['cost']) . ")";
					...
					// bowikaxu realhost March 2008 - Insert a la Relacion WoGl
					$SQL = "INSERT INTO rh_wogl (wo,
								type,
								transno,
								amount,
								narrative
							...
							VALUES (".$_POST['WO'].",
								...
								" . ($AutoIssueCompRow['qtypu'] * $AutoIssueCompRow['cost']) . ",
								'" . $_POST['WO'] . ' - ' . DB_escape_string($_POST['StockID']) . ' -> ' . DB_escape_string($AutoIssueCompRow['stockid']) . ' x ' . DB_escape_string($AutoIssueCompRow['qtypu']) . ' @ ' . number_format($AutoIssueCompRow['cost'],2) . "')";
					$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' .  _('The GL rh_wogl');
					$DbgMsg = _('The following SQL to insert the GLTrans record was used');
					$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
					
			}//end GL-stock linked
			}else { // bowikaxu - el articulo es variable, inserts quedan igual
				//Need to get the previous locstock quantity for the component at the location where the WO manuafactured
			$CompQOHResult = DB_query("SELECT locstock.quantity
									FROM locstock
									WHERE locstock.stockid='" . DB_escape_string($AutoIssueCompRow['stockid']) . "'
