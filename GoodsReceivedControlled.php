...
echo '<BR><FONT SIZE=2><B>'. _('Receive controlled item'). ' ';
if(isset($_REQUEST['Barras'])) echo ' - '.htmlentities($_REQUEST['Barras']);
echo 
	 $LineItem->StockID ; 
echo  ' - ' . $LineItem->ItemDescription .
	' ' . _('on order') . ' ' . $_SESSION['PO']->OrderNo . ' ' . _('from') . ' ' . $_SESSION['PO']->SupplierName . '</B></FONT>';

/** vars needed by InputSerialItem : **/


if(isset($LineItem->id_agrupador)){
	$ArticulosSeleccionados=$Articulos=array();
	$SQL="select * from stockmaster where is_farmacia=1 and id_agrupador='".$LineItem->id_agrupador."'";
	$LineItemsResult = db_query($SQL,$db,$ErrMsg,$DbgMsg);
		  if (db_num_rows($LineItemsResult) > 0) {
				while ($myrow=db_fetch_assoc($LineItemsResult)) {
					$ArticulosSeleccionados[$myrow['stockid']]=$myrow['barcode'].' <br >'.$myrow['stockid'].' <br >'.$myrow['description'];
				}
		  }
	if(isset($_REQUEST['Barras'])){
		$barras=explode(',',trim($_REQUEST['Barras'],','));
		$barras=array_map(function($d){return DB_escape_string(trim($d));},$barras);
		$SQL.=' and barcode in("'.(implode('","',$barras)).'")';
	}
	
	$LineItemsResult = db_query($SQL,$db,$ErrMsg,$DbgMsg);
		  if (db_num_rows($LineItemsResult) > 0) {
				while ($myrow=db_fetch_assoc($LineItemsResult)) {
					$Articulos[$myrow['stockid']]=$myrow['barcode'].' - '.$myrow['stockid'].' - '.$myrow['description'];
				}
		  }
}
