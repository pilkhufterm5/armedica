...
$sql = "SELECT *
		FROM worksorders
		WHERE worksorders.woref >= ".$FromWORef."
		AND worksorders.woref  <=" . $ToWORef;
...
//} elseif (DB_num_rows($result)>=1){ /*There is only one order header returned - thats good! */
	...
	// Ciclo para todas las worksorders regresadas
	
	while($myrow = DB_fetch_array($result)){

		$CurrentWORef = $myrow['woref'];
		/*
...
$sql = "SELECT *
	FROM wodetail
	 WHERE wodetail.woref =" . $CurrentWORef;
...
/*Yes there are line items to start the ball rolling with a page header */
	...
	$LeftOvers = $pdf->addTextWrap(13,$YPos,135,$FontSize,_('A Producir'));
	$YPos -= ($line_height);
	...
	while ($myrow2=DB_fetch_array($result)){
		...
		$DetailsCost = number_format($myrow2['cost'],2);
...
$sql = "SELECT *
	FROM worequirements
	 WHERE worequirements.woref =" . $CurrentWORef;
