...
if ( isset($_POST['AddBatches']) && $_POST['AddBatches']!='') {
	...
	for ($i=0;$i < 10;$i++){
		if(strlen($_POST['SerialNo' . $i])>0 && $_POST['SerialNo' . $i]!='-1'){
			if ($ItemMustExist){
				$ExistingBundleQty = ValidBundleRefPedimento ($StockID, $LocationOut, $_POST['SerialNo' . $i]);
				if ($ExistingBundleQty >0){
					...
					if ($AddThisBundle==true && $_POST['SerialNo' . $i]>=0){

						$LineItem->PedimentoItems[$_POST['SerialNo' . $i]] = new PedimentoItem($_POST['SerialNo' . $i],$_POST['Fecha' . $i],$_POST['Aduana' . $i], ($InOutModifier>0?1:-1) * $_POST['Qty' . $i]);
					}
			   ...
			   /*	if ($LineItem->Serialised){
				...
				if (!$SerialError && $_POST['SerialNo' . $i]>=0){
					$LineItem->PedimentoItems[$NewSerialNo] = new PedimentoItem($_POST['SerialNo' . $i],$_POST['Fecha' . $i],$_POST['Aduana' . $i], $NewQty);
				}
			}
...
if (isset($_GET['Delete'])){
        unset($LineItem->PedimentoItems[$_GET['Delete']]);
}
?>
