			...
			if ($ItemMustExist){
				...
				if ($ExistingBundleQty >0){
					...
					if ($AddThisBundle==true){
						$LineItem->SerialItems[$_POST['SerialNo' . $i]] = new SerialItem ($_POST['SerialNo' . $i], ($InOutModifier>0?1:-1) * $_POST['Qty' . $i], $_POST['expirationdate'.$i]);
						IF(isset($_POST['StockID' . $i]))
							$LineItem->SerialItems[$_POST['SerialNo' . $i]]->stockid=$_POST['StockID' . $i];
					}
                ...
                ***************************************************************************/
                ...
                /*********************************************************************************************************************/
					...
					$NewSerialNo = stripslashes(stripslashes($NewSerialNo));
					$NewSerialNo = str_replace(' ',' ',$NewSerialNo);
					$NewSerialNo = str_replace('  ',' ',$NewSerialNo);
					$NewSerialNo = str_replace('   ',' ',$NewSerialNo);
					$NewSerialNo = str_replace('\'','',$NewSerialNo);
					$NewSerialNo = str_replace('+','',$NewSerialNo);
					...
					$NewSerialNo = str_replace('.','',$NewSerialNo);
                /*********************************************************************************************************************/
					$LineItem->SerialItems[$NewSerialNo] = new SerialItem ($_POST['SerialNo' . $i], $NewQty, $_POST['expirationdate'.$i]);
					IF(isset($_POST['StockID' . $i]))
							$LineItem->SerialItems[$NewSerialNo]->stockid=$_POST['StockID' . $i];
				}
                                /**************************************************************************
                                ***************************************************************************/
			}
...
/*********************************************************************************************************************/
		...
		$SerialNo = stripslashes(stripslashes($SerialNo));
...
/*********************************************************************************************************************/
		if ($LineItem->Serialised==1){	/*only if the item is serialised */
			...
			IF(isset($_POST['StockID'][$i]))
					$LineItem->SerialItems[$_POST['Bundles'][$i]]->stockid=$_POST['StockID'][$i];
		} else {
            ...
            if ($Qty != 0) {
				...
				IF(isset($_POST['StockID'][$i]))
					$LineItem->SerialItems[$SerialNo]->stockid=$_POST['StockID'][$i];
			}
		}
	}
...
if ( isset($_POST['AddSequence']) && $_POST['AddSequence']!='') {
	...
	if ($BeginNo > $EndNo){
		...
		while ($myrow=db_fetch_array($SeqItems)) {
			...
			$LineItem->SerialItems[$myrow['serialno']]->stockid=$StockID;
			...
			if ($ExistingBundleQty >0){
				$AddThisBundle = true;
					...
					if ($NewQty > $ExistingBundleQty){
							if ($LineItem->Serialised ==1){
									echo '<BR>' . '<a href="StockSerialItemResearch.php?serialno='. $NewSerialNo . '" target=_blank>'.$NewSerialNo. '</a> '. _('has already been sold'). '.';
									$AddThisBundle = false;
								} elseif ($ExistingBundleQty==0) { /* and its a batch */
									...
									echo '<BR>'. _('There is only') . ' ' . $ExistingBundleQty . ' '. _('of') . ' ' . 
												'<a href="StockSerialItemResearch.php?serialno='. $NewSerialNo . '" target=_blank>'.$NewSerialNo. '</a> ' . _('remaining') . '. '. 
												_('The entered quantity will be reduced to the remaining amount left of this batch/bundle/roll');
					...
					if ($AddThisBundle==true){
							...
							$LineItem->SerialItems[$NewSerialNo]->stockid=$StockID;
					}
			} /*end if ExistingBundleQty >0 */
			else {
				echo '<BR>';
				prnMsg( '<a href="StockSerialItemResearch.php?serialno='. $NewSerialNo . '" target=_blank>'.$NewSerialNo. '</a>  ' . _('not available') ,'', 'Notice' );
			}
