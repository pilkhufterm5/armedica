...
<head>
    ...
    <link rel="stylesheet" href="<?php echo $Path2;?>/login.css" type="text/css" />
</head>
	...
	if ($AllowCompanySelectionBox == true){
		...
		while (false != ($CompanyEntry = $DirHandle->read())){
			...
			//if($CompanyEntry{0}!='.'&&is_file('companies/' . $CompanyEntry.'/config.php'))
			 if(
//count(explode('.',$_SERVER["HTTP_HOST"]))!=4&&
			$CompanyEntry{0}!='.'&&is_file('companies/' . $CompanyEntry.'/config.php')&&(strpos(' '.$CompanyEntry,$WebERP[0])==1)/**/
//)//PRODUCCION
...
/*||
count(explode('.',$_SERVER["HTTP_HOST"]))==4&&
is_dir('companies/' . $CompanyEntry) AND $CompanyEntry != '..' AND $CompanyEntry != 'CVS' AND $CompanyEntry!='.svn' AND $CompanyEntry!='.'/**/
)
			$show=true;
			if($show){
...
<script language="JavaScript" type="text/javascript">
document.getElementById('logo_empresa').innerHTML = '<CENTER><img width=70% src="<?php echo $LogoURL;?>/'+document.getElementById('CompanyNameField').value+'/logo.jpg"></CENTER>';
function cambiologo(q){
	document.getElementById('logo_empresa').innerHTML = '<CENTER><img width=70% src="<?php echo $LogoURL;?>/'+q+'/logo.jpg"></CENTER>';
}
</script>
<!--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////-->
</body>
