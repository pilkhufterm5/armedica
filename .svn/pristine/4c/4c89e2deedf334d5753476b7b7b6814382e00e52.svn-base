...
if (DB_num_rows($result)==1){
	...
	if (DB_num_rows($result)>0){
		...
		include('rh_templateheaderRamosContado.inc.php');

		//no factura
		if ($rh_tipo==10){
			$pdf->addTextWrap(481.5,530,100,$rh_fontsize+1,$extInvoice.$ExtRes['extinvoice'], 'right');
			$YPos = 415-32 - $line_height - 6 - $line_height - $line_height - 2 + 19;
		}else{
...
$rh_fontsize=$rh_fontsize+2;
		while ($myrow2=DB_fetch_array($result)){
			...
			{
				//cantidad
				$pdf->addTextWrap(362,$YPos,54,$rh_fontsize-1,$DisplayQty, 'right');

				//precio
				$pdf->addTextWrap(423,$YPos,72,$rh_fontsize-1,$rh_DisplayPrice, 'right');

				//total
				$pdf->addTextWrap(504-5,$YPos,81,$rh_fontsize-1,$DisplayNet, 'right');

				//Codigo
				$pdf->addTextWrap(30,$YPos,81,$rh_fontsize-2.5,$myrow2['stockid'], 'left');
			}
			else
			{
				//total
			...
			for($i = 0; $i < count($rh_arraydesc1); $i=$i + 1)
			{
				if(strlen($rh_arraydesc1[$i]) >= 115)
				{
					...
					while($nuevo >= 105)
					{
						...
						$pdf->addTextWrap(30,156-$i*10-1+32+19,340,$rh_fontsize-2,$nueva_cadena, 'left');
						$pdf->addTextWrap(30,156-$i*10-12+32+19,340,$rh_fontsize-2,$nueva_cadena2, 'left');
						$rh_sumar = 12;
					}
				}
				else
				$pdf->addTextWrap(30,156-$i*10-$rh_sumar+32+19,340,$rh_fontsize-2,$rh_arraydesc1[$i], 'left');
		...
		if ($rh_tipo==10)
		{
			...
			$pdf->y = $Page_Height - 129 - 30 - 32 - 12 + 20 - 19;
			$pdf->x = 30;
			$pdf->SetFontSize($rh_fontsize-3);
			$pdf->MultiCell(432,$line_height,strtoupper($ConLetra),0,'J',0,15);

			//subtotal
			$pdf->addTextWrap(504-8,97-51+60+40+10+11-20+19,81,$rh_fontsize+1,$DisplaySubTot, 'right');
	/*
			//iva %
	...
	*/
			//iva
			$pdf->addTextWrap(504-8,75-51+60+40+10+11-20+19,81,$rh_fontsize+1,$DisplayTax, 'right');

			//total
			$pdf->addTextWrap(504-8,51-51+60+40+10+11-20+19,81,$rh_fontsize+1,$DisplayTotal, 'right');
		}
		else
		{
			//total letra
