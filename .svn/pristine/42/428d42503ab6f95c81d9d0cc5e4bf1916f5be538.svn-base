...
If (isset($_POST['PrintPDF']) && isset($_POST['FromCust']) && $_POST['FromCust']!=''){
    ...
    */
    if(isset($_POST['FromCust']) && !empty($_POST['FromCust']) && isset($_POST['ToCust']) && !empty($_POST['ToCust'])){
        $DebtorFrom = mysql_real_escape_string($_POST['FromCust']);
        $DebtorTo = mysql_real_escape_string($_POST['ToCust']);
...
/*Figure out who all the customers in this range are */
	...
	$sql = "SELECT debtorsmaster.debtorno,
			...
			currencies.currency,
			paymentterms.terms
		FROM debtorsmaster INNER
		JOIN currencies ON debtorsmaster.currcode=currencies.currabrev
		JOIN rh_titular ON debtorsmaster.debtorno = rh_titular.debtorno
		INNER JOIN paymentterms
			ON debtorsmaster.paymentterms=paymentterms.termsindicator
		WHERE 1=1 {$BetWeenDebtor}
...
/*now get all the settled transactions which were allocated this month */
		...
		//SAINTS modificación de consulta para obtener series y folios de FE 31/01/2011
	   		$sql = "SELECT DISTINCT debtortrans.id,
						...
						debtortrans.alloc,
						debtortrans.ovamount+debtortrans.ovdiscount+debtortrans.ovfreight+debtortrans.ovgst-debtortrans.alloc AS ostdg
				FROM debtortrans left join rh_cfd__cfd c on c.id_debtortrans = debtortrans.id INNER JOIN systypes
					ON debtortrans.type=systypes.typeid
			...
			if ($_SESSION['Show_Settled_LastMonth']==1){
				if (DB_num_rows($SetldTrans)>=1) {
					...
					while ($myrow=DB_fetch_array($SetldTrans)){
						...
						//SAINTS series y folios de FE 31/01/2011
						if ($myrow['folio']!="")
							$LeftOvers = $pdf->addTextWrap($Left_Margin+61,$YPos,50,$FontSize,$myrow['serie'].$myrow['folio'].'('.$myrow['transno'].')', 'left');
						else
							$LeftOvers = $pdf->addTextWrap($Left_Margin+61,$YPos,50,$FontSize,$ExtInvoice['rh_serie'].$ExtInvoice['extinvoice'].'('.$myrow['transno'].')', 'left');
		    			//SAINTS fin
						}else {
		...
		/* head up a new statement page */
			...
			//SAINTS fin
						...
						//SAINTS series y folios de FE 31/01/2011
						if ($ExtInvoice2['folio']!="")
							$LeftOvers = $pdf->addTextWrap($Left_Margin+61,$YPos,50,$FontSize,$ExtInvoice2['serie'].$ExtInvoice2['folio'].'('.$myrow['transno'].')', 'left');
						else
							$LeftOvers = $pdf->addTextWrap($Left_Margin+61,$YPos,50,$FontSize,$ExtInvoice['rh_serie'].$ExtInvoice['extinvoice'].'('.$myrow['transno'].')', 'left');
						//SAINTS fin
	...
	/*if FromTransNo is not set then show a form to allow input of either a single statement number or a range of statements to be printed. Also get the last statement number created to show the user where the current range is up to */
		...
		echo '<TR><TD>' . _('Starting Customer statement to print (Customer code)'). '
			...
			<TR><TD>'. _('Ending Customer statement to print (Customer code)').'</TD><TD>
				<input Type=text max=6 size=7 name=ToCust value=""></TD></TR></TABLE>
				<BR>
