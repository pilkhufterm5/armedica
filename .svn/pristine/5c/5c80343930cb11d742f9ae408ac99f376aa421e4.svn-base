...
if(isset($_POST['client'])){
	foreach ($_POST['client'] as $llave => $valor){
		$_SESSION['app_data']['client'][$llave] = $valor;
		//$test = explode(" - ",$valor);
		//echo ' #'.$llave.' @ '.$valor;
	}
	?>
	<script type='text/javascript' language='javascript'>
	window.opener.document.getElementById("rhbutton1").click();
	window.close();
	</script>
	<?php
}
...
if (isset($_POST['Search']) OR isset($_POST['Go']) OR isset($_POST['Next']) OR isset($_POST['Previous'])){
	...
	If (($_POST['Keywords']=="") AND ($_POST['CustCode']=="") AND ($_POST['CustPhone']=="") AND ($_POST['RFC']=="")) {
...
?>
<FORM ACTION="<?php echo $_SERVER['PHP_SELF'] . '?id_app=' . $_GET['id_app']; ?>" METHOD=POST>
<?php
//Realhost Alberto 29/Agosto/07
foreach($_POST as $llave => $valor){
	echo "<input type='hidden' name='" . $llave . "' value='" . $valor . "'>";
}
//end Alberto
?>
<CENTER>
...
<?php // bowikaxu - april 2007 search by taxref 
?>
<TD><FONT SIZE=3><B><?php echo _('OR'); ?></B></FONT></TD>
<TD><B><?php echo _('RFC'); ?></B>:</TD>
<TD>
<?php
if (isset($_POST['RFC'])) {
?>
...
<?php
  ...
  echo '<BR><BR>';
	echo "<center><form id='upd_app' action='app_select_cust.php' method='post'>";
  ...
  if (DB_num_rows($result)<>0){
	...
	while (($myrow=DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
		...
		printf("<td><FONT SIZE=1><input type='checkbox' name='client[]' value='%s'></FONT></td>
...
//end of while loop
	...
	echo '</TABLE>';
	echo "<input type='button' onclick='javascript:enviar_upd()' value='Agregar'>";
	echo "<input type='hidden' name='id_app' value='" . $_GET['id_app'] . "'>";
	$_SESSION['app_data']['submit']="add_client";
	foreach($_POST as $llave => $valor){
		echo "<input type='hidden' name='" . $llave . "' value='" . $valor . "'>";
	}
	echo "</form></center>";
...
function enviar_upd(){
	document.getElementById("upd_app").submit();
}
            //-->
    //]]>
</script>