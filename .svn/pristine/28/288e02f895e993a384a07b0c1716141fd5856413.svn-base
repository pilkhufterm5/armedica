	...
	While ($AgedAnalysis = DB_fetch_array($CustomerResult,$db)){
		...
		if ($_POST['DetailedReport']=='Yes'){
		   ...
		   $pdf->line($Page_Width-$Right_Margin, $YPos+10,$Left_Margin, $YPos+10);
				...
				$sql = "SELECT rh_get_serie_folio(debtortrans.id) as seriefolio, systypes.typename,
			   			debtortrans.transno,
		    ...
		    while ($DetailTrans = DB_fetch_array($DetailResult)){
			    ...
			    $LeftOvers = $pdf->addTextWrap($Left_Margin+65,$YPos,60,$FontSize,$DetailTrans['seriefolio'] . '('.$DetailTrans['transno'].')','left');
