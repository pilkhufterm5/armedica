...
// bowikaxu realhost - april 2008
if(isset($_GET['DiscAmt']) && $_GET['DiscAmt']>0){
	
	prnMsg('Generando datos del descuento...','success');
	
	$sql = "SELECT chartmaster.accountcode, chartmaster.accountname FROM chartmaster WHERE accountcode = '".$_SESSION['CompanyRecord']['pytdiscountact']."'";
	$res = DB_query($sql,$db);
	$actname = DB_fetch_array($res);
	
	$_SESSION['SuppTrans']->Add_GLCodes_To_Trans($actname['accountcode'],
								$actname['accountname'],
								$_GET['DiscAmt'],
								'',
								'');
