...
*/
function create_order($products, $subtotal, $tax, $total, $customer_id, $branch,$fecha,$invtext) {
	global $db;
	...
	$OrderHeaderSQL = "SELECT
			...
			AND custbranch.debtorno = '".$customer_id."'
			AND locations.loccode=custbranch.defaultlocation
	...
	//$_SESSION['Items']->OrderNo = $myrow['orderno'];create_order
	...
	for($i = 0; $i < count($products); $i++) {
		...
		if (db_num_rows($LineItemsResult)>0) {
			...
			$price = GetPrice ($myrow['stockid'], $customer_id ,'', $db);
			$discount = 0;
			// bowikaxu set the price
		...
		}else { // no exixte SALDO_INICIAL
				
				echo "NO EXISTE EL ARTICULO SALDO_INICIAL";
				exit;
	...
	foreach ($_SESSION['Items']->LineItems as $StockItem) {
		...
		$StockItem->LineNumber . ",
					...
					'" . DB_escape_string($StockItem->Narrative) . "', 1
				)";
	...
	$date = explode('-',$fecha);
	$DefaultDispatchDate = $date[2].'-'.$date[1].'-'.$date[0];
	...
	foreach ($_SESSION['Items']->LineItems as $OrderLine) {
		...
		if ($MBFlag=="B" OR $MBFlag=="M") {
			...
			$SQL = "SELECT bom.component,
					...
					AND bom.effectiveto > '" . $fecha . "'
					AND bom.effectiveafter < '" . $fecha . "'";
		...
		/*if ($_SESSION['CompanyRecord']['gllink_stock']==1 AND $OrderLine->StandardCost !=0){
			...
			/*$SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						'" . $_SESSION['Items']->DebtorNo . " - " . $OrderLine->StockID . " x " . $OrderLine->Quantity . " @ " . $OrderLine->StandardCost . "',
		...
		/*if ($_SESSION['CompanyRecord']['gllink_debtors']==1 AND $OrderLine->Price !=0){
			...
			$SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						'" . $_SESSION['Items']->DebtorNo . " - " . $OrderLine->StockID . " x " . $OrderLine->Quantity . " @ " . $OrderLine->Price . "',
			...
			AND $_POST['CreditType']!='ReverseOverCharge'){
				...
				$SQL = "INSERT INTO gltrans (type,
						...
						VALUES (11,
							...
							" . $PeriodNo . ",
							" . $COGSAccount . ",
							'" . $_SESSION['CreditItems']->DebtorNo . " - " . $OrderLine->StockID . " x " . $OrderLine->Quantity . " @ " . $OrderLine->StandardCost . "',
							" . -round($OrderLine->Price * $OrderLine->Quantity,2) . "
							)";
			...
			if ($_SESSION['CompanyRecord']['gllink_debtors']==1 AND $OrderLine->Price !=0){
				...
				$SQL = "INSERT INTO gltrans (type,
						...
						VALUES (11,
							...
							" . $PeriodNo . ",
							" . $SalesGLAccounts['salesglcode'] . ",
							'" . $_SESSION['CreditItems']->DebtorNo . " - " . $OrderLine->StockID . " x " . $OrderLine->Quantity . " @ " . $OrderLine->Price . "',
				...
				if ($OrderLine->DiscountPercent !=0){
					...
					$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (11,
								...
								" . -round(($OrderLine->Price * $OrderLine->Quantity * $OrderLine->DiscountPercent)/$_SESSION['CurrencyRate'],2) . "
								)";
