...
If ($_POST['ResetPart']){
		...
		unset($SelectedStockItem);
		unset($_POST['SelectedStockItem']);
		unset($_GET['SelectedStockItem']);
}
...
If (isset($OrderNumber)) {
	...
	If (isset($SelectedCustomer)) {
		...
		$_2GetAfilData = "SELECT ti.folio, cobranza.cobrador, fa.tipo_membresia
                               ...
                               WHERE ti.debtorno = '{$SelectedCustomer}'";
        $_GetAfilData=DB_query($_2GetAfilData,$db);
        $GetAfilData = DB_fetch_assoc($_GetAfilData);

		echo _('For') . ' ' . $GetAfilData['tipo_membresia'] . ' - ' . $GetAfilData['folio'] . ' - ' . $_REQUEST['SelectedCustomer'] . ' ' . _('and') . ' ';
		echo "<input type=hidden name='SelectedCustomer' value='$SelectedCustomer'>";
	}

	If (isset($SelectedStockItem)) {

		echo _('for the part1') . ': ' . $SelectedStockItem . ' ' . _('and') . ' ' ."<input type=hidden name='SelectedStockItem' value='$SelectedStockItem'>";
		// bowikaxu - april 2007 - limpiar archivo de memoria
		echo "<INPUT TYPE=Submit NAME='ResetPart' VALUE='Limpiar'>";
...
if ($_POST['SearchParts']!=''){
	...
	if (strlen($SQL)<2){
		...
		if (DB_num_rows($StockItemsResult)==1){
			...
			$_POST['SearchOrders']='True';
		  	...
		  	echo '<BR>' . _('For the part2') . ': ' . $SelectedStockItem . ' ' . _('and') . " <input type=hidden name='SelectedStockItem' value='$SelectedStockItem'>";
		}
	}
	...
	if (isset($OrderNumber)) {
			$SQL = "SELECT salesorders.orderno,
				...
				AND salesorders.orderno='". DB_escape_string($OrderNumber) ."'
			...
			$SQL = "SELECT salesorders.orderno,
				...
				AND salesorders.debtorno = debtorsmaster.debtorno
				AND debtorsmaster.debtorno = custbranch.debtorno
				AND salesorders.customerref like '%". DB_escape_string($CustomerRef)."%'
				AND salesorders.quotation=0
				GROUP BY salesorders.orderno,
					debtorsmaster.name,
					custbranch.brname,
...
if ($OrderNumber=='' OR !isset($OrderNumber)){
	// bowikaxu - se agrego el calendar popup
	echo '<TR><TD>' . _('Order Number') . ':</TD><TD>' . "<INPUT type=text name='OrderNumber' MAXLENGTH =8 SIZE=9></TD><TD rowspan=2>" . _('for all orders placed after') .
			": </TD><TD rowspan=2><INPUT type=text name='OrdersAfterDate' MAXLENGTH =10 SIZE=11 value=" . $_POST['OrdersAfterDate'] . ">
			<a href=\"#\" onclick=\"form.OrdersAfterDate.value='';cal.select(document.forms['form'].OrdersAfterDate,'from_date_anchor','d/M/yyyy');
...
/*show a table of the orders returned by the SQL */
	...
	while ($myrow=DB_fetch_array($SalesOrdersResult)) {
		...
		// bowikaxu - april 2007 - get order user
		$sql = "SELECT user_ FROM rh_usertrans WHERE order_ = ".$myrow['orderno']."";
		$UsrRes = DB_query($sql,$db);
		$ord_user = DB_fetch_array($UsrRes);

		if($myrow['rh_status']==1){
			$Comment = _('Cancel');
				// bowikaxu realhost - may 2007 - show alt text with status
		printf("<td><A target='_blank' HREF='%s' TITLE='".$Comment."'><FONT COLOR='red'><STRONG>%s</FONT></STRONG></A></td>
			<td><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			<td><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			<td><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			<td><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			<td><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			<td><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			<td><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			<td ALIGN=RIGHT><FONT COLOR='red'><STRONG>%s</FONT></STRONG></td>
			</tr>",
			$ViewPage,
			$myrow['orderno'],
			$ord_user['user_'],
			...
			$Comment = $myrow['comments'];
				// bowikaxu realhost - may 2007 - show alt text with status
		printf("<td><A target='_blank' HREF='%s' TITLE='".$Comment."'>%s</A></td>
			<td>%s</td>
			<td>%s</td>
			<td>%s</td>
			<td>%s</td>
