...
/*CustomerID defined in header.inc */
	...
	echo '</TABLE><BR>';
	/*
	$invoices = preg_split("/(Rem \n*)/", $_SESSION['Items']->Comments);
	if(!is_numeric($invoices)){
		$invoices = "Ya Facturada";
	}
	foreach($invoices as $inv){
		if (trim($inv)!=''){
			echo '<A HREF="' . $rootpath. '/rh_PDFRemGde.php?FromTransNo=' . $inv . '&InvOrCredit=Invoice&PrintPDF=Yes" target=_blank>' . _('Rem') . '# '. $inv . '</A><BR>';
		}
	}
...
$LineItemsSQL = 'SELECT
				...
				actualdispatchdate,
				qtyinvoiced
			FROM stockmoves, salesorderdetails, stockmaster
			WHERE salesorderdetails.stkcode = stockmaster.stockid AND orderno =' . $_GET['OrderNumber'].'
			 AND reference = '.$_GET['OrderNumber'].' group by orderlineno';

	$ErrMsg =  _('The line items of the order cannot be retrieved because');
	$DbgMsg =  _('The SQL used to retrieve the line items, that failed was');
	$LineItemsResult = db_query($LineItemsSQL,$db, $ErrMsg, $DbgMsg);
	
	echo "<br>".db_num_rows($LineItemsResult)."<br>";

	if (db_num_rows($LineItemsResult)>0) {

		while ($myrow=db_fetch_array($LineItemsResult)) {
			
//			echo " xxx: ".$myrow['units']." xxx ".$myrow['quantity']." xxx ".$myrow['qtyinvoiced'];
...
// MODIFICAR LA VARIABLE que apunta a $myrow['quantity'] para que sea el valor guardado en stockmoves

//print_r($_SESSION['Items']);

// TERMINA MODIFICAR LA CANTIDAD CORRECTA DE LA REMISION
...
foreach ($_SESSION['Items']->LineItems as $StockItem) {
	...
	$_SESSION['Items']->totalVolume = $_SESSION['Items']->totalVolume + $StockItem->qty * $StockItem->Volume;
	$_SESSION['Items']->totalWeight = $_SESSION['Items']->totalWeight + $StockItem->qty * $StockItem->Weight;
}
...
echo "<HR><a href=rh_ViewRemisiones.php>"._('Return to Remisiones')."</a>";
