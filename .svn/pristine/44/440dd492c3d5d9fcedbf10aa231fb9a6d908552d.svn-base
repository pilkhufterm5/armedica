...
if (isset($_POST['DoIt']) AND strlen($_POST['EmailAddr'])>3){
	echo "<META HTTP-EQUIV='Refresh' CONTENT='0; URL=" . $rootpath . '/rh_PrintCustTrans_Shipment.php?' . SID . '&FromTransNo=' . $_POST['TransNo'] . '&PrintPDF=Yes&InvOrCredit=' . $_POST['InvOrCredit'] .'&Email=' . $_POST['EmailAddr'] . "'>";
	prnMsg(_('The transaction should have been emailed off') . '. ' . _('If this does not happen') . ' (' . _('if the browser does not support META Refresh') . ')' . "<a href='" . $rootpath . '/rh_PrintCustTrans_Shipment.php?' . SID . '&FromTransNo=' . $_POST['FromTransNo'] . '&PrintPDF=Yes&InvOrCredit=' . $_POST['InvOrCredit'] .'&Email=' . $_POST['EmailAddr'] . "'>" . _('click here') . '</a> ' . _('to email the customer tranaction'),'success');
	exit;
...
echo '<TR><TD>' . _('Email') . ' Remision ' . _('number') . ' ' . $_GET['FromTransNo'] . ' ' . _('to') . ":</TD>
