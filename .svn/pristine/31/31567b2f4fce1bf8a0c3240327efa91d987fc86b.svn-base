...
/**
 * REALHOST 2008
 * $LastChangedDate: 2008-03-04 09:04:50 -0600 (Tue, 04 Mar 2008) $
 * $Rev: 106 $
 */
$PageSecurity = 11;
		...
		/*now the credit Stock entry*/
			...
			// bowikaxu realhost March 2008 - Insert a la Relacion WoGl
					$SQL = "INSERT INTO rh_wogl (wo,
							...
							VALUES (".DB_escape_string($_POST['WO']).",
								28,
								" . $WOIssueNo . ",
								" . ($IssueItemRow['cost'] * $QuantityIssued) . ",
								'" . DB_escape_string($_POST['WO']) . " " . DB_escape_string($_POST['IssueItem']) . ' x ' . DB_escape_string($QuantityIssued) . " @ " . number_format($IssueItemRow['cost'],2) . "')";
...
// bowikaxu realhost Feb 2008 - C item type
		if ($_POST['StockCat']=='All'){
			$SQL = "SELECT stockmaster.stockid,
					...
					AND (mbflag='B' OR mbflag='M' OR mbflag='D' OR mbflag='C')
...
if (!isset($_POST['IssueItem'])){ //no item selected to issue yet
	...
	while ($RequirementsRow = DB_fetch_array($RequirmentsResult)){
		...
		// bowikaxu - get req qty if fixed
			$sql = "SELECT rh_type FROM bom WHERE component = '".$RequirementsRow['stockid']."' AND parent = '".DB_escape_string($_POST['StockID'])."'";
