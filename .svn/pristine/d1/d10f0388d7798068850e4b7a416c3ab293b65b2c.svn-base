...
if (DB_num_rows($result)==1){
	...
	if (DB_num_rows($result)>0){
		...
		include('rh_templateheader1.inc.php');

		//no factura
		if ($rh_tipo==10){		
			$pdf->addTextWrap(481.5+3,520,100,$rh_fontsize,$extInvoice.$ExtRes['extinvoice'], 'right');
			$YPos = 390 - $line_height + 9;
		}else{
			$pdf->addTextWrap(451.5,530-5,100,$rh_fontsize,$extInvoice.$extCN, 'right');
...
$rh_fontsize=$rh_fontsize+2;
		...
		while ($myrow2=DB_fetch_array($result)){
			...
			{	
				//cantidad
				$pdf->addTextWrap(362-25,$YPos,54,$rh_fontsize-1,$DisplayQty, 'right');
				
				//precio
				$pdf->addTextWrap(423-25,$YPos,72,$rh_fontsize-1,$rh_DisplayPrice, 'right');

				//total
				$pdf->addTextWrap(504-25,$YPos,81,$rh_fontsize-1,$DisplayNet, 'right');

				//Codigo
				$pdf->addTextWrap(30-20,$YPos,81,$rh_fontsize-2.5,$myrow2['stockid'], 'left');
			}
			else
			{
				//total
				...
				//$pdf->addTextWrap(30+8,$YPos,81,$rh_fontsize-2.5,$myrow2['stockid'], 'left');
                                $facturas = explode(',',$numF[1]);
				$pdf->addTextWrap(30+8,$YPos,81,$rh_fontsize-2.5,implode('-', $facturas), 'left');

                                //Descripcion
                                $descripcion = $numberFact[1];
			}
			...
			if ($rh_tipo==10){
				...
				$pdf->addTextWrap(105,$YPos,$showCar,$rh_fontsize-2,$myrow2['narrative'], 'left');
                                //$pdf->addTextWrap(105,$YPos,$showCar,$rh_fontsize-2,$descripcion, 'left');
				$YPos -= $line_height;
			}
			//Eleazar
			//Realhost
		...
		if ($rh_tipo==10)
		{
                        ...
                        $pdf->addTextWrap(36-25,110+9,280,$rh_fontsize-2,"Rem: ".$listRem, 'left');


			//total letra
			//$pdf->addTextWrap(126,153,288,8,"(".$ConLetra.")", 'left');
			$pdf->y = $Page_Height - 129 + 51 - 9;
			$pdf->x = 36-25;
			$pdf->SetFontSize($rh_fontsize-2);
			$pdf->MultiCell(432,$line_height,strtoupper($ConLetra),0,'J',0,15);

			//subtotal
			$pdf->addTextWrap(504-25,97-51+9,81,$rh_fontsize+1,$DisplaySubTot, 'right');
	/*
			//iva %
	...
	*/
			//iva
			$pdf->addTextWrap(504-25,75-51+9,81,$rh_fontsize+1,$DisplayTax, 'right');

			//total
			$pdf->addTextWrap(504-25,51-51+9,81,$rh_fontsize+1,$DisplayTotal, 'right');
		}
		else
		{
			//total letra
	...
	*/
			...
			/*$pdf->addTextWrap(490,338-51-20,81,$rh_fontsize+1,$DisplayTax, 'right');*/
                         ...
                         * iJPe
                         * 2010-04-08
                         * Codigo utilizado para las notas de cargo realizadas por diferencia en IVA
