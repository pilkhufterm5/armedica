...
/*read in all the guff from the selected invoice into the Items cart	*/
	...
	$InvoiceHeaderSQL = "SELECT DISTINCT
			...
			FROM rh_invoiceshipment, debtortrans INNER JOIN debtorsmaster ON
			...
			WHERE debtortrans.transno = " . $_GET['InvoiceNumber'] . "
				...
				AND rh_invoiceshipment.Shipment = ".$_GET['InvoiceNumber']."
				AND stockmoves.type=20000";
// bowikaxu se modificaron los tipos a 20000 para que sea remision y no factura
...
// bowikaxu aqui esta el problema parece que es porque la remision tiene mas de un articulo
...
/*CustomerID variable registered by header.inc */
		...
		if($myrow['Facturado']!=0){
			
			echo _('<strong> <center> Esta Remision ya ha sido facturada ') . $_GET['InvoiceNumber'];
			echo "<br> Crear la nota de credito a partir de la Factura <center> <strong>";
			include('includes/footer.inc');
			exit;
		...
		if($myrow['rh_status']=='C'){
			...
			echo _('<strong> <center> Esta Remision fue cancelada Remision #') . $_GET['InvoiceNumber'];
			echo "<br> Revisar la Cancelacion <center> <strong>";
			include('includes/footer.inc');
			exit;
...
/*now populate the line items array with the stock movement records for the invoice*/
		...
		if (db_num_rows($LineItemsResult)>0) {
			...
			while ($myrow=db_fetch_array($LineItemsResult)) {
				...
				$test = $_SESSION['CreditItems']->add_to_cart($myrow['stockid'],
								$myrow['quantity'],
								$myrow['description'],
								$myrow['price'],
...
<TD class='tableheader'>" . _('Remisionado') . "</TD>
...
*/
echo "<TD class='tableheader'>" . _('Total') . "</TD></TR>";
...
/*
	...
	*/
	...
	//echo '<TD ALIGN=RIGHT>' . $DisplayTaxAmount . '</TD>
	  echo '<TD ALIGN=RIGHT>' . $DisplayGrossLineTotal . "</TD>
...
echo '<TR>
	...
	<TD ALIGN=RIGHT><HR><B>" . number_format(($_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost),2) . "</B><HR></TD>
</TR></TABLE>";

$DefaultDispatchDate = Date($_SESSION['DefaultDateFormat']);

// bowikaxu aqui inicia cuando se deshace la remision
...
/*Now need to update the invoice DebtorTrans record for the amount to be allocated and if the invoice is now settled*/
		...
		$SQL = "UPDATE debtortrans
			...
			AND type=20000";
// bowikaxu se modifico el tipo a 20000
...
/*Now insert the Credit Note into the DebtorTrans table with the allocations as calculated above*/
	...
	$SQL = "INSERT INTO debtortrans (transno,
		...
		VALUES (". $CreditNo . ",
			...
			'" . $DefaultDispatchDate . "',
			" . $PeriodNo . ", 'Rms-" . $_SESSION['ProcessingCredit'] . "',
...
// bowikaxu se quitaron los inserts a los taxes
				...
				if ($_POST['CreditType']=='WriteOff'){
					$SQL = "INSERT INTO gltrans (type,
							...
							VALUES (11,
								...
								'" . $_SESSION['CreditItems']->DebtorNo . " - " . $OrderLine->StockID . " x " . $OrderLine->QtyDispatched . " @ " . $OrderLine->StandardCost . "',
								" . round($OrderLine->StandardCost * $OrderLine->QtyDispatched,2) . ")";
					echo "agarro el sql de la cuenta<BR>";
				} else {
...
/*Post credit note transaction to GL credit debtors, debit freight re-charged and debit sales */
	...
	echo "<BR><A HREF='". $rootpath . "/rh_PrintCustTrans.php?" . SID . "&FromTransNo=" . $CreditNo . "&InvOrCredit=Credit&PrintPDF=True'>" . _('Print this credit note') . '</A>';
