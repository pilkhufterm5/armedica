...
if (DB_num_rows($result)==1){
	...
	if (DB_num_rows($result)>0){
		...
		include('rh_templateheaderSaltillo.inc.php');

		//no factura
		if ($rh_tipo==10){		
			$pdf->addTextWrap(481.5-25,530+9,100,$rh_fontsize+1,$extInvoice.$ExtRes['extinvoice'], 'right');
			$YPos = 390 - $line_height + 9 - 12;
		}else{
...
$rh_fontsize=$rh_fontsize+2;
		while ($myrow2=DB_fetch_array($result)){
			...
			{	
				//cantidad
				$pdf->addTextWrap(362-25+10,$YPos,54,$rh_fontsize-1,$DisplayQty, 'right');
				
				//precio
				$pdf->addTextWrap(423-25+10,$YPos,72,$rh_fontsize-1,$rh_DisplayPrice, 'right');

				//total
				$pdf->addTextWrap(504-25+10,$YPos,81,$rh_fontsize-1,$DisplayNet, 'right');

				//Codigo
				$pdf->addTextWrap(30-25+10,$YPos,81,$rh_fontsize-2.5,$myrow2['stockid'], 'left');
			}
			else
			{
				//total
		...
		if ($rh_tipo==10)
		{
                        ...
                        $pdf->addTextWrap(36-25+10,110+9+46,280,$rh_fontsize-2,"Rem: ".$listRem, 'left');


			//total letra
			//$pdf->addTextWrap(126,153,288,8,"(".$ConLetra.")", 'left');
			$pdf->y = $Page_Height - 129 + 51 - 9 - 46;
			$pdf->x = 36-25+10;
			$pdf->SetFontSize($rh_fontsize-2);
			$pdf->MultiCell(432,$line_height,strtoupper($ConLetra),0,'J',0,15);

			//subtotal
			$pdf->addTextWrap(504-25,145-51+9,81,$rh_fontsize+1,$DisplaySubTot, 'right');
	/*
			//iva %
	...
	*/
			//iva
			$pdf->addTextWrap(504-25,121-51+9,81,$rh_fontsize+1,$DisplayTax, 'right');

			//total
			$pdf->addTextWrap(504-25,97-51+9,81,$rh_fontsize+1,$DisplayTotal, 'right');
		}
		else
		{
			//total letra
