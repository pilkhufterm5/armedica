...
if (isset($_POST['submit'])) {
	...
	if ( trim( $_POST['Description'] ) == '' ) {
        ...
        $tipoDeImpuesto = $_POST['tipoDeImpuesto'];
        if(!$tipoDeImpuesto){
            $InputError = 1;
            prnMsg(_('You must select a tax type'), 'error');
        }
	...
	if (isset($SelectedTaxAuthID)) {
		...
		$sql = 'UPDATE taxauthorities
				...
				rh_purchtaxglcoderec = ".$_POST['PurchTaxGLCodeRec'].",
				rh_taxglcodepaid = ".$_POST['TaxGLCodePaid'].",
				...
				bankswift = '".DB_escape_string($_POST['BankSwift'])."',
                                rh_tipo_de_impuesto = '".DB_escape_string($tipoDeImpuesto)."'
	...
	/*Selected tax authority is null cos no item selected on first time round so must be adding a	record must be submitting new entries in the new tax authority form */
		// bowikaxu realhost - insert de nuevas cuentas para cash base
		$sql = "INSERT INTO taxauthorities (
						taxglcode,
						purchtaxglaccount,
						
						rh_purchtaxglcoderec,
						rh_taxglcodepaid,
			...
			VALUES (
				...
				'" .DB_escape_string($_POST['BankSwift']) . "',
                                '" .DB_escape_string($tipoDeImpuesto) . "'
				)";
...
/* It could still be the second time the page has been run and a record has been selected for modification - SelectedTaxAuthID will exist because it was sent with the new call. If its the first time the page has been displayed with no parameters then none of the above are true and the list of tax authorities will be displayed with links to delete or edit each. These will call the same page again and allow update/input or deletion of the records*/
	...
	echo "<tr>
		...
		<td class='tableheader'>" . _('Bank Swift') . "</td>
                <td class='tableheader'>" . _('Tax Type') . "</td>
		</tr></FONT>";
...
if (isset($SelectedTaxAuthID)) {
	...
	$_POST['TaxGLCodePaid']	= $myrow['rh_taxglcodepaid'];
	$_POST['PurchTaxGLCodeRec']= $myrow['rh_purchtaxglcoderec'];
	...
	$_POST['BankSwift']	= $myrow['bankswift'];
        $_POST['rh_tipo_de_impuesto']	= $myrow['rh_tipo_de_impuesto'];
...
// bowikaxu realhost - may 2007 - Cash Base Accounts
DB_data_seek($result,0);

echo '<TR><TD>' . _('Input tax GL Account Received') . ':</TD>
	<TD><SELECT name=PurchTaxGLCodeRec>';


while ($myrow = DB_fetch_array($result)) {
	if ($myrow['accountcode']==$_POST['PurchTaxGLCodeRec']) {
		echo "<OPTION SELECTED VALUE='";
	} else {
		echo "<OPTION VALUE='";
	}
...
echo '<TR><TD>' . _('Output tax GL Account Paid') . ':</TD>
	<TD><SELECT name=TaxGLCodePaid>';


while ($myrow = DB_fetch_array($result)) {
	if ($myrow['accountcode']==$_POST['TaxGLCodePaid']) {
		echo "<OPTION SELECTED VALUE='";
	} else {
		echo "<OPTION VALUE='";
	}
...
<tr>
    ...
    <td>
        Traslado:<input type="radio" name="tipoDeImpuesto" value="traslado" <?php echo $_POST['rh_tipo_de_impuesto']=='traslado'?'checked':'' ?> />
        Retenci&oacute;n:<input type="radio" name="tipoDeImpuesto" value="retencion" <?php echo $_POST['rh_tipo_de_impuesto']=='retencion'?'checked':'' ?> />
    </td>
</tr>
<?php
echo '</TABLE>';
