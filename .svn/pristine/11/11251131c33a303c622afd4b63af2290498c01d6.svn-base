...
/*First off do the stock check file stuff */
	if ($_POST['MakeStkChkData']=='New'){
		...
		$sql = "INSERT INTO stockcheckfreeze (stockid,
                   ...
                   stockmaster.mbflag!='E' AND
                   stockmaster.mbflag!='K' AND
                   stockmaster.mbflag!='D'"; // bowikaxu realhost - 15 july 2008 - ensamblado costo por componente
...
/*Libreria para exportar a excel */
	...
	$objPHPExcel->getActiveSheet()->    setCellValue('A1', "Stock Check");
	...
	$objPHPExcel->getActiveSheet()->    setCellValue('A2', $_SESSION['CompanyRecord']['coyname'])
									  ...
									  ->setCellValue('C2', _('Check Sheets for Categories between') . ' ' . $_POST['FromCriteria'] . ' ' . _('and') . ' ' . $_POST['ToCriteria'] . ' ' . _('for stock at') . ' ' . $_POST['Location']);
									  
	$objPHPExcel->getActiveSheet()->setCellValue('A3', _('Item'));

if (isset($_POST['ShowInfo']) and $_POST['ShowInfo']==true){
	
	$objPHPExcel->getActiveSheet()->setCellValue('B3', _('QOH'));
	$objPHPExcel->getActiveSheet()->setCellValue('C3', _('Cust Ords'));
	$objPHPExcel->getActiveSheet()->setCellValue('D3', _('Available'));
} else {
	$objPHPExcel->getActiveSheet()->setCellValue('B3', _('Quantity'));
	$objPHPExcel->getActiveSheet()->setCellValue('C3', _('Remarks'));
	...
	While ($InventoryPlan = DB_fetch_array($InventoryResult,$db)){
		...
		if ($Category!=$InventoryPlan['categoryid']){
			...
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, $InventoryPlan['qoh']);
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, number_format($DemandQty,0));
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, number_format($InventoryPlan['qoh']-$DemandQty,0));
			
	
		}
		$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, $InventoryPlan['stockid']);
		$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, $InventoryPlan['description']);
...
if (DB_error_no($db) !=0) {
	...
	/*if $FromCriteria is not set then show a form to allow input	*/
		...
		if ($_POST['NonZerosOnly'] == false){
	       	...
	       	echo '</TD></TR></TABLE><INPUT TYPE=Submit Name="PrintExcel" Value="' . _('Print EXCEL and Process') . '"> <INPUT TYPE=Submit Name="PrintPDF" Value="' . _('Print PDF and Process') . '"></CENTER></FORM>';
	}
