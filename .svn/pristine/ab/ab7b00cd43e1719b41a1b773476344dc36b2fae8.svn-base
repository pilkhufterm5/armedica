...
/*
	...
	$Id: inc_calendar.php,v 1.23.2.5 2005/12/26 13:54:13 mlutfy Exp $
*/
...
function http_calendrier_journee($jour_today,$mois_today,$annee_today, $date){
	...
	$retour .= "\n<td width='$largeur_centre' valign='top'>"  .
		"<div>" .
		http_calendrier_navigation_jour($jour,$mois,$annee, $GLOBALS['echelle'], 'rh_calendar.php', '') .
		"</div>".
...
function http_calendrier_jour($jour,$mois,$annee,$large = "wide", $partie_cal, $echelle, $le_message = 0, $script =  'rh_calendar.php')
{
	global $lcm_lang_rtl, $bleu, $vert,$jaune;
	global $calendrier_message_fermeture;
	
//las horas que abarcara el dia, dependiendo de la configuracion
	if ($partie_cal == "soir") {
		$debut_cal = 12;
		$fin_cal = 23;
	} else if ($partie_cal == "matin") {
...
*/
	...
	//Realhost Alberto 27/Agosto/07 comenté esta linea aparentemente de autenticacion porque causaba que no se mostraran las citas en el calendario.
	//if (!$connect_id_auteur) return $evenements;

	$q = "SELECT rh_app.*
				FROM rh_app, rh_author_app
				WHERE (rh_author_app.id_author='" . $_SESSION['UserID'] . "'
				AND rh_app.id_app=rh_author_app.id_app
				AND ((end_time >= $avant OR start_time >= $avant) AND start_time <= $apres))
				GROUP BY rh_app.id_app
				ORDER BY start_time";
	$result = lcm_query($q);
...
//adapte el query a las tablas que se usan ahora
		$result_aut=lcm_query("SELECT	realname
					FROM	www_users, rh_author_app
					WHERE	(rh_author_app.id_app=$id_message
					AND	www_users.userid!='" . $_SESSION['UserID'] . "'
					AND	rh_author_app.id_author=www_users.userid)");
//end Alberto
...
function sql_calendrier_agenda ($mois, $annee) {
	...
	$result_messages=lcm_query("SELECT 
rh_app.start_time
					FROM rh_app, rh_author_app
					WHERE rh_author_app.id_author='" . $_SESSION['UserID'] . "'
					AND rh_app.id_app=rh_author_app.id_app
