...
/* An account section could be posted when one has been edited and is being updated 
  ...
  or deletion of the records*/
	...
	$sql = "SELECT accountsection.sectionid,
			accountsection.sectionname,
			accountbase.baseid,
			accountbase.basename
		FROM accountsection INNER JOIN accountbase 
		ON accountsection.baseid = accountbase.baseid
		ORDER BY sectionid";
	...
	while ($myrow = DB_fetch_row($result)) {
		...
		echo '<TD>' . $myrow[0] . '</TD><TD>' . $myrow[1] . '</TD><TD>' . $myrow[3] . '</TD>';		
...
/*
rleal Jul 05 2006 modif 1.1
Se agregó el campo de AccountBase
*/
	echo '<TR><TD>' . _('Section In Accounts') . ':' . '</TD>
	<TD><SELECT name=BaseInAccounts>';

	$sql = 'SELECT baseid, basename FROM accountbase ORDER BY baseid';
	$baseresult = DB_query($sql, $db);
	while( $baserow = DB_fetch_array($baseresult) ) {
		if ($_POST['baseid']==$baserow['baseid']) {
			echo "<OPTION SELECTED VALUE=".$baserow['baseid'].">".$baserow['basename'].' ('.$baserow['baseid'].')';
		} else {
			echo "<OPTION VALUE=".$baserow['baseid'].">".$baserow['basename'].' ('.$baserow['baseid'].')';
		}
	}
	echo '</SELECT>';
	echo '</TD></TR>';
