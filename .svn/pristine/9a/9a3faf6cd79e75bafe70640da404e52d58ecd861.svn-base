...
$Documento=0;
$grupo=' order by purchorderdetails.orderno , rh_etiquetas.stockid';
...
if((isset($_REQUEST['completed'])||isset($_REQUEST['print']))&&count($_REQUEST['porimprimir'])>0){
	...
	foreach($_REQUEST['porimprimir'] as $id=>$barras){
		...
		$res=DB_query(str_replace("'#'",'rh_etiquetas_impresion.id',$sql).$where.$grupo." limit 1 ",$db);
	...
	foreach($_REQUEST['porimprimir'] as $id=>$barras)
		if(isset($_REQUEST['impresion'][$id])){
			$cantidad=(int)$_REQUEST['impresion'][$id];
			$cantidadImpresion='impresiones-'.((int)$cantidad).'';
			...
			DB_query(
			'update rh_etiquetas_impresion set impresiones='.$cantidadImpresion.' where id="'.DB_escape_string($id).'"'
			,$db);
			DB_query(
...
if(isset($_REQUEST['print'])&&isset($_REQUEST['porimprimir'])&&count($_REQUEST['porimprimir'])>0){
	foreach($_REQUEST['porimprimir'] as $id=>$barras)
		if($_REQUEST['impresion'][$id]>0)
			$Mensaje[]=array('codigo'=>$barras,'cantidad'=>$_REQUEST['impresion'][$id],'descripcion'=>$_REQUEST['descripcion'][$id]);
...
document.getElementsByClassName('porimprimirParent')[0].onclick=function(){
	...
	for(i in checkboxes){
		...
		foreach($tabla as $imprimir){
			...
			$selector='';
			for($i=1//$imprimir['diferencia']
				;$i>=0;$i--){
				$selector.='<option '.(
				$i==1/* $i==$imprimir['diferencia']*/?' selected=selected ':''
