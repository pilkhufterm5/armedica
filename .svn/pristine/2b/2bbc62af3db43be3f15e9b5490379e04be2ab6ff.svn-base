...
If (!isset($_GET['TransNo']) OR $_GET['TransNo']==""){
        ...
        echo '<BR><BR><BR><table class="table_index"><tr><td class="menu_group_item">
                <li><a href="'. $rootpath . '/rh_ViewRemisiones.php?'. SID .'">' . _('Sales Orders Shipment') . '</a></li>
...
if (DB_num_rows($result)==0){
        ...
        echo '<BR><BR><BR><table class="table_index"><tr><td class="menu_group_item">
                <li><a href="'. $rootpath . '/rh_ViewRemisiones.php?'. SID .'">' . _('Sales Orders Remisiones') . '</a></li>
        ...
        if ($myrow['printedpackingslip']==1 AND ($_GET['Reprint']!='OK' OR !isset($_GET['Reprint']))){
              ...
              echo '<P><A HREF="' . $rootpath . '/rh_PrintCustOrder_generic.php?' . SID . '&TransNo=' . $_GET['TransNo'] . '&Reprint=OK">'
                . _('Do a Re-Print') . ' (' . _('On Pre-Printed Stationery') . ') ' . _('Even Though Previously Printed') . '</A><P>' .
                '<A HREF="' . $rootpath. '/rh_PrintCustOrder_generic.php?' . SID . '&TransNo=' . $_GET['TransNo'] . '&Reprint=OK">'. _('Do a Re-Print') . ' (' . _('Plain paper') . ' - ' . _('A4') . ' ' . _('landscape') . ') ' . _('Even Though Previously Printed'). '</A>';
...
for ($i=1;$i<=2;$i++){  /*Print it out twice one copy for customer and one for office */
	...
	$sql = "SELECT salesorderdetails.stkcode,
			...
			(stockmoves.qty*-1) as qty,
			stockmoves.reference,
			stockmoves.transno
		FROM stockmoves, debtortrans, salesorderdetails INNER JOIN stockmaster
			ON salesorderdetails.stkcode = stockmaster.stockid
		WHERE debtortrans.transno =" . $_GET['TransNo']."
		AND salesorderdetails.orderno = debtortrans.order_
		AND stockmoves.transno =".$_GET['TransNo']."
		AND stockmoves.reference = debtortrans.order_";
	$result=DB_query($sql,$db, $ErrMsg);

	if (DB_num_rows($result)>0){
		/*Yes there are line items to start the ball rolling with a page header */
		include('includes/rh_PDFOrderPageHeader_generic.inc');
		...
		while ($myrow2=DB_fetch_array($result)){
			...
			$DisplayQtySupplied = number_format($myrow2['qty'],2);
			$total = number_format($myrow2['unitprice']*$myrow2['qty'],2);
			$totalfinal = $totalfinal + ($myrow2['unitprice']*$myrow2['qty']);
			...
			$LeftOvers = $pdf->addTextWrap(703,$YPos,85,$FontSize,$total,'right');
			...
			$pdf->addText(708, $YPos,$FontSize,  "Total: ".$totalfinal,'right');
...
if ($len<=20){
        ...
        echo '<p>'. _('There were no oustanding items on the order to deliver') . '. ' . _('A dispatch note cannot be printed').
                '<BR><A HREF="' . $rootpath . '/rh_SelectSalesOrder_Shipment.php?' . SID . '">'. _('Print Another Packing Slip/Order').
