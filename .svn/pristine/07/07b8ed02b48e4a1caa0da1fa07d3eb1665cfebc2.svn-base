                ...
                <?php
                        if(isset($_POST['series'])&&$_POST['series']!='-10'){
                                      ...
                                      if($row[0] > 0){
                                        ...
                                        $SERIEss=explode('_',$serie);
                                        $sqlInsert = "insert into rh_cfd__locations__systypes__ws_csd(id_locations, id_systypes, serie,folio,fsiguiente) values('$idLocation', $idSystype,'".$SERIEss['0']."',".$SERIEss['1'].",".$SERIEss['1'].")";
                       ...
                       $sql= "select noserie from rh_csd";
                       $result=DB_query($sql,$db);

                       while($rs=DB_fetch_array($result)){
                                $CSD[$rs['noserie']]=$rs['noserie'];

                       }

                       $sql= "select serie,folioInicial from rh_cfdi_folio";
                       $result=DB_query($sql,$db);

                       while($rs=DB_fetch_array($result)){
                                $SERIE[$rs['serie']]=$rs['folioInicial'];
                  ...
                  while($objeto = mysql_fetch_array($resultado)){
                       ...
                       foreach($SERIE as $key=>$value){
                            if($key==$objeto2['serie']){
                                $selectSerie.='<option value="'.$key.'_'.$value.'" selected="selected" >'.$key.'</option>';
                            }else{
                               $selectSerie.='<option value="'.$key.'_'.$value.'" >'.$key.'</option>';
                            }
                       }
                       }
