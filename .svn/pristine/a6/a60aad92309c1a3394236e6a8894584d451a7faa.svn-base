...
if (isset($_POST['submit'])) {
	...
	// bowikaxu realhost june 30 2008 - change supplier name field size
	if (strlen($_POST['SuppName']) > 100 or strlen($_POST['SuppName']) == 0 or $_POST['SuppName'] == '') {
		$InputError = 1;
		...
		if (!isset($_POST['New'])) {
			...
			if ($supptrans[0] == 0) {
				$sql = "UPDATE suppliers SET suppname='" . $_POST['SuppName'] . "', 
							...
							rh_taxref = '".$_POST['rh_taxref']."',
							rh_comments = '".$_POST['rh_comments']."',
							rh_exchangeid = '".$_POST['rh_exchangeid']."'
						WHERE supplierid = '$SupplierID'";
			} else {
            ...
            if($_SESSION['SuppAuto']==1){
			    $SupplierID =  GetNextTransNo(20003, $db);

			    //$SupplierID = zerofill($SupplierID,5);
			    // rlea Ene 04, 2014 Se agrega la P de prefijo
			    $SupplierID ="P".str_pad($SupplierID, 4, "0", STR_PAD_LEFT);
            }
			...
			// bowikaxu - march 2007 - unset la variable rh_taxref
			unset($_POST['rh_taxref']);
			unset($_POST['rh_comments']);
			unset($_POST['exchangeid']);
...
/*If the page was called without $SupplierID passed to page then assume a new supplier is to be entered show a form with a Supplier Code field other wise the form showing the fields with the existing entries against the supplier will show for editing with only a hidden SupplierID field*/
    ...
    if($_SESSION['SuppAuto']==0){
    echo '<TR><TD>' . _('Codigo') . ":</TD><TD><INPUT TYPE='text' NAME='SupplierID' SIZE=42 MAXLENGTH='250'></TD></TR>";
    }
	echo '<TR><TD>' . _('Supplier Name') . ":</TD><TD><INPUT TYPE='text' NAME='SuppName' SIZE=42 MAXLENGTH='250'></TD></TR>";
	echo '<TR><TD>' . _('Address Line 1') . ":</TD><TD><INPUT TYPE='text' NAME='Address1' SIZE=42 MAXLENGTH='200'></TD></TR>";
	...
	echo '<TR><TD>' . _('Address Line 4') . ":</TD><TD><INPUT TYPE='text' name='Address4' SIZE=42 MAXLENGTH='100'></TD></TR>";
	
	//Saints>
	echo '<tr><td>' . _('Localidad') . ":</td><td><input type='text' name='Address5' size=42 maxlength='200'></td></tr>";
	echo '<tr><td>' . _('Referencia') . ":</td><td><input type='text' name='Address6' size=42 maxlength='250'></td></tr>";
	echo '<tr><td>' . _('Municipio') . ":</td><td><input type='text' name='Address7' size=42 maxlength='200'></td></tr>";
	//echo '<tr><td>' . _('Estado') . ":</td><td><input type='text' name='Address8' size=42 maxlength='100'></td></tr>";
	echo '<tr><TD>' . _('Estado') . ':</td><td><SELECT tabindex=9 name="Address8">';
	echo '<option selected="selected">OTRO FUERA DE MEXICO</option>';
	...
	echo '<tr><td>' . _('Pa&iacute;s') . ":</td><td><input type='text' name='Address9' size=42 maxlength='100'></td></tr>";
	echo '<tr><td>' . _('C&oacute;digo postal') . ":</td><td><input type='text' name='Address10' size=42 maxlength='50'></td></tr>";
	//Saints<
    echo "</TABLE></TD><TD>";
    echo "<TABLE>"; 
	...
	<TD><INPUT TYPE='TEXT' NAME='rh_taxref' VALUE='".$_POST['rh_taxref']."'></TD>
	</TR>";
	
	echo "<TR>
	<TD>"._('Comments').":</TD>
	<TD><INPUT TYPE='TEXT' NAME='rh_comments' VALUE='".$_POST['rh_comments']."' SIZE=30 MAXSIZE=200></TD>
	</TR>";
	
	// bowikaxu realhost - supplier exchange id
	echo "<TR>
	<TD>"._('Exchange ID').":</TD>
	<TD><INPUT TYPE='TEXT' NAME='rh_exchangeid' VALUE='".$_POST['rh_exchangeid']."' SIZE=13 MAXSIZE=10></TD>
	</TR>";
	echo "</TD></TR></TABLE>";
	echo "</TABLE><p><CENTER><INPUT TYPE='Submit' NAME='submit' VALUE='" . _('Insert New Supplier') . "'>";
	echo '</FORM>';
...
//SupplierID exists - either passed when calling the form or from the form itself
	...
	if (!isset($_POST['New'])) {
		...
		$_POST['Address7']  = stripcslashes($myrow['address7']);
		$_POST['Address8']  = stripcslashes($myrow['address8']);
		$_POST['Address9']  = stripcslashes($myrow['address9']);
		$_POST['Address10']  = stripcslashes($myrow['address10']);
		...
		// bowikaxu - obtener la variable del RFC
		$_POST['rh_taxref'] = $myrow['rh_taxref'];
		$_POST['rh_comments'] = $myrow['rh_comments'];
		// bowikaxu realhost nov 2007
		$_POST['rh_exchangeid']=$myrow['rh_exchangeid'];
	...
	// bowikaxu realhost june 30 2008 - change supplier name field size
	echo '<TR><TD>' . _('Supplier Name') . ':</TD><TD><INPUT '.(in_array('Name',$Errors) ? 'class="inputerror"' : '').' TYPE="text" NAME="SuppName" VALUE="' . $_POST['SuppName'] . '" SIZE=42 MAXLENGTH=99></TD></TR>';
	echo '<TR><TD>' . _('Address Line 1') . ':</TD><TD><INPUT TYPE="text" NAME="Address1" VALUE="' . $_POST['Address1'] . '" SIZE=42 MAXLENGTH="200"></TD></TR>';
	...
	echo '<TR><TD>' . _('Address Line 4') . ':</TD><TD><INPUT TYPE="text" NAME="Address4" VALUE="' . $_POST['Address4'] . '" SIZE=42 MAXLENGTH="100"></TD></TR>';
	echo '<TR><TD>' . _('Address Line 5') . ':</TD><TD><INPUT TYPE="text" NAME="Address5" VALUE="' . $_POST['Address5'] . '" SIZE=42 MAXLENGTH="200"></TD></TR>';
	echo '<TR><TD>' . _('Address Line 6') . ':</TD><TD><INPUT TYPE="text" NAME="Address6" VALUE="' . $_POST['Address6'] . '" SIZE=42 MAXLENGTH="250"></TD></TR>';
	echo '<TR><TD>' . _('Address Line 7') . ':</TD><TD><INPUT TYPE="text" NAME="Address7" VALUE="' . $_POST['Address7'] . '" SIZE=42 MAXLENGTH="200"></TD></TR>';
	//echo '<TR><TD>' . _('Address Line 8') . ':</TD><TD><INPUT TYPE="text" NAME="Address8" VALUE="' . $_POST['Address8'] . '" SIZE=42 MAXLENGTH="100"></TD></TR>';
	echo '<tr><TD>' . _('Address Line 8') . ':</td><td><SELECT tabindex=9 name="Address8">';
	echo '<option selected="selected">OTRO FUERA DE MEXICO</option>';
	$sql="select * from rh_estados order by estado;";
	$rs = DB_query($sql,$db);
	while($Rw= DB_fetch_array($rs)){
	...
	echo '<TR><TD>' . _('Address Line 9') . ':</TD><TD><INPUT TYPE="text" NAME="Address9" VALUE="' . $_POST['Address9'] . '" SIZE=42 MAXLENGTH="100"></TD></TR>';
	echo '<TR><TD>' . _('Address Line 10') . ':</TD><TD><INPUT TYPE="text" NAME="Address10" VALUE="' . $_POST['Address10'] . '" SIZE=42 MAXLENGTH="50"></TD></TR>';
