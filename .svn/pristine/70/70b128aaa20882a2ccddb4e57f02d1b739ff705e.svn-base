...
/*now populate the line items array with the stock movement records for the invoice*/
		...
		$LineItemsSQL = "SELECT stockmoves.stkmoveno,
				...
				AND stockmoves.type=20000
				AND stockmoves.rh_orderline = rh_remdetails.line
...
echo '<FONT COLOR=BLUE><B>' . _('Numero de Cancelacion') . ' ' . $_SESSION['ProcessingCredit'] . '</B></FONT>
	...
	<FONT SIZE=3> - ' . _('Canecalcion en Moneda') . ' ' . $_SESSION['CreditItems']->DefaultCurrency . "</CENTER><BR>";
...
<TD class='tableheader'>" . _('Total') . "<BR></TD>";
...
//echo '
//	<TD ALIGN=RIGHT>' . number_format($_SESSION['CreditItems']->FreightCost,2) . '</TD>
//	</TR>';
...
/*Start an SQL transaction */
	...
	if ($myrow[0]>0){ /*the invoice is not already fully allocated */
		...
		if ($myrow[0] > ($_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost)){

			$Allocate_amount = $_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost;
			$Settled = 1;
		...
		if (($_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost) !=0) {
			$SQL = "INSERT INTO gltrans (type,
					...
					VALUES (11,
						...
						'" . $_SESSION['CreditItems']->DebtorNo . "',
						" . -round(($_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost)/$_SESSION['CurrencyRate'],2) . "
					)";
...
/*Could do with setting up a more flexible freight posting schema that looks at the sales type and area of the customer branch to determine where to post the freight recovery */
	...
	if ($_SESSION['InvoicePortraitFormat']==0){
		echo "<BR><A HREF='". $rootpath . "/rh_PrintCustTrans.php?" . SID . "&FromTransNo=" . $CreditNo . "&InvOrCredit=Credit&PrintPDF=True'>" . _('Imprimir Cancelacion') . '</A>';
	} else {
...
/*if the credit note is a return of goods then need to know which location to receive them into */
	...
	echo "<INPUT TYPE=SUBMIT NAME='ProcessCredit' Value='" . _('Procesar Cancelacion') ."'></CENTER>";
