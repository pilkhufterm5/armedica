...
//Realhost Alberto 30/08/07
//codigo para hacer que se cierre la ventana y se guarde el id del proveedor que se va a agregar
if(isset($_POST['supplier'])){
	foreach ($_POST['supplier'] as $llave => $valor){
		$_SESSION['app_data']['supplier'][$llave] = $valor;
	}
	?>
	<script type='text/javascript' language='javascript'>
	window.opener.document.getElementById("rhbutton2").click();
	window.close();
	</script>
	<?php
}
...
If (isset($result) AND !isset($SingleSupplierReturned)) {
	...
	echo '<br><br>';
	echo "<form action='".$_SERVER['PHP_SELF']."' method='post' id='forma2'>";
	...
	while (($myrow=DB_fetch_array($result)) AND ($RowIndex <> $_SESSION['DisplayRecordsMax'])) {
		...
		printf("<tr>
			<td><input type='checkbox' name='supplier[]' value='%s'</td>
			<td>%s</td>
			<td>%s</td>
			<td>%s</td>
			<td>%s</td>
