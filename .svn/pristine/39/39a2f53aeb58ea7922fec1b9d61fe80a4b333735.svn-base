...
class CFDViewer extends FPDF{
	...
	private function TopPrint(){
		...
		$this->SetXY(32,31 + $x);
		$this->Cell(20,10,str_replace("\\'","'",utf8_decode($this->CFD['receptor']['nombre'])),0,0,'L');
		$this->SetXY(32,34 + $x);
		...
		*/
		...
		if(trim($this->CFD['general']['com_general']) != ''){
			...
			$this->multicell(300,3.5,utf8_decode(html_entity_decode($this->CFD['general']['com_general'])),0,'L');
	...
	public function PrintCFD(){
		...
		return $this->Output("CFD_".$this->CFD["comprobante"]["serie"].'-'.$this->CFD["comprobante"]["folio"].".pdf",'I');
	}
	public function PrintCFD2($name=null,$opc){
		...
		if(!(($this->CFD["conceptos"]["cantidad"]==1)&&($this->CFD["impuestos"]["retenido"]["cantidad"]==2))){
			...
			//----Logo
				if (file_exists($this->CFD['general']['logo'])&&($this->mailing)){
					$this->Image($this->CFD['general']['logo'],157,6,50);
				}
			$this->TopPrint();
			$this->printFactura();
        ...
        if(!is_null($name)){
            ...
            $this->Output("CFD_".$this->CFD["comprobante"]["serie"].'-'.$this->CFD["comprobante"]["folio"].".pdf",$opc);
        }
	}
	...
	private function printFactura(){
		...
		while(($row<$this->CFD["conceptos"]["cantidad"])and($totalrow<=30)){
            ...
            if(strlen($this->CFD["concepto"]["pedimento"][$row])>0){
			      $txt=$this->CFD["concepto"]["descripcion"][$row].' Pedimento: '.$this->CFD["concepto"]["pedimento"][$row].'|'.$this->CFD["concepto"]["fecha"][$row].'|'.$this->CFD["concepto"]["aduana"][$row];
