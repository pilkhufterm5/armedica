...
If (isset($PrintPDF) 
	...
	*/
	...
	if ($InvOrCredit=='Invoice'){
		$pdf->addinfo('Title',_('Sales Shipment') . ' ' . $FromTransNo . ' to ' . $_POST['ToTransNo']);
		$pdf->addinfo('Subject',_('Shipments from') . ' ' . $FromTransNo . ' ' . _('to') . ' ' . $_POST['ToTransNo']);
	} else {
	...
	nobble the invoice reprints */
	   ...
	   if (DB_error_no($db)!=0) {
		...
		prnMsg( _('There was a problem retrieving the shipment or credit note details for note number') . ' ' . $InvoiceToPrint . ' ' . _('from the database') . '. ' . _('To print an shipment, the sales order record, the customer transaction record and the branch record for the customer must not have been purged') . '. ' . _('To print a credit note only requires the customer, transaction, salesman and branch records be available'),'error');
		if ($debug==1){
	   ...
	   if (DB_num_rows($result)==1){
		...
		if (DB_error_no($db)!=0) {
			$title = _('Transaction Print Error Report');
			include ('includes/header.inc');
			echo '<BR>' . _('There was a problem retrieving the shipment or credit note stock movement details for shipment number') . ' ' . $FromTransNo . ' ' . _('from the database');
			if ($debug==1){
	...
	/* Print out the payment terms */
		...
		$YPos+=10;
		if ($InvOrCredit=='Invoice'){
			$pdf->addText($Page_Width-$Right_Margin-220, $YPos - ($line_height*3)-6,$FontSize, _('TOTAL SHIPMENT'));
			$FontSize=6;
	...
	$title=_('Select Shipments/Credit Notes To Print');
	include('includes/header.inc');
