...
$SQL = "SELECT  purchorders.supplierno, suppliers.suppname,
				stockcategory.categoryid,
				stockcategory.categorydescription,
				stockmaster.stockid,
				stockmaster.description,
				SUM(purchorderdetails.qtyinvoiced) AS qtyinvoiced,
				SUM(purchorderdetails.qtyinvoiced*purchorderdetails.unitprice/currencies.rate) AS amtinvoiced,
				
				SUM(purchorderdetails.quantityord) AS quantityord,
				SUM(purchorderdetails.quantityord*purchorderdetails.unitprice/currencies.rate) AS amtord,
				
				SUM(purchorderdetails.quantityrecd) AS quantityrecd,
				SUM(purchorderdetails.quantityrecd*purchorderdetails.unitprice/currencies.rate) AS amtrecd
				FROM
					stockcategory,
					stockmaster,
					purchorders,
					purchorderdetails,
					suppliers  JOIN currencies ON (suppliers.currcode = currencies.currabrev)
				WHERE stockmaster.stockid = purchorderdetails.itemcode
					...
					ORDER BY purchorders.supplierno, stockmaster.stockid";
...
if ($Category!=$InventoryValn['supplierno']){
$FontSize=8;
...
$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,260-$Left_Margin,$FontSize,$InventoryValn['supplierno'] . ' - ' . $InventoryValn['suppname']);
$LeftOvers = $pdf->addTextWrap(200,$YPos,60,$FontSize,number_format($Catqtyord,2), 'right');
$LeftOvers = $pdf->addTextWrap(260,$YPos,60,$FontSize,number_format($Catamtord,2), 'right');
$LeftOvers = $pdf->addTextWrap(320,$YPos,60,$FontSize,number_format($Catqtyrec,2), 'right');
$LeftOvers = $pdf->addTextWrap(380,$YPos,60,$FontSize,number_format($Catamtrec,2), 'right');
...
$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,290-$Left_Margin,$FontSize,$InventoryValn['supplierno'] . ' - ' . $InventoryValn['suppname']);
$Category = $InventoryValn['supplierno'];
$CategoryName = $InventoryValn['suppname'];
}
...
$SQL = "SELECT 	purchorders.supplierno, suppliers.suppname,
				...
				WHERE stockmaster.stockid = purchorderdetails.itemcode
					...
					AND stockmaster.categoryid >= '" . $_POST['FromCriteria'] . "'
					AND stockmaster.categoryid <= '" . $_POST['ToCriteria'] . "'
					AND purchorders.supplierno >= '0' 
					AND purchorders.supplierno <= 'zzzzzzz' 
					AND purchorders.orddate >= '".ConvertSQLDate($_POST['FromDate'])."'
					AND purchorders.orddate <= '".ConvertSQLDate($_POST['ToDate'])."'
					".$Location."
					GROUP BY purchorders.supplierno
					ORDER BY suppliers.suppname, stockmaster.categoryid";
...
$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,230-$Left_Margin,$FontSize,$InventoryValn['supplierno'] . ' - ' . $InventoryValn['suppname']);
