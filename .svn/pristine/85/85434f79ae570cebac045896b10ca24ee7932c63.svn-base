 ...
 * iJPe
 * 2010-02-26
 * Modificacion de archivo rh_ExtInvoice.php, el cual muestra la relacion entre el numero
 * externo y el numero interno de los movimientos
 *
 * Archivo para cambiar numero externo de las notas de credito
 */
...
$title = _('Manejo de Notas de Credito Internas y Externas');
...
echo '<TR><TD>' ._('Nota de Cr&eacute;dito').' ' ._('Desde') . ":</TD><TD><INPUT TYPE=TEXT NAME='FromInv' MAXLENGTH=10 SIZE=11 VALUE=" . $_POST['FromInv'] . '></TD>';
echo '<TD>' ._('Nota de Cr&eacute;dito') .' '. _('Hasta') . ":</TD><TD><INPUT TYPE=TEXT NAME='ToInv' MAXLENGTH=10 SIZE=11 VALUE=" . $_POST['ToInv'] . '></TD></TR>';
...
echo "</TR></TABLE><INPUT TYPE=SUBMIT NAME='ShowResults' VALUE='" . _('Ver Notas') . "'>";
echo '<HR>';
...
if(isset($_POST['submit']) && $_POST['submit']=='Actualizar' && isset($_POST['einvs'])){
	...
	foreach($_POST['einvs'] as $ext_inv){
		...
		$sql = "SELECT id from rh_crednotesreference WHERE extcn = '".$_POST['einv_'.$ext_inv]."'
				AND loccode IN (SELECT loccode FROM rh_crednotesreference WHERE ref = '".$ext_inv."')";
		$res = DB_query($sql,$db);
		if(DB_num_rows($res)>0){
			prnMsg('El numero de nota de credito externa '.$_POST['einv_'.$ext_inv].' ya existe !!!','error');
		}else {		
			$sql = "UPDATE rh_crednotesreference SET extcn ='".$_POST['einv_'.$ext_inv]."' WHERE ref = '".$ext_inv."'";
			DB_query($sql,$db,'Imposible actualizar nota de credito con referencia '.$ext_inv,'SQL: '.$sql);
			prnMsg("Notas de Credito Actualizadas",'success');
		}
...
if (isset($_POST['ShowResults'])){
   ...
   $tableheader = "<TR><TD class='tableheader'>" . _('Interna') . "</TD>
	...
	while ($myrow=DB_fetch_array($TransResult)) {
		...
		$SQL = "SELECT ref,rh_crednotesreference.extcn,rh_crednotesreference.loccode FROM rh_crednotesreference WHERE rh_crednotesreference.ref = ".$myrow['id']." AND rh_crednotesreference.loccode = '".$_POST['StockLocation']."'";
		
		$ExtResult = DB_query($SQL, $db,$ErrMsg,$DbgMsg);
   		$ErrMsg = _('Error debido a') . ' - ' . DB_error_msg($db);
   		$DbgMsg =  _('The SQL that failed was');
		...
		$format_base = "<td>%s</td>
			...
			printf("$format_base
				<td><a target='_blank' href='%s/rh_PrintCustTrans.php?%&FromTransNo=%s&InvOrCredit=Credit'><IMG SRC='%s' TITLE='" . _('Clic para ver Nota de Credito') . "'></a></td>
				<td><a target='_blank' href='%s/rh_PrintCustTrans.php?%&FromTransNo=%s&InvOrCredit=Credit&PrintPDF=True'><IMG BORDER=0 SRC='%s' TITLE='" . _('Click para imprimir') . "'></a></td>
				</tr>",
				$myrow['transno'],
				"<input type=checkbox name='einvs[]' value='".$external['ref']."'>",
				$external['rh_serie']."<INPUT TYPE='text' name='einv_".$external['ref']."' value='".$external['extcn']."' size=9>",
