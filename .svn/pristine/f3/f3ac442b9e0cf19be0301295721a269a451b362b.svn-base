...
if (count($_SESSION['errors'])) {
	...
	if ($id_app>0) {
		...
		$q="UPDATE rh_app SET $fl,date_update=NOW() WHERE id_app = $id_app ";
		// Only admin or appointment author itself could change it
		if ( !($GLOBALS['author_session']['status'] === 'admin') )
			$q .= " AND id_author = '" . $_SESSION['UserID'] . "'";
		$result = lcm_query($q);
	} else {
		// Add the new appointment
		$q = "INSERT INTO rh_app SET id_app=0";
		// Add case ID if available
		$q .= ( ($_SESSION['app_data']['id_case']) ? ',id_case=' . $_SESSION['app_data']['id_case'] : '' );
		// Add ID of the creator
		$q .= ",id_author='" . $_SESSION['UserID']."'";//GLOBALS['author_session']['id_author'];
		...
		lcm_query("INSERT INTO rh_author_app SET id_app=$id_app,id_author='" . $_SESSION['UserID']."'"); //$GLOBALS['author_session']['id_author']);
	...
	if (!empty($_SESSION['app_data']['author'])) {
		$q = "INSERT IGNORE INTO rh_author_app SET id_app=$id_app,id_author='" . $_SESSION['app_data']['author'] . "'";
	...
	if (!empty($_SESSION['app_data']['rem_author'])) {
		$q = "DELETE FROM rh_author_app WHERE id_app=$id_app AND id_author IN ('" . join("','",$_SESSION['app_data']['rem_author']) . "')";
...
//Realhost Alberto 29/Agosto/07 se cambió el query para agregar a los clientes de diferente manera
	// Add/update appointment clients/organisations
	if (!empty($_SESSION['app_data']['client'])) {
		foreach($_SESSION['app_data']['client'] as $llave => $valor){
			$client_org = explode(':',$valor);
			$q = "INSERT IGNORE INTO rh_app_client_org SET id_app=$id_app";
			$q .= ',id_client="' . $client_org[0].'"';
			$q .= ',tipo=1';
			if ($client_org[1]) $q .= ',id_org=' . $client_org[1];
			// bowikaxu debug calendario
			echo "<HR>".$q."<HR>";
			if ($result = lcm_query($q))
				$_SESSION['errors']['client_added'] = "An client/organisation was added to the participants of this appointment.";
		}
		unset($_SESSION['app_data']['client']);
	}

/*	Esta es la manera en la que originalmente se agregaban los clientes, me la quedare por si acaso :D
	// Add/update appointment clients/organisations
	if (!empty($_SESSION['app_data']['client'])) {
...
//end Alberto
	...
	if (!empty($_SESSION['app_data']['rem_client'])) {
		$q = "DELETE FROM rh_app_client_org WHERE id_app=$id_app AND tipo=1 AND (0";
		...
		$q .= ")";
		if ( ($result = lcm_query($q)) && (mysql_affected_rows() > 0) )
			$_SESSION['errors']['client_added'] = "An client/organisation was erased from the participants of this appointment.";
...
//Realhost Alberto 28/Agosto/07
	// Add/update appointment suppliers
	if (!empty($_SESSION['app_data']['supplier'])) {
		foreach($_SESSION['app_data']['supplier'] as $llave => $valor){
			$supplier_org = explode(':',$valor);
			$q = "INSERT IGNORE INTO rh_app_client_org SET id_app=$id_app";
			$q .= ',id_client="' . $supplier_org[0].'"';
			$q .= ',tipo=2';
			if ($supplier_org[1]) $q .= ',id_org=' . $supplier_org[1];
			if ($result = lcm_query($q));
		}
		$_SESSION['errors']['client_added'] = "Un proveedor ha sido agregado a los participantes de esta cita.";
		unset($_SESSION['app_data']['supplier']);
	}

	// Remove appointment participants (suppliers)
	if (!empty($_SESSION['app_data']['rem_supplier'])) {
		$q = "DELETE FROM rh_app_client_org WHERE id_app=$id_app AND tipo=2 AND (0";
		foreach($_SESSION['app_data']['rem_supplier'] as $rem_supplier) {
			$supplier_org = explode(':',$rem_supplier);
			$co = 'id_client="' . $supplier_org[0].'"';
			/*if ($client_org[1])
				$co = "($co AND id_org=" . $client_org[1] . ')';*/
			$q .= " OR $co";
		}
		$q .= ")";
		if ( ($result = lcm_query($q)) && (mysql_affected_rows() > 0) )
			$_SESSION['errors']['client_added'] = "Un proveedor ha sido removido de los participantes de esta cita.";
			unset($_SESSION['app_data']['rem_supplier']);
	}
//end Alberto
...
//		header('Location: ' . $_SERVER['HTTP_REFERER'] );
	...
	switch($_SESSION['app_data']['submit']) {
		...
		case 'add_client':
			...
			header('Location: edit_app.php?app='.$id_app);
