...
$DInvoice = 'companies/'.$DefaultCompany.'/Inv1.jpg';
...
$pdf->addText(370,612-10-$minux,$FontSize,$myrow['locationname'], 'center' );
$pdf->addText(520,612-10-$minux,$FontSize,$myrow['salesmanname'], 'center');	
...
$pdf->addTextWrap(348,657-10-$minux,20,$FontSize,$myrow['dd'], 'center');	
$pdf->addTextWrap(378,657-10-$minux,20,$FontSize,$myrow['mm'], 'center');	
$pdf->addTextWrap(408,657-10-$minux,20,$FontSize,$myrow['yy'], 'center');	
...
if ($myrow['invaddrbranch']==0){
	...
	if(strlen($myrow['name2'])>=1){
		$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['name2']);
		$minus += 12;
	}
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['address1'].' '.$myrow['address2']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['address3'].' '.$myrow['address5']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['address4'].' '.$myrow['address6']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['taxref']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['brname'].' - '.$myrow['debtorno']);
	...
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['braddress1'].' '.$myrow['braddress2']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['braddress3'].' '.$myrow['braddress5']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['braddress4'].' '.$myrow['braddress6'].'     '.$myrow['debtorno']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['taxref']);
		$minus += 12;
	$pdf->addText($XPos, $YPos-$minus-$minux, $FontSize, $myrow['brname']);
}
...
$XPos = 100;
$YPos = 563-10;
$pdf->addText($XPos, $YPos-$minux, $FontSize, $myrow['customerref']);
$pdf->addText(240, $YPos-$minux, $FontSize, $myrow['terms']);
$pdf->addText(450, $YPos-$minux, $FontSize, $myrow['rh_Tel']);
