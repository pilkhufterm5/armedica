...
if (isset($_POST['ExRate'])){
	...
	//rleal Jun 23 2011 se agrega lo del almacén
	$_SESSION['$SuppTrans']->rh_loccode = $_POST['StockLocation'];
...
if (!isset($_POST['PostInvoice'])){
	...
	if (!isset($_SESSION['SuppTrans']->TranDate)){
		...
		$_SESSION['SuppTrans']->rh_InvDate = Date($_SESSION['DefaultDateFormat'], Mktime(0,0,0,Date('m'),Date('d'),Date('y')));
	}
	echo '<TD>' . _('Fecha de factura') . ' (' . _('en formato') . ' ' . $_SESSION['DefaultDateFormat'] . ") :</TD>
			<TD><INPUT TYPE=TEXT SIZE=11 MAXLENGTH=10 NAME='TranDate' VALUE=" . $_SESSION['SuppTrans']->TranDate . '></TD>';

	echo '<TD>' . _('Fecha de Recibida') . ' (' . _('en formato') . ' ' . $_SESSION['DefaultDateFormat'] . ") :</TD>
			<TD><INPUT TYPE=TEXT SIZE=11 MAXLENGTH=10 NAME='rh_InvDate' VALUE=" . $_SESSION['SuppTrans']->rh_InvDate . '></TD>';

	echo '</TR><TR align = "center"><TD colspan=3>' . _('Exchange Rate') . ": <INPUT TYPE=TEXT SIZE=11 MAXLENGTH=10 NAME='ExRate' VALUE=" . $_SESSION['SuppTrans']->ExRate . '></TD>';

	/*
	 * rleal
	 * Jun 23 2011
	 * Se agrega a que almacén se relaciona
	 */
		echo  '<td colspan=3>'._('From Stock Location') . ":<SELECT name='StockLocation'> ";
...
/*Need to recalc the taxtotal */
			...
			the credit is to creditors control act  done later for the total invoice value + tax*/
				...
				$SQL = 'INSERT INTO gltrans (type,
						...
						VALUES (20, ' .
							...
							" . round($EnteredGLCode->Amount/ $_SESSION['SuppTrans']->ExRate,4) . ",
				...
				$LocalTotal += round($EnteredGLCode->Amount/ $_SESSION['SuppTrans']->ExRate,4);
			...
			foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){
				...
				/*so its not a shipment item
				  enter the GL entry to reverse the GRN suspense entry created on delivery at standard cost used on delivery */
					...
					// hacer insert de cada producto
					// $EnteredGRN->QtyRecd se cambia por   $EnteredGRN->This_QuantityInv fcarrizalest  para que muestre la correcta cantida facturada
             	 	$sql = "INSERT INTO rh_suppinvdetails (
             	 				transno,
             	 				trandate,
             	 				itemcode,
             	 				...
             	 				".$EnteredGRN->OrderPrice * $EnteredGRN->This_QuantityInv.")";
             	 			$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('La transaccion del asiento no se pudo agregar en el resumen de la factura');
							$DbgMsg = _('The following SQL to insert the GL transaction was used');
							...
							// bowikaxu - fin de los inserts de goods received
								...
								if ($TotalQuantityOnHand>0) {
									...
									$CostIncrement = ($PurchPriceVar - $WriteOffToVariances) / $TotalQuantityOnHand;
										...
										//rleal Se agrega al historial de costos

									$text_cost_history= "<br>EnteredGRN->This_QuantityInv:".$EnteredGRN->This_QuantityInv;
        				            $text_cost_history .= "<br>EnteredGRN->ChgPrice:".$EnteredGRN->ChgPrice;
                				    $text_cost_history .= "<br>SESSION[SuppTrans]->ExRate:".$_SESSION['SuppTrans']->ExRate;
		                		    $text_cost_history .= "<br>EnteredGRN->StdCostUnit:".$EnteredGRN->StdCostUnit;
 			       		            $text_cost_history .= "<br>PurchPriceVar:".$PurchPriceVar;
        				            $text_cost_history .= "<br>WriteOffToVariances:".$WriteOffToVariances;
        		    		        $text_cost_history .= "<br>TotalQuantityOnHand:".$TotalQuantityOnHand;
                		    		$text_cost_history .= "<br>CostIncrement = (PurchPriceVar - WriteOffToVariances) / TotalQuantityOnHand";
       			             		$text_cost_history .= "<br>CostIncrement:".$CostIncrement;
                		    		//echo $text_cost_history;
                    				//echo "<br><br>";

		       			             if ($CostIncrement != 0) {
										$sqlcost = "INSERT INTO rh_costhistory (stockid,
											cost, trandate, texto, user_) VALUES (
											'".$EnteredGRN->ItemCode."',
											'".$EnteredGRN->StdCostUnit."',
											'".Date('Y-m-d H:m:s')."',
											'".$text_cost_history."',
											'".$_SESSION['UserID']."')";
									// rleal realhost - insert the cost history
									DB_query($sqlcost,$db,'error al insertar el historial de costos <br>'.$sqlcost.'<br>','',true);
                    			}
			...
			foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){
				...
				/*so its not a shipment item
				  enter the GL entry to reverse the GRN suspense entry created on delivery at standard cost used on delivery */
					...
					//rleal May 02 2011 Se quita esta validaciÔøΩn
					//if ($EnteredGRN->StdCostUnit * $EnteredGRN->This_QuantityInv != 0) {
					// hacer insert de cada producto
					// $EnteredGRN->QtyRecd se cambia por  $EnteredGRN->This_QuantityInv fcarrizalest  para que muestre la correcta cantida facturada
             	 	// rleal Ago 28 2011 se cambia $EnteredGRN->OrderPrice por $EnteredGRN->ChgPrice
					$sql = "INSERT INTO rh_suppinvdetails (
             	 				transno,
             	 				trandate,
             	 				itemcode,
             	 				...
             	 				'".$SQLInvoiceDate."',
             	 				'".$EnteredGRN->ItemCode."',
             	 				'".DB_escape_string($EnteredGRN->ItemDescription)."',
             	 				".$EnteredGRN->This_QuantityInv.",
             	 				".$EnteredGRN->ChgPrice.",
             	 				".$EnteredGRN->ChgPrice * $EnteredGRN->This_QuantityInv.")";
	...
	/*Now insert the invoice into the SuppTrans table*/
		...
		// rleal realhost - Jun 23 2011 - se insertan hacia que almacén
			$sql = "INSERT INTO rh_supptrans_locations (supptransid, loccode)
					VALUES (".$SuppTransID.", '".$_SESSION['$SuppTrans']->rh_loccode."')";
			DB_query($sql,$db,'Error al insertar detalles del descuento automatico','',$true);
		...
		foreach ($_SESSION['SuppTrans']->GRNs as $EnteredGRN){
			...
			//Obtenemos la orden de compra a la cual pertenece la l√≠nea que se esta facturando
			$sql_oc = 'select orderno from purchorderdetails WHERE podetailitem = ' . $EnteredGRN->PODetailItem;
			$rs_oc = DB_query($sql_oc,$db);
			$rw_oc = DB_fetch_array($rs_oc);
			$orderno = $rw_oc['orderno'];

			//Se insert la relaci√≥n orden de compra-factura
			$sql = "SELECT * FROM  grns  WHERE grnno = '{$EnteredGRN->GRNNo}' ;";
			$rs_oc2 = DB_query($sql,$db);
			$rw_oc2 = DB_fetch_array($rs_oc2);
			$grnbatch = $rw_oc2['grnbatch'];
			$sql_hist = 'insert into rh_factura_oc(type,transno,orderno,podetailitem,grnno,grnbatch)
			values(20,'.$InvoiceNo.','.$orderno.','.$EnteredGRN->PODetailItem.','.$EnteredGRN->GRNNo.',\''.$grnbatch.'\')';
			...
			$Result = DB_query($sql_hist, $db, $ErrMsg, $DbgMsg, True);
