...
if($PrintPDF=='Imprimir PDF (imagen)' && $rh_tipo!=20001){
	...
	//no factura	
	if ($rh_tipo!=20001){	
		$pdf->addTextWrap(481.5-25,507+9,72,12,$extInvoice.$extCN, 'right');
	}else{
		$pdf->addTextWrap(481.5-25,507+9,72,12,$extInvoice.$ExtRes['extinvoice'], 'right');
	}
}
	//color normal
...
if ($rh_tipo==10)
{
	...
	//fecha expedicion
	$pdf->addTextWrap(478-25+10,475.5+9-12,30,$rh_fontsize,$myrow['dd'], 'right');
	$pdf->addTextWrap(514-25+10,475.5+9-12,30,$rh_fontsize,$myrow['mm'], 'right');
	$pdf->addTextWrap(550-25+10,475.5+9-12,33,$rh_fontsize,$myrow['yy'], 'right');

	//lugar expedicion
	//$pdf->addTextWrap(468,652.5,108,7,$myrow['locationname'], 'right');

	//terminos
	$pdf->addTextWrap(481.5-25+10,412.5+9-12,108,$rh_fontsize-1,$myrow['terms'], 'center');

	//no cliente
	$pdf->addTextWrap(395-25+10,428.5+9-12,72,$rh_fontsize+1,$myrow['debtorno'], 'left');

	//cliente
	$pdf->addTextWrap(81-25+10,469+9-12,360,$rh_fontsize-1,$myrow['name'].' '.$myrow['name2'], 'left');
	...
	if (strlen($rh_unirDir) > 50)
	{
		...
		//direccion
		$pdf->addTextWrap(81-25+10,457+9-12,360,$rh_fontsize-1.5,$rh_strtmpI, 'left');//linea 1
		$pdf->addTextWrap(81-25+10,449+9-12,360,$rh_fontsize-1.5,$rh_strtmpF, 'left');//linea 2
		$pdf->addTextWrap(81-25+10,442+9-12,360,$rh_fontsize-1.5,$myrow['address4'].' '.$myrow['address5'].' C.P. '.$myrow['address6'], 'left');//linea 3
	}
	else
	{
		//direccion
		$pdf->addTextWrap(81-25+10,455+9-12,360,$rh_fontsize-1,$rh_unirDir, 'left');//linea 1
		$pdf->addTextWrap(81-25+10,444+9-12,360,$rh_fontsize-1,$myrow['address4'].' '.$myrow['address5'].' C.P. '.$myrow['address6'], 'left');//linea 2
	}
	...
	//$pdf->addTextWrap(350-25,455+9,360,$rh_fontsize-1,"Referencia del cliente: ".$myrow['customerref'], 'left');

	//rfc
	$pdf->addTextWrap(90-25+10,430.5+9-12,144,$rh_fontsize-1,$myrow['taxref'], 'left');

	//Vendedor
	$pdf->addTextWrap(409.5-25+10,418+9-12,54,$rh_fontsize-1,$myrow['salesmanname'], 'left');
