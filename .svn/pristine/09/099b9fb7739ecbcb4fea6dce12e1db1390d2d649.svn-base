...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedSalesPostingID)) {
		...
		// bowikaxu - Se agrego el insert con el valor de las Remisiones
		$sql = 'UPDATE salesglpostings SET
				rh_InvoiceShipmentcode = '.$_POST['RemisionesGLCode'].',
	...
	/*Selected Sales GL Posting is null cos no item selected on first time round so must be	adding a record must be submitting new entries in the new SalesGLPosting form */
		...
		if ($myrow[0] == 0) {
			// bowikaxu - Se agrego el insert a rh_InvoiceShipmentcode
			$sql = 'INSERT INTO salesglpostings (
						rh_InvoiceShipmentcode,
						salesglcode,
						discountglcode,
						area,
						stkcat,
	...
	// bowikaxu - borrar de memoria la variable RemisionesGLCode
	unset($_POST['RemisionesGLCode']);
...
if (!isset($_GET['delete'])) {
	...
	if (isset($SelectedSalesPostingID)) {
		...
		// bowikaxu - Se agrego obtener campo de numero para Remisiones de la BD
		$sql = "SELECT salesglpostings.stkcat,
				salesglpostings.rh_InvoiceShipmentcode,
				salesglpostings.salesglcode,
				salesglpostings.discountglcode,
				salesglpostings.area,
				salesglpostings.salestype
		...
		// bowikaxu - Obtener el numero de las Remisiones
		$_POST['RemisionesGLCode'] = $myrow['rh_InvoiceShipmentcode'];
...
// bowikaxu - LISTADO DE CUENTAS PARA REMISIONES

	echo '<TR><TD>' . _('Remisiones GL Account') . ":</TD><TD><SELECT name='RemisionesGLCode'>";
	...
	while ($myrow = DB_fetch_array($result)) {
// bowikaxu - Mostrar Seleccionado el Numero de Remisiones
		if ($myrow['accountcode']==$_POST['RemisionesGLCode']) {
			echo "<OPTION SELECTED VALUE='";
		} else {
			echo "<OPTION VALUE='";
		}
...
// bowikaxu - FINALIZAR LISTADO DE CUENTAS REMISIONES
