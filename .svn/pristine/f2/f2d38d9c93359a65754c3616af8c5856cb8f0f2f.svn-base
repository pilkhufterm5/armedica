...
<form name="Form" method="POST" enctype="multipart/form-data"
	style="width: 100%;" action="Reporte_VentasVendedor.php" target="_blank">
...
<table cellpadding="0" cellspacing="2" borde="0" width="35%">
	...
	<tr>
		<td>Sucursal:</td>
		<td><select name="sucursal" style="width: 100%">
<?php
...
?>
	...
	<tr>
		<td>Categoria:</td>
		<td><select name="category[]" id="category" style="width: 100%" multiple="multiple" size="6">
<?php
$sql = "select categoryid as id,categorydescription as nombre from stockcategory;";
$result = DB_query ( $sql, $db );
while ( $myrow = DB_fetch_array ( $result ) ) {

	echo "<option " . ((in_array($myrow ['id'], $_POST ['category'])  ? "selected='selected'" : " ")) . "value='" . $myrow ['id'] . "'>" . $myrow ['nombre'] . "</option>";
}
...
?>
	...
	<tr>
		<td>Gamma:</td>
		<td><select name="gamma[]" id="gamma" style="width: 100%" multiple="multiple" size="6">
<?php
$sql = "select id,descripcion as nombre from rh_gamma;";
$result = DB_query ( $sql, $db );
while ( $myrow = DB_fetch_array ( $result ) ) {

	echo "<option " . ((in_array($myrow ['id'], $_POST ['gamma'])  ? "selected='selected'" : " ")) . "value='" . $myrow ['id'] . "'>" . $myrow ['nombre'] . "</option>";
}
...
?>
	...
	<tr>
		<td>Especie:</td>
		<td><select name="especie[]" id="especie" style="width: 100%" multiple="multiple" size="6">
<?php
$sql = "select id,descripcion as nombre from rh_especie;";
$result = DB_query ( $sql, $db );
while ( $myrow = DB_fetch_array ( $result ) ) {

	echo "<option " . ((in_array($myrow ['id'], $_POST ['especie'])  ? "selected='selected'" : " ")) . "value='" . $myrow ['id'] . "'>" . $myrow ['nombre'] . "</option>";
}
...
?>
    ...
    <tr>
		<td align="center" colspan="2"><input type="submit" name="PrintPDFResume"
			value="Mostrar  Resumen" style="width: 100%;" /></td>
	</tr>
</table>
</center>
</form>
