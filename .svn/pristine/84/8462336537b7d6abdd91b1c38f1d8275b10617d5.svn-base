...
$title = _('Importar Codigos de Barras de CSV');
...
// OPCIONES PARA INVENTARIOS
    ...
    echo "Codigo | Codigo de Barras<br></center></h2>";
...
if(($tamano < 3000000)&&(($tipo=="text/csv")||($tipo=="text/text")||($tipo=="application/vnd.ms-excel"))){
    ...
    if(@$fh_in = fopen($filename,"r")){
        ...
        while(($line = fgetcsv($fh_in,0,'|'))!==false){
            if($line!= null){
                            $Query2 = "INSERT INTO stockmaster (
                            stockid,
                            barcode)
                ...
                $sqlSelect = "select barcode from stockmaster where stockid='".$line[0]."'";
                $rp = DB_query($sqlSelect,$db);
                if(DB_num_rows($rp)==0){
                 //$result = DB_query($Query2,$db);
                 echo "<B>SATISFACTORIO !!!</B>";
                }else{
                    $Query2="update stockmaster set barcode='".$line[1]."' where stockid='".$line[0]."'";
                    $result = DB_query($Query2,$db);
                    echo "<B>SATISFACTORIO !!!</B>";
                }
                echo $Query2."<BR>";
