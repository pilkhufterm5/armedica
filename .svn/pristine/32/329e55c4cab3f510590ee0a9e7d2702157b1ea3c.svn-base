...
If (isset($PrintPDF) 
	...
	*/
	...
	/* Then there's an invoice (or credit note) to print. So print out the invoice header and GST Number from the company record */
				...
				$ID = $myrow['ID'];
				$sql2 = "SELECT rh_invoicesreference.extinvoice, locations.rh_serie FROM rh_invoicesreference, locations WHERE rh_invoicesreference.ref = ".$ID." AND locations.loccode = rh_invoicesreference.loccode";
