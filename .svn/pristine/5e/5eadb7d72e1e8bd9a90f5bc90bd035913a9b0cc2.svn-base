 ...
 * bowikaxu realhost
 * january 2008
 * compare the stock adjustemnts from the actual stockmoves to the temporary locstock table
 * 
 */
...
if($_GET['table']=='temp'){
	$sql = "SELECT rh_locstocktmp.*, SUM(stockmoves.qty) AS movesqty FROM rh_locstocktmp, stockmoves
			WHERE rh_locstocktmp.stockid = stockmoves.stockid
			AND stockmoves.prd <= 5
			AND rh_locstocktmp.loccode = stockmoves.loccode
			GROUP BY stockmoves.stockid
			HAVING movesqty != rh_locstocktmp.quantity";
}
