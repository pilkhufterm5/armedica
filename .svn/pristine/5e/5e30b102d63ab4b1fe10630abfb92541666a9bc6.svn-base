...
echo '<table class="no_print"><tr><td><input type=submit name="SearchParts" value="' . _('Search Parts Now') . '">';
...
<!--
$(function(){
	$('<csv title=".Listado"><button name="xls" value="xls">Exportar a excel</button></csv><br />').insertBefore($('.Listado'));
...
$completed = " AND purchorderdetails.completed=0";
	...
	echo '<table cellpadding=2 colspan=7 width=97% class="selection Listado">';
...
//				   '</td><td class="tableheader">' . _('Receive') .
	...
	echo '<th>' . _('Status') .
			'</th><th class=no_print>' . _('Modify') .
			'</th><th class=no_print>' . _('Print') .
			'</th><th class=no_print>' . _('Receive') .
	'</th></tr><tbody>';
	$j = 1;
	$k=0; //row colour counter
	while ($myrow=DB_fetch_array($PurchOrdersResult)) {
		...
		if (($myrow['status'] == PurchOrder::STATUS_PRINTED) || ($myrow['status'] =='Impresa')) {
			echo '>';
			$ReceiveOrder = "<a href='".$rootpath . "/GoodsReceived.php?" . SID . "&PONumber=" . $myrow["orderno"]."'>".
				_('Receive').'</a>';
				
				//AL INTRODUCIR EL NUMERO DE ORDEN REDIRECCIONA A GOODSRECIVED
				
				if($_SESSION['Module']=='stock'&&DB_num_rows($PurchOrdersResult)==1&&$_POST['OrderNumber']{0}==0){
					echo "<script>window.location='".$rootpath . "/GoodsReceived.php?" . SID . "&PONumber=" . $myrow["orderno"]."'</script>";
				}
...
//				<td><a href='%s'>" . _('Receive') . "</a></td>
		...
		if (in_array($PricesSecurity, $_SESSION['AllowedPageSecurityTokens']) OR !isset($PricesSecurity)) {
			...
			<td class=no_print><a href='".$ModifyPage."'>Modify</a></td>
			<td class=no_print>".$PrintPurchOrder."</td>
			<td class=no_print>".$ReceiveOrder."</td>
			</tr>";
	//end of page full new headings if
	}
	//end of while loop
