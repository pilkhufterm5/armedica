...
include('includes/class.pdf.php');
	...
	$title = $_SESSION['CompanyRecord']['coyname'];
...
if (isset($_POST['rh_comments'])){
	if(!isset($_SESSION['rh_comments'])){
...
function validate(F)
{
	if (F.value.length > 40){
		document.forma.DeliveryDetails.disabled = false;
	}else{
		document.forma.DeliveryDetails.disabled = true;
	}
}
</script>
<?php
...
/*CustomerID defined in header.inc */
		...
		$_SESSION['Items']->Location = $myrow['fromstkloc'];
                $_SESSION['Items']->LocationReal = $myrow['rh_fromstkloc'];
	...
	If ($_POST['Keywords']=='' AND $_POST['CustCode']=='') {
		...
		//insert wildcard characters in spaces
			...
			$SQL = "SELECT custbranch.brname,
				...
				AND custbranch.disabletrans=0 ORDER BY custbranch.brname";
...
# the branch was also selected from the customer selection so default the delivery details from the customer branches table CustBranch. The order process will ask for branch details later anyway
		...
		$_SESSION['Items']->DispatchTaxProvince = $myrow[13];
                $_SESSION['Items']->LocationReal = $myrow[14];
...
# defaulted from the entry of the userid and password.
	...
	// the order process will ask for branch details later anyway
		...
		$_SESSION['Items']->TaxGroup = $myrow['taxgroupid'];
		$_SESSION['Items']->DispatchTaxProvince = $myrow['taxprovinceid'];
                $_SESSION['Items']->LocationReal = $myrow[13];
		//$_SESSION['Items']->GetFreightTaxes();
...
if ($_SESSION['RequireCustomerSelection'] ==1
	...
	<CENTER><FONT SIZE=3><B><?php echo '- ' . $_SESSION['CompanyRecord']['coyname']; ?></B></FONT><BR>
...
// everything below here only do if a customer is selected
 	if (isset($_POST['CancelOrder'])) {
		...
		if($_SESSION['ExistingOrder']!=0) { //need to check that not already dispatched
			...
			echo '<BR><BR>';
			prnMsg(_('Procesando nueva venta'),'success');
			echo "<CENTER><FORM ACTION='POSEntry.php' METHOD=POST>";
			echo '<INPUT TYPE=submit NAME="CancelOrder" value="Continuar">';
			echo "</FORM></CENTER>";
			include('includes/footer.inc');
			exit;
	...
	If (isset($_POST['Search'])){
			...
			if ($_POST['StockCat']=='All'){
				$SQL = "SELECT stockmaster.stockid,
					...
					AND (stockmaster.stockid " . LIKE . " '" . $SearchString . "'
						OR stockmaster.barcode = '" . $_POST['StockCode'] ."') 
					AND stockmaster.discontinued=0
					ORDER BY stockmaster.stockid";
			} else {
				$SQL = "SELECT stockmaster.stockid,
		...
		if (DB_num_rows($SearchResult)==1){
			// bowikaxu realhost - check if item already exists in actuall sale<br>
	 ...
	 If (isset($_POST['QuickEntry'])){
	     ...
	     //do {
			...
			$sql = "SELECT stockmaster.mbflag
					...
					OR stockmaster.barcode = '".$NewItem."')";
					// bowikaxu - 10 july 2007 - search in barcode also				
			...
			if (DB_num_rows($KitResult)==0){
				...
				if ($myrow['mbflag']=='K'){	/*It is a kit set item */
					...
					// bowikaxu
					$update = 0;
				foreach ($_SESSION['Items']->LineItems as $Itms) {
				if($Itms->StockID==$NewItem){ // yes
					//echo "ENTRO A UPDATE CART ".DateAdd (Date($_SESSION['DefaultDateFormat']),'d', 2).'  -> '.$Itms->LineNumber."<BR>";
					$_SESSION['Items']->update_cart_item($Itms->LineNumber,
										($Itms->Quantity+1),
										$Itms->Price,
										($Itms->DiscountPercent/100),
										$Itms->Narrative,
										'No',DateAdd (Date($_SESSION['DefaultDateFormat']),'d', 2),'');
										$update = 1;
				}
			}
			if($update == 0){
	...
	if(isset($_POST['PriceLess'])){
		...
		foreach($_SESSION['Items']->LineItems as $OrderLine){
			if($OrderLine->Price < $OrderLine->StandardCost){ // el precio es menor
					...
					<INPUT TYPE=SUBMIT NAME='DeliveryDetails' VALUE='" . _('YES') . "' disabled=true>";
...
//        $sqlVerifyLoc = "SELECT rh_master_loccode FROM rh_locations_virtual WHERE loccode = '".$_SESSION['Items']->LocationReal."'";
...
//        }else{
//            $locSM = $_SESSION['Items']->LocationReal;
//        }
	...
	// bowikaxu realhost - insert price authorization
	...
	// Verifica si la venta se pago completa o es una parte
		...
		if($_POST['pago']=='cash'){
			...
			$sql = "INSERT INTO rh_possales (trans, ip, user, cash, total, credcard, cheque, bono) VALUES ('".$OrderNo."', '".$_SERVER['REMOTE_ADDR']."',
			...
			$sql = "INSERT INTO rh_possales (trans, ip, user, credcard, total, cash, cheque, bono) VALUES ('".$OrderNo."', '".$_SERVER['REMOTE_ADDR']."',
				 '".$_SESSION['UserID']."', '".$_SESSION['Items']->total."', '".$_SESSION['Items']->total."',0,0,0)";
			$pago = DB_query($sql,$db,'Imposible Insertar la Transaccion','Fallo insert a venta al publico',true);
		
		}else if($_POST['pago']=='bono'){
			$sql = "INSERT INTO rh_possales (trans, ip, user, bono, total, cash, cheque, credcard) VALUES ('".$OrderNo."', '".$_SERVER['REMOTE_ADDR']."',
				 '".$_SESSION['UserID']."', '".$_SESSION['Items']->total."', '".$_SESSION['Items']->total."',0,0,0)";
			$pago = DB_query($sql,$db,'Imposible Insertar la Transaccion','Fallo insert a venta al publico',true);
		
		}else {
			$sql = "INSERT INTO rh_possales (trans, ip, user, cheque, total, cash, credcard, bono) VALUES ('".$OrderNo."', '".$_SERVER['REMOTE_ADDR']."',
				 '".$_SESSION['UserID']."', '".$_SESSION['Items']->total."', '".$_SESSION['Items']->total."',0,0,0)";
			$pago = DB_query($sql,$db,'Imposible Insertar la Transaccion','Fallo insert a venta al publico',true);
		}
	...
	unset($_SESSION['Items']);
	echo "<CENTER>";	
	echo "<a href='" . $rootpath . '/ConfirmDispatch_Invoice.php?' . SID ."&OrderNumber=$OrderNo"."' target='_blank'><H1>Facturar</H1></A><BR>";
	echo "<a href='" . $rootpath . '/rh_ConfirmDispatch_Shipment.php?' . SID ."&OrderNumber=$OrderNo"."&ProcessInvoice=1&type=0' target='_blank'><H1>Remisionar (Rem. Chica)</H1></A>";
	echo "<a href='" . $rootpath . '/rh_ConfirmDispatch_Shipment.php?' . SID ."&OrderNumber=$OrderNo"."&ProcessInvoice=1&type=1' target='_blank'><H1>Remisionar (Rem. Grande)</H1></A>";
	echo "</CENTER>";
	echo "<CENTER><FORM ACTION='POSEntry.php' METHOD=POST>";
	echo '<INPUT TYPE=submit NAME="CancelOrder" value="Otra Venta">';
	echo "</FORM></CENTER>";
	include('includes/footer.inc');
	exit;
	//unset($_SESSION['Items']->LineItems);
...
/****************************************************************************************************************/
			echo '<TR><TD COLSPAN=7><TEXTAREA  NAME="Narrative_' . $OrderLine->LineNumber . '" cols=100% rows=1>' . $OrderLine->Narrative . '</TEXTAREA><BR><HR></TD></TR>';
/****************************************************************************************************************/
			//echo $RowStarter;
			//echo '<INPUT TYPE=text  NAME="Narrative_' . $OrderLine->LineNumber . '">';
		...
		// bowikaxu realhost - condicion del importe
		if($_SESSION['PayInCounter']==1){
			echo "<TD COLSPAN>"._('Importe')." $<INPUT TYPE=text NAME='importe' SIZE=10></TD>";
		}
		echo "</TR>";
	 ...
	 if (isset($_POST['QuickEntry']) && $_POST['QuickEntry']!=''){
	   ...
	   //}
	     ...
	     echo '<TR><INPUT TYPE="submit" name="QuickEntry" value="' . _('Entrada Rapida') . '">
                     <INPUT TYPE="submit" name="PartSearch" value="' . _('Buscar Articulo') . '"></TR>';
	     echo '</TABLE>';
...
<?php
		...
		echo "<FORM ACTION=".$_SERVER['PHP_SELF'] . '?' .SID." METHOD=POST>";
	   	echo '<input type="hidden" name="Search" value="' .  _('Buscar') . '">';
		...
		<INPUT TYPE="Text" NAME="StockCode" SIZE=15 MAXLENGTH=18 VALUE=''></TD>
		</TR>
		<TR><TD>
			...
			while ($myrow=DB_fetch_array($SearchResult)) {
				...
				if (file_exists($_SERVER['DOCUMENT_ROOT'] . $ImageSource)){
					...
					printf("<td><FONT SIZE=1>%s</FONT></td>
						<td><FONT SIZE=1>%s</FONT></td>
						<td><FONT SIZE=1>%s</FONT></td>
						<td ALIGN=CENTER><i>"._('NO PICTURE')."</i></td>
