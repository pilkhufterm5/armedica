...
/*
	...
	allocated 
*/
if(!isset($PreLlamada))$PreLlamada=false;
include_once ('includes/DefineSuppAllocsClass.php');
...
echo "<FORM id='ProcesarPagoProveedores' ACTION='" . $_SERVER['PHP_SELF'] . "?" . SID . "' METHOD=POST>";
...
if (isset($_POST['AllocTrans'])){
   ...
   /*Now display the potential and existing allocations put into the array above */
        ...
        echo '<TABLE class="tablaFacturas" CELLPADDING=2 COLSPAN=7 BORDER=0>';
	...
	$RowCounter = 0;
	    ...
	    if ($k == 1){
		    echo "<TR BGCOLOR='#CCCCCC' class='DataRow'>";
		    $k = 0;
	    } else {
		    echo "<TR BGCOLOR='#EEEEEE' class='DataRow'>";
		    $k = 1;
	    }
	    $RowCounter++;
...
<!--
	$(function(){
		$('.tablaFacturas tr').addClass('DataRow');
		$('.DataRow input').change(function(){
			$(this).closest('tr:not(.noRemove)').addClass('noRemove').removeClass('DataRow');
		});
		$('.DataRow input[type=text][value!=0]').map(function(){$(this).closest('tr').removeClass('DataRow');});
		$('#ProcesarPagoProveedores').submit(function(){
			$('.DataRow').remove();
// 			$(this).find('table.tablaFacturas tr input[type="checkbox"]:not(:checked)').map(function(){
// 					if($(this).closest('tr').find('input[name^="Amt"]').val()==0)
// 						$(this).closest('tr').hide().appenTo('body');
// 			});
		});
	});
//-->
