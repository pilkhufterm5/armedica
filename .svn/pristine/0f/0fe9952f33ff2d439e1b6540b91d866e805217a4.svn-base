		...
		if(error == 0){
			...
			jQuery.post("rh_pos_procesa.php",{type:"item", item:item, desc:desc, cate:cate}, function(ResBusqueda){
				...
				if(bandera_error == "si"){
                    ...
                    if (myObject.Scale=='YES'){
                        var stockid = myObject.item;/*este el que se agrega de forma automaticamente*/
                        var qty = myObject.qty;/*este el que se agrega de forma automaticamente*/
                        add_itemScale(stockid,qty);
                    }else{
...
function add_item(item){
    jQuery.post("rh_pos_procesa.php",{type:"addItem", item:item, qty:"NO"}, function(ResBusqueda){
       if(ResBusqueda == "no"){
...
function add_itemScale(item,qty){
    jQuery.post("rh_pos_procesa.php",{type:"addItem", item:item, qty:qty}, function(ResBusqueda){
       if(ResBusqueda == "no"){
           alert('No se pudo procesar tu peticion\nIntenta de Nuevo');
           limpiar();
           return false;
...
function get_lasItem(qty){
    jQuery.post("rh_pos_procesa.php",{type:"getLastItem", qty:qty}, function(ResBusqueda){
       eval(ResBusqueda);
    });
}
...
function caja_registradora(){
    $.msgbox("<p>Ingresar el codigo del articulo:</p>", {
        type: "prompt"
    },function(result){
        if(result){
            /*aki es donde se carga la venta*/
