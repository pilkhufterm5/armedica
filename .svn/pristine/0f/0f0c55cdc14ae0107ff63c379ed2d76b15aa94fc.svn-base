...
$title = _('Create Credit Note').' '._('Discount');
...
/* everything below here only do if a customer is selected
   first add a header to show who we are making a credit note for */
	...
	$sql = "SELECT rh_extcn FROM locations WHERE loccode = '".$_SESSION['CreditItems']->Location."'";
	$res = mysql_query($sql);
...
/*Now show options for the credit note */
		...
		*/
				// bowikaxu realhost - change the tag to discount
			   echo '<OPTION SELECTED VALUE="WriteOff">' . _('Discount');
		/*
		...
		// bowikaxu realhost - set the initial value
		$_POST['CreditType'] = 'WriteOff';
		if (!isset($_POST['CreditType']) OR $_POST['CreditType']=='Return'){
...
/*if the credit note is a return of goods then need to know which location to receive them into */
		  ...
		  echo '<TR><TD>' . _('Credit Note Text') . ' :</TD>
		  		<TD><TEXTAREA NAME=CreditText COLS=31 ROWS=5>' . $_POST['CreditText'] . '</TEXTAREA></TD></TR>
			</TR>';
    ...
    //finSERIES
		...
		/*Check for the worst */
		  if ($_POST['CreditType']=='WriteOff' AND !isset($_POST['WriteOffGLCode'])){
			// bowikaxu realhost setp 2007 - no necesita verificacion porque obtiene la cuenta al hacer el insert
...
/* SQL to process the postings for sales credit notes... 
	...
	$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg);
	 ...
	 // bowiaxu seleccionar la cuenta contable
	$sql = "SELECT rh_descaccount FROM locations WHERE loccode ='".$_POST['rh_Loc']."'";
	$res = DB_query($sql,$db,'imposible obtener cuenta de descuentos de la sucursal');
	$DescAccount = DB_fetch_array($res);
	$_POST['WriteOffGLCode']=$DescAccount['rh_descaccount'];
	...
	$DescAccount = DB_fetch_array($res);
					...
					$SQL = "INSERT INTO gltrans (
						...
						VALUES (
							...
							'" . $SQLCreditDate . "',
							" . $PeriodNo . ",
							" . $DescAccount['rh_descaccount'] . ",
							'" . $_SESSION['CreditItems']->DebtorNo .' '. _('Descuento')."',
							" . ($_SESSION['CreditItems']->total) . ",1
							)";
					// bowikaxu realhost sept 2007 - change the total amount
					//echo $SQL;
...
/*Now insert the Credit Note into the DebtorTrans table allocations will have to be done seperately*/
	...
	if($_POST['rh_Fiscal']==1){
		...
		$sql = "SELECT rh_extcn FROM locations WHERE loccode = '".$_POST['rh_Loc']."'";
...
/*first reverse credit the cost of sales entry*/
				  ...
				  $SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						" . $PeriodNo . ",
						" . $COGSAccount . ",
						'1-" . $_SESSION['CreditItems']->DebtorNo . " - " . $CreditLine->StockID . " x " . $CreditLine->Quantity . " @ " . $CreditLine->StandardCost . "',
						" . ($CreditLine->StandardCost * -$CreditLine->Quantity) . 					")";
...
/*the goods are coming back into stock so debit the stock account*/
					...
					$SQL = "INSERT INTO gltrans (
						...
						VALUES (
							...
							'3-" . $_SESSION['CreditItems']->DebtorNo . " - " . $CreditLine->StockID . " x " . $CreditLine->Quantity . " @ " . $CreditLine->StandardCost . "',
							" . ($CreditLine->StandardCost * $CreditLine->Quantity) . "
							)";
...
//Post sales transaction to GL credit sales
				...
				$SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						'4-" . $_SESSION['CreditItems']->DebtorNo . " - " . $CreditLine->StockID . " x " . $CreditLine->Quantity . " @ " . $CreditLine->Price . "',
						" . ($CreditLine->Price * $CreditLine->Quantity)/$_SESSION['CurrencyRate'] . "
						)";
				...
				if ($CreditLine->DiscountPercent !=0){
						...
						VALUES (
							...
							'5-" . $_SESSION['CreditItems']->DebtorNo . " - " . $CreditLine->StockID . " @ " . ($CreditLine->DiscountPercent * 100) . "%',
...
/*Post credit note transaction to GL credit debtors, debit freight re-charged and debit sales */
		  if (($_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost + $TaxTotal) !=0) {
			$SQL = "INSERT INTO gltrans (
				...
				VALUES (
					...
					'" . $_SESSION['CreditItems']->DebtorNo . "',
					" . -($_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost + $TaxTotal)/$_SESSION['CurrencyRate'] . ",1)";
		  ...
		  if ($_SESSION['CreditItems']->FreightCost !=0) {
			$SQL = "INSERT INTO gltrans (
				...
				VALUES (
					...
					'7-" . $_SESSION['CreditItems']->DebtorNo . "',
			...
			if ($TaxAmount !=0 ){
				$SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						'" . $_SESSION['CreditItems']->DebtorNo . "', 
						" . ($TaxAmount/$_SESSION['CurrencyRate']) . ",1
					)";
        ...
        //echo $transno;
        $datosSat = cfd($db, $idDebtortrans, $transno, $type, $serie, $idCsd, $idXsd, $xmlXsd,$_POST['rh_Loc']);
        ...
        echo '<A target="_blank" HREF="'.$rootpath."/rh_j_downloadFacturaElectronicaXML.php?downloadPath=XMLFacturacionElectronica/facturasElectronicas/" . $datosSat['noCertificado'] . "/" . $datosSat['serie'] . $datosSat['folio'] . '-' . $datosSat['transno'] .'.xml'. SID . '">'._('Descargar Nota de Cr&eacute;dito (CFD) en formato XML').'</A><BR>';
	...
	/* $SQL='COMMIT';
	 ...
	 echo _('Discount').' '._('Credit Note number') . ' ' . $CreditNo . ' ' . _('processed') . '<BR>';
	 echo '<A target="_blank" HREF="' . $rootpath . '/rh_PrintCustTrans.php?' . SID . '&FromTransNo=' . $CreditNo . '&InvOrCredit=Credit">' . _('Show this Credit Note on screen') . '</A><BR>';
	if ($_SESSION['InvoicePortraitFormat']==0){
	 	echo '<A HREF="' . $rootpath . '/rh_PrintCustTrans.php?' . SID . '&FromTransNo=' . $CreditNo . '&InvOrCredit=Credit&PrintPDF=True">' . _('Print this Credit Note') . '</A>';
	} else {
	 ...
	 echo '<P><A HREF="' . $rootpath . '/rh_DiscountCredit.php">' . _('Enter Another Credit Note') . '</A>';
