...
if (!isset($_GET['OrderNumber']) && !isset($_SESSION['ProcessingOrder'])) {
	...
	echo '<CENTER><A HREF="' . $rootpath . '/rh_SelectSalesOrder_Shipment.php?' . SID . '">' . _('Select a sales order to shipment'). '</A></CENTER>';
	echo '<P><BR><BR>';
...
//Quite el echo para que no se imprima el query.
	...
	if (DB_num_rows($GetOrdHdrResult)==1) {
		...
		if($_GET['type']=='1'){ // remision grande
			$sql = "SELECT rh_maxitmrem as Max FROM rh_locations WHERE loccode ='".$_SESSION['Items']->Location."'";
		}else { // remision punto de venta
			$sql = "SELECT rh_maxitmpos as Max FROM rh_locations WHERE loccode ='".$_SESSION['Items']->Location."'";
		}
		...
		if (db_num_rows($LineItemsResult)>0) {
			...
			echo '<CENTER><A HREF="'. $rootpath. '/rh_SelectSalesOrder_Shipment.php?' . SID . '">' ._('Select a different sales order to remisionar') .'</A></CENTER>';
			echo '<P>';
...
if (isset($_POST['ProcessInvoice']) && $_POST['ProcessInvoice'] != ""){
	...
	if (DB_num_rows($Result) != count($_SESSION['Items']->LineItems)){
		...
		echo '<CENTER><A HREF="'. $rootpath.'/rh_SelectSalesOrder_Shipment.php?' . SID . '">'. _('Select a sales order for confirming deliveries and shipment'). '</A></CENTER>';
 ...
 * rleal
 * Jul 1 2010
 * El ultimo stkmove despues de hacer cualquier insert
 
	$sqlUpdatSM = "select max(stkmoveno) as last from stockmoves";
    $rowSM = DB_fetch_array(DB_query($sqlUpdatSM, $db));
	$rh_stockmove2=$rowSM['last'];
							
    $sqlUpdatSM = "call update_allstockmovesnewqoh(". $rh_stockmove1 .",". $rh_stockmove2 .")";
	DB_query($sqlUpdatSM, $db);
 ...
 */
 	...
 	$sqlUpdatSM = "call update_locstock_newqoh(20000,". $InvoiceNo .")";
	DB_query($sqlUpdatSM, $db);
	...
	echo '<A HREF="'.$rootpath.'/rh_PDFRemGde.php?' . SID . 'FromTransNo='.$InvoiceNo.'&InvOrCredit=Invoice&PrintPDF=True">'. _('Imprimir Remision Grande'). '</A><BR>';
	...
	echo '<A HREF="'.$rootpath.'/rh_SelectSalesOrder_Shipment.php?' . SID . '">'. _('Select another order for shipment'). '</A><BR>';
	...
	echo "<TR>
		<TD>" . _('Tipo de Remision'). ":</TD>
		<TD><SELECT NAME='RemType'>
		
			<OPTION SELECTED VALUE='0'>"._('Remision Normal')."</OPTION>	
