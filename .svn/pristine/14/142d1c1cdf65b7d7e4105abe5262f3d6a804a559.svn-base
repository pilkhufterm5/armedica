...
$title = _('Orders Cancelled Report');

$InputError=0;

if (isset($_POST['FromDate']) AND !Is_Date($_POST['FromDate'])){
	$msg = _('The date from must be specified in the format') . ' ' . 'd/m/Y';
	$InputError=1;
	unset($_POST['FromDate']);
}
if (isset($_POST['ToDate']) AND !Is_Date($_POST['ToDate'])){
	$msg = _('The date to must be specified in the format') . ' ' . 'd/m/Y';
	$InputError=1;
	unset($_POST['ToDate']);
}
...
if (!isset($_POST['FromDate']) OR !isset($_POST['ToDate']) OR $InputError==1){
	...
	if ($InputError==1){
     ...
     echo '<CENTER><TABLE><TR><TD>' . _('Enter the date from which orders are to be listed') . ":</TD><TD><INPUT TYPE=text NAME='FromDate' MAXLENGTH=10 SIZE=10 VALUE='" . Date('d/m/Y', Mktime(0,0,0,Date('m'),Date('d')-1,Date('y'))) . "'></TD></TR>";
     echo '<TR><TD>' . _('Enter the date to which orders are to be listed') . ":</TD>
     		<TD><INPUT TYPE=text NAME='ToDate' MAXLENGTH=10 SIZE=10 VALUE='" . Date('d/m/Y') . "'></TD></TR>";
...
$pdf->addinfo('Title',_('This sales order has been cancelled as requested'));
...
include ('includes/rh_PDFOrdersCancelledPageHeader.inc');
...
header('Content-Disposition: inline; filename=OrdersCancelled.pdf');
header('Expires: 0');
