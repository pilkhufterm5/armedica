...
If (isset($_POST['PrintPDF'])
	...
	$SQL = "SELECT bom.parent,
			...
			(stockmaster.materialcost+stockmaster.overheadcost+stockmaster.labourcost) as costo,
			bom.quantity,
			bom.loccode,
			bom.workcentreadded,
	...
	While ($BOMList = DB_fetch_array($BOMResult,$db)){
		...
		// bowikaxu realhost sept 07 - show 4 decimals for all products
		...
		$LeftOvers = $pdf->addTextWrap(470,$YPos,50,$FontSize,$DisplayQuantity,'right');
		
		// bowikaxu realhost - show cost
		$LeftOvers = $pdf->addTextWrap(520,$YPos,50,$FontSize,number_format($BOMList['costo'],2),'right');
