...
if(isset($_POST['Select'])){
		...
		$resnewcust = DB_query($SQL,$db,'Error: imposible intentar obtener datos del cliente','el sql fue ');
		$newcust = DB_fetch_array($resnewcust);
		
		$_SESSION['NewCustRem'] = $newcust['debtorno'];
		$_SESSION['NewBranchRem'] = $newcust['branchcode'];
		$_SESSION['CustomerID']=$newcust['debtorno'];
	
		echo '<FORM NAME="quick" ACTION="' . $_SERVER['PHP_SELF'] . '?' . SID . '" METHOD=POST>';
		$sql = 'SELECT typeno FROM systypes WHERE typeid=20000';

		$lastrem = DB_query($sql,$db);
		$myrow = DB_fetch_row($lastrem);
		
		echo '<P>' . _('La ultima remision es la numero') . ' ' . $myrow[0] . '<BR>';
		
		  echo '<BR><CENTER><FONT SIZE=4 COLOR=BLUE><B>' . _('Quick Entry') . '</B></FONT><BR>
	     	<TABLE BORDER=1>
			<TR>
				<TD class="tableheader">' . _('Remision') . ' #</TD>
			</TR>';

		  //$_SESSION['QuickEntries']-1
	    for ($i=0;$i<=5;$i++){

	     	echo '<tr bgcolor="#CCCCCC">
			<TD><INPUT TYPE="text" name="remisiones[' . $i . ']" size=21 maxlength=20></TD>
			</TR>';
	   }
	   echo '</TABLE><INPUT TYPE="submit" name="facturar" value="' . _('Quick Entry') . '"></FORM>';

}
// bowikaxu realhost - oct 2007 - get the quick entry remisions field
// CREAR LA FACTURA
if(isset($_POST['facturar'])||(isset($_POST['Update']))||isset($_POST['ProcessInvoice'])){
	//if(isset($_POST['facturar'])||(isset($_POST['Update']))||isset($_POST['ProcessInvoice'])){
	$i=0;
	foreach($_POST['remisiones'] as $remact){
		if (strlen($remact)>0){
	...
	if(DB_num_rows($res)>0){
		...
		$remfact = DB_fetch_array($res);
		prnMsg('La remision: '.$remfact['Shipment'].' ya ha sido facturada !!','error');//$NoRem
...
/*read in all the guff from the selected order into the Items cart  */
	...
	if (DB_num_rows($GetOrdHdrResult)==1) {
		...
		$_SESSION['Invoice']->Location = $myrow['fromstkloc'];
	...
	if($prheader==1){
		$sql = "SELECT rh_serie, extinvoices FROM locations WHERE loccode = '".$_SESSION['Invoice']->Location."'";
		$res = mysql_query($sql);
...
if($_SESSION['GroupItems']==1){
		$LineItemsSQL = 'SELECT stkcode,
				...
				ORDER BY rh_remdetails.stockid';
}else {
	$LineItemsSQL = 'SELECT stkcode,
					stockmaster.description,
					stockmaster.controlled,
				...
				$LineItemsSQL .= ') ORDER BY rh_remdetails.stockid';
}
		...
		if (db_num_rows($LineItemsResult)>0) {
			$sql = "SELECT rh_maxitminv FROM locations WHERE loccode ='".$_SESSION['Invoice']->Location."'";
			$itmmaxres = DB_query($sql,$db);
			$ItmMax = DB_fetch_array($itmmaxres);
			while ($myrow=db_fetch_array($LineItemsResult)) {
					
				if(count($_SESSION['Invoice']->LineItems) >= $ItmMax['rh_maxitminv']){
				...
				if($_SESSION['GroupItems']==1){
					if(($myrow['price2']/$myrow['qty2'])<=0){
						...
						VUELVA A CREAR LA REMISION CON LOS DATOS CORRECTOS</H2></CENTER>";
...
if($_SESSION['GroupItems']==1){
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if ($_SESSION['Invoice']->LineItems[$k]->Quantity != $myrow['qty']) {
			...
			echo '<BR>'. _('Orig order for'). ' ' . 'Invoice' .' -- '.$_SESSION['Invoice']->LineItems[$k] . ' '. _('has a quantity of'). ' ' .
				$myrow['qty'] . ' '.
...
while($ii < $NoRem){	
	...
	$sql = "INSERT INTO gltrans (type, typeno, chequeno, trandate, periodno, account, narrative, amount, posted, jobref)
			SELECT type, typeno, chequeno, '".$TranDate."','".$PeriodNo."', account, CONCAT(narrative,'".'-F:'.$InvoiceNo."'), (-1*amount), 1, jobref FROM
...
/*Update order header for invoice charged on */
	$SQL = "UPDATE salesorders SET comments = 'Inv " . $InvoiceNo . "' WHERE orderno = " . $_SESSION['ProcessingOrder'];
	// bowikaxu realhost - nuevo cliente
	if(isset($_SESSION['NewCustRem'])){
...
*/
	...
	$sql = "SELECT extinvoices, rh_serie FROM locations WHERE loccode = '".$_SESSION['Invoice']->Location."'";
				...
				if ($OrderLine->DiscountPercent !=0){
					...
					$SQL = "INSERT INTO gltrans (
						...
						VALUES (
							...
							" . (($OrderLine->Price*$OrderLine->QtyDispatched)*-1)*$OrderLine->DiscountPercent/$_SESSION['CurrencyRate'] . ",1
						)";
...
// bowikaxu 18/08/06 INSERT A LA REMISION DE YA FACTURADA
	foreach($remisiones as $rem){
		...
		$SQL = 'UPDATE rh_InvoiceShipment SET Facturado=1, Invoice="'.$InvoiceNo.'" WHERE Shipment= '.$rem;
				...
				$SQL2 = "UPDATE debtortrans SET alloc = (ovamount+ovfreight), 
				...
				$SQL3 = "UPDATE stockmoves SET qty = 0, 
				debtorno = '".$_SESSION['Invoice']->DebtorNo."',
				branchcode = '".$_SESSION['Invoice']->Branch."'
				WHERE type = 20000 AND transno = ".$rem;
	...
	$SQL='COMMIT';
	$Result = DB_query($SQL,$db,'Error Imposible grabar transacciones','Fallo el QUERY: COMMIT',true);
...
<CENTER>
<B><?php echo "<H2><B>Cliente a Facturar </B></H2>".$msg; ?></B>
...
?>
    ...
    //<![CDATA[
            <!--
            document.forms['quick'].remisiones[0].select();
            document.forms['quick'].remisiones[0].focus();
            //-->
    //]]>
	    </script>
<?php
