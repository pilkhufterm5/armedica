...
if (isset($_POST['PrintExcel'])){
	...
	if (DB_error_no($db) !=0) {
		prnMsg(_('The stock usage for the selected criteria could not be retrieved because'));echo  ' - ' . DB_error_msg($db);
		if ($debug==1){
...
/*Libreria para exportar a excel */
	...
	$objPHPExcel->getActiveSheet()
		->setCellValue('A1', _('Month'))
		->setCellValue('B1', _('Usage'));
	...
	$objPHPExcel->getActiveSheet()->
							setCellValue('A'.$i, $DisplayDate)
                             ->setCellValue('B'.$i, number_format($myrow['qtyused'],$DecimalPlaces));
...
if (($myrow[2]=='K') OR ($myrow[2]=='A') OR ($myrow[2]=='D')) {
	...
	prnMsg( _('The selected item is a dummy or assembly or kit-set item and cannot have a stock holding') . '. ' . _('Please select a different item'));
...
echo _('Stock Code') . ":<input type=text name='StockID' size=21 maxlength=20 value='".htmlentities($_REQUEST['StockID'])."' >";
...
echo '</SELECT>';
echo " <INPUT TYPE=SUBMIT NAME='PrintExcel' VALUE='" . _('Print Excel') . "'>";
...
if (isset($_POST['ShowUsage'])){
	...
	echo "<br>";
	open_flash_chart_object( 700, 300, 'rh_charts10.php?&StockLocation='.$_POST['StockLocation'].'&StockID='.$StockID.'',false);
	}
