	 ...
	 If ($_POST['Keywords']=='' AND $_POST['CustCode']=='') {
		  ...
		  //insert wildcard characters in spaces
			   ...
			   $SQL = 'SELECT
				...
				AND custbranch.disabletrans=0 limit 100";
...
will be booked back into. */
     ...
     $_SESSION['CreditItems']->BrAdd7 = $myrow['braddress7'];
     $_SESSION['CreditItems']->BrAdd8 = $myrow['braddress8'];
     $_SESSION['CreditItems']->BrAdd9 = $myrow['braddress9'];
     $_SESSION['CreditItems']->BrAdd10 = $myrow['braddress10'];
			...
			if ($_POST['StockCat']=='All'){
				$SQL = "SELECT stockmaster.stockid,
					...
					GROUP BY stockmaster.stockid,
						stockmaster.description,
						stockmaster.units
					ORDER BY stockmaster.stockid limit 100";
			} else {
				$SQL = "SELECT stockmaster.stockid,
						stockmaster.description,
						stockmaster.units
	  ...
	  $OKToProcess = true;
		/*Check for the worst */
		  ...
		  if (!isset($_POST['ProcessCredit']) AND $OKToProcess == true){
			echo '<INPUT TYPE="SUBMIT" onclick="return validateForm()" NAME="ProcessCredit" VALUE="' . _('Process Credit Note') . '">';
		  }
		  echo '</CENTER><HR>';
	 } # end of if lines
...
/*Post credit note transaction to GL credit debtors, debit freight re-charged and debit sales */
		...
		foreach ( $TaxTotals as $TaxAuthID => $TaxAmount){
         ...
         //Jaime, el commit se hace en cfd
//    	 $SQL='COMMIT';
//    	 $Result = DB_query($SQL,$db);
         //\Jaime, el commit se hace en cfd
        //Factura Electronica
	...
	if ($_SESSION['InvoicePortraitFormat']==0){
	 ...
	 echo '<br/><A HREF="' . $rootpath . '/SelectCreditItems.php">' . _('Enter Another Credit Note') . '</A><br/>';
