...
if (isset($_POST['submit'])) {
	...
	if (!isset($_POST['EstDeliveryDays'])) {
        ...
        //iJPe
        //2010-02-18
        //Modificacion para poder ingresar sucursales virtuales como por default
        /*
         * iJPe
         * realhost
         * 2010-02-13
	...
	if (isset($SelectedBranch) AND $InputError !=1) {
		...
		$sql = "UPDATE custbranch SET brname = '" . $_POST['BrName'] . "',
						...
						braddress8 = '" . $_POST['BrAddress8'] . "',
						braddress9 = '" . $_POST['BrAddress9'] . "',
						braddress10 = '" . $_POST['BrAddress10'] . "',
						...
						defaultlocation='" . $locSM . "',
                                               rh_defaultlocation='" . $_POST['DefaultLocation'] . "',
...
//rleal Mar 3, 2010 Se agregaron las address7-10 para FE
		...
		$msg = _('La Rama del Cliente<b>').' ' . $_POST['BranchCode'] . ': ' . $_POST['BrName'] . ' '._('</b>ha sido agregada, agregue otra rama, o Regrese al <a href=index.php>Menu Principal</a>');
	}
	...
	if ($InputError==0) {
        ...
        if(strlen($_POST['CollectionPath'])>0){
            $sql="select * from rh_rutas_debtors where debtorno='".$DebtorNo."' and branchcode='".$_POST['BranchCode']."'";
            $result = DB_query($sql,$db);
            if(DB_num_rows($result)>0){
             $sql="update rh_rutas_debtors set idrutas='".$_POST['CollectionPath']."' where debtorno='".$DebtorNo."' and branchcode='".$_POST['BranchCode']."'";
             DB_query($sql,$db);
            }else{
              $sql="insert into rh_rutas_debtors (idrutas,debtorno,branchcode)  values('".$_POST['CollectionPath']."', '".$DebtorNo."','".$_POST['BranchCode']."');";
             DB_query($sql,$db);
            }
        }
...
//rleal Mar 3, 2010 Se agregaron las address7-10 para FE
	if (DB_error_no($db) ==0 and $InputError==0) {
		...
		unset($_POST['BrAddress8']);
		unset($_POST['BrAddress9']);
		unset($_POST['BrAddress10']);
 ...
 * rleal
 * Jun 01 2011
 * Se corrige un bug, se usa el id en ves del codigo en la tabla de rh_rutas
 */
	$sql = "SELECT debtorsmaster.name,
            ...
            rh_rutas.descripcion
		FROM custbranch
                left join rh_rutas_debtors on rh_rutas_debtors.debtorno =custbranch.debtorno and  rh_rutas_debtors.branchcode = custbranch.branchcode
                left join rh_rutas on rh_rutas.id =rh_rutas_debtors.idrutas  ,
			debtorsmaster ,
			areas,
			salesman,
			taxgroups
	...
	if ($myrow) {
		...
		echo "<tr><td class='tableheader'>"._('Code')."</td>
            ...
            <td class='tableheader'>"._('Ruta')."</td>
		...
		$_POST['BrAddress7'] = $myrow[7];
		$_POST['BrAddress8'] = $myrow[8];
		$_POST['BrAddress9'] = $myrow[9];
		$_POST['BrAddress10'] = $myrow[10];
		unset($myrow);
	}
...
//rleal Mar 3, 2010 Se agregaron las address7-10 para FE
		if ($InputError==0) {
			...
			$_POST['BrAddress8']  = $myrow['braddress8'];
			$_POST['BrAddress9']  = $myrow['braddress9'];
			$_POST['BrAddress10']  = $myrow['braddress10'];
			...
			$_POST['DefaultLocation'] = $myrow['rh_defaultlocation'];
			$_POST['DefaultShipVia'] = $myrow['defaultshipvia'];
			$_POST['CustBranchCode'] = $myrow['custbranchcode'];
			$_POST['DeliverBlind'] = $myrow['deliverblind'];
            $_POST['CollectionPath'] = $myrow['idrutas'];
            $_POST['metodoPago'] = $myrow['metodopago'];
            $_POST['cuentaPago'] = $myrow['cuentapago'];
		}
	...
	*/
	//rleal Mar 3, 2010 Se agregaron las address7-10 para FE
		if (isset($_GET['BranchCode'])){
			...
			$_POST['BrAddress7'] = $myrow['addrsss7'];
			$_POST['BrAddress8'] = $myrow['addrsss8'];
			$_POST['BrAddress9'] = $myrow['addrsss9'];
			$_POST['BrAddress10'] = $myrow['addrsss10'];
	...
	//rleal Mar 3, 2010 Se agregaron las address7-10 para FE
		...
		$_POST['BrAddress7'] = $_GET['BrAddress7']?$_GET['BrAddress7']:$_POST['BrAddress7'];
		$_POST['BrAddress8'] = $_GET['BrAddress8']?$_GET['BrAddress8']:$_POST['BrAddress8'];
		$_POST['BrAddress9'] = $_GET['BrAddress9']?$_GET['BrAddress9']:$_POST['BrAddress9'];
		$_POST['BrAddress10'] = $_GET['BrAddress10']?$_GET['BrAddress10']:$_POST['BrAddress10'];
	...
	echo '<TD><input tabindex=3 type="Text" name="ContactName" SIZE=41 MAXLENGTH=150 value="'. $_POST['ContactName'].'"></TD></TR>';
	echo '<TR><TD>'._('Calle').':</TD>';
	if (!isset($_POST['BrAddress1'])) {$_POST['BrAddress1']='';}
	echo '<TD><input tabindex=4 type="Text" name="BrAddress1" SIZE=41 MAXLENGTH=150 value="'. $_POST['BrAddress1'].'"></TD></TR>';
	echo '<TR><TD>'._('N&uacute;mero Exterior').':</TD>';
	if (!isset($_POST['BrAddress2'])) {$_POST['BrAddress2']='';}
	echo '<TD><input tabindex=5 type="Text" name="BrAddress2" SIZE=41 MAXLENGTH=150 value="'. $_POST['BrAddress2'].'"></TD></TR>';
	echo '<TR><TD>'._('N&uacute;mero Interior').':</TD>';
	if (!isset($_POST['BrAddress3'])) {$_POST['BrAddress3']='';}
	echo '<TD><input tabindex=6 type="Text" name="BrAddress3" SIZE=41 MAXLENGTH=150 value="'. $_POST['BrAddress3'].'"></TD></TR>';
	echo '<TR><TD>'._('Colonia').':</TD>';
	if (!isset($_POST['BrAddress4'])) {$_POST['BrAddress4']='';}
	echo '<TD><input tabindex=7 type="Text" name="BrAddress4" SIZE=31 MAXLENGTH=150 value="'. $_POST['BrAddress4'].'"></TD></TR>';
	echo '<TR><TD>'._('Localidad').':</TD>';
	if (!isset($_POST['BrAddress5'])) {$_POST['BrAddress5']='';}
	echo '<TD><input tabindex=8 type="Text" name="BrAddress5" SIZE=21 MAXLENGTH=150 value="'. $_POST['BrAddress5'].'"></TD></TR>';
	echo '<TR><TD>'._('Referecia').':</TD>';
	if (!isset($_POST['BrAddress6'])) {$_POST['BrAddress6']='';}
	echo '<TD><input tabindex=9 type="Text" name="BrAddress6" SIZE=45  MAXLENGTH=150 value="'. $_POST['BrAddress6'].'"></TD></TR>';

	//rleal Mar 3, 2010 Se agregaron las address7-10 para FE
	echo '<TR><TD>'._('Municipio').':</TD>';
	if (!isset($_POST['BrAddress7'])) {$_POST['BrAddress7']='';}
	echo '<TD><input tabindex=9 type="Text" name="BrAddress7" SIZE=45  MAXLENGTH=200 value="'. $_POST['BrAddress7'].'"></TD></TR>';

	echo '<TR><TD>'._('Estado').':</TD>';
	if (!isset($_POST['BrAddress8'])) {$_POST['BrAddress8']='';}
	//echo '<TD><input tabindex=9 type="Text" name="BrAddress8" SIZE=45  MAXLENGTH=150 value="'. $_POST['BrAddress8'].'"></TD></TR>';
    echo '<TD><SELECT tabindex=9 name="BrAddress8">';
    ...
    while($Rw= DB_fetch_array($rs)){
    	if (isset($_POST['BrAddress8']) and iconv("utf-8", 'ASCII//TRANSLIT',mb_strtoupper($Rw['estado'],'utf8'))==iconv("utf-8", 'ASCII//TRANSLIT',mb_strtoupper($_POST['BrAddress8'],'utf8'))) {
	...
	echo '<TR><TD>'._('Pa&iacute;s').':</TD>';
	if (!isset($_POST['BrAddress9'])) {$_POST['BrAddress9']='';}
	echo '<TD><input tabindex=9 type="Text" name="BrAddress9" SIZE=45  MAXLENGTH=150 value="'. $_POST['BrAddress9'].'"></TD></TR>';
	echo '<TR><TD>'._('C&oacute;digo postal').':</TD>';
	if (!isset($_POST['BrAddress10'])) {$_POST['BrAddress10']='';}
	echo '<TD><input tabindex=9 type="Text" name="BrAddress10" SIZE=25 MAXLENGTH=30 value="'. $_POST['BrAddress10'].'"></TD></TR>';
	...
	// bowikaxu realhost august 07 - only give permissions to authorized users
	if($auth){ // authorized
	...
	echo '<TD><input tabindex=18 type="Text" name="Email" SIZE=66 MAXLENGTH=95 value="'. $_POST['Email'].'"></TD></TR>';
    ...
    if($_SESSION['Rutas']==1){
	echo '<TR><TD>'._('Ruta de Cobranza').'  '.$_POST['CollectionPath'].' :</TD>';
	echo '<TD><SELECT tabindex=19 name="CollectionPath">';

	DB_data_seek($result,0);

	$sql = 'SELECT id as codigo, descripcion FROM rh_rutas';
	$result = DB_query($sql,$db);

	while ($myrow = DB_fetch_array($result)) {
		if (isset($_POST['CollectionPath']) and $myrow['codigo']==$_POST['CollectionPath']) {
			echo '<OPTION SELECTED VALUE=';
		} else {
			echo '<OPTION VALUE=';
		}
		echo $myrow['codigo'] . '>' . $myrow['descripcion'];

	} //end while loop
	echo '</SELECT></TD></TR>';
    }else{
      echo '<input type="hidden" name="CollectionPath" value="0" />';
    }
    ...
    echo '<TR><TD>'._('Metodo de Pago').':</TD>';
	if (!isset($_POST['metodoPago'])) {$_POST['metodoPago']='';}
	echo '<TD><input tabindex=27 type="Text" name="metodoPago" SIZE=60  value="'. $_POST['metodoPago'].'"></TD></TR>';

    echo '<TR><TD>'._('Cuentas de Pago').':</TD>';
	if (!isset($_POST['cuentaPago'])) {$_POST['cuentaPago']='';}
	echo '<TD><input tabindex=27 type="Text" name="cuentaPago" SIZE=60  value="'. $_POST['cuentaPago'].'"></TD></TR>';
