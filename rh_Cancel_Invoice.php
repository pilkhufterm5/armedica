...
bowikaxu - CANCELAR UNA FACTURA
regresar los articulos.
...
if (isset($_GET['InvoiceNumber']) && !$_SESSION['ProcessingCredit']&&isset($_SESSION['DiasCancel'])&&$_SESSION['DiasCancel']!='') {
	$sql="select * from debtortrans where debtortrans.type =10 and  debtortrans.transno = '" . $_GET['InvoiceNumber'] . "' and DATEDIFF(now(),LAST_DAY(trandate))<='".((int)$_SESSION['DiasCancel'])."'";
	$rs=DB_query($sql,$db);
	if(!($row=DB_fetch_array($rs))){
	    prnMsg('Solo durante de los primeros '.$_SESSION['DiasCancel'].' del mes siguiente puede cancelar una factura del mes anterior','error');
	    include("includes/footer.inc");
	    exit ;
	}
}
...
<script language="javascript">
	...
	function acceptterms(seleccion){
		// enable accept button
		//alert("TESTEANDO");
		//alert(seleccion);
		if(seleccion == 1){
			document.forms['formulario'].ProcessCredit.disabled=false;
		}else {
			document.forms['formulario'].ProcessCredit.disabled=true;
		}
	}
...
// bowikaxu realhost - get the ivoice period
	...
	// bowikaxu realhost dic 07 - check allocated AND debtortrans.alloc=0
...
/*CustomerID variable registered by header.inc */
		...
		// bowikaxu realhost 10 jan 2008 - get the ivoice date
		$_SESSION['CreditItems']->DeliveryDate = $myrow['trandate'];
...
/*now populate the line items array with the stock movement records for the invoice*/
		...
		if (db_num_rows($LineItemsResult)>0) {
			...
			while ($myrow=db_fetch_array($LineItemsResult)) {
				...
				$_SESSION['CreditItems']->LineItems[$myrow['rh_orderline']]->StandardCost = $myrow['standardcost'];
				$_SESSION['CreditItems']->GetExistingTaxes($myrow['rh_orderline'], $myrow['stkmoveno']);
				...
				if ($myrow['controlled']==1){/* Populate the SerialItems array too*/
					...
					while ($SerialItemsRow = DB_fetch_array($SerialItemsResult)){
						...
						$_SESSION['CreditItems']->LineItems[$myrow['rh_orderline']]->SerialItems[$SerialItemsRow['serialno']] = new SerialItem($SerialItemsRow['serialno'], -$SerialItemsRow['moveqty']);
						$_SESSION['CreditItems']->LineItems[$myrow['rh_orderline']]->QtyDispatched -= $SerialItemsRow['moveqty'];
					}
		...
		prnMsg( _('This invoice can not be credited using the automatic facility') . ' <BR>' . _('CRITICAL ERROR') . ': ' . _('Please report that a duplicate DebtorTrans header record was found for invoice') . ' '._('O tiene asignado algun pago o nota de credito'). ' ' . $SESSION['ProcessingCredit'],'warn');
		include("includes/footer.inc");
		exit;
...
if (isset($_POST['Location'])){
	...
	foreach ($_SESSION['CreditItems']->LineItems as $LineItem) {
		...
		//bowikaxu realhost - remove regenerate taxes, so it get the existing taxes on the invoice
...
echo '<FONT COLOR=BLUE><B>' . _('Numero de Cancelacion') . ' ' . $_SESSION['ProcessingCredit'] . '</B></FONT>
	<CENTER><FONT SIZE=4><B><U>' . $_SESSION['CreditItems']->CustomerName . '</U></B></FONT>
	<FONT SIZE=3> - ' . _('Cancelacion en Moneda') . ' ' . $_SESSION['CreditItems']->DefaultCurrency . "</CENTER><BR>";

echo "<FORM ACTION='" . $_SERVER['PHP_SELF'] . "?" . SID . "' METHOD=POST NAME='formulario'>";
...
foreach ($_SESSION['CreditItems']->LineItems as $LnItm) {
	...
	if ($LnItm->Controlled==1){
		...
		echo "<TD align=center><input type=hidden name='Quantity_" . $LnItm->LineNumber ."' maxlength=6 SIZE=6 value=" . $LnItm->QtyDispatched . ">".$LnItm->QtyDispatched."</TD>";
	...
	echo "<TD align=center><INPUT TYPE=HIDDEN NAME='Price_" . $LnItm->LineNumber . "' MAXLENGTH=6 SIZE=6 VALUE=" . $LnItm->Price . ">".$LnItm->Price."</TD>
	<TD><INPUT TYPE=HIDDEN NAME='Discount_" . $LnItm->LineNumber . "' MAXLENGTH=3 SIZE=3 VALUE=" . ($LnItm->DiscountPercent * 100) . "></TD>
	...
	if (is_array($LnItm->Taxes) ){
		foreach ($LnItm->Taxes AS $Tax) {
			...
			echo '<input type=hidden name="' . $LnItm->LineNumber . $Tax->TaxCalculationOrder . '_TaxRate" maxlength=4 SIZE=4 value="' . $Tax->TaxRate*100 . '">'.$Tax->TaxRate*100;
			$i++;
			if ($Tax->TaxOnTax ==1){
...
echo '<TR>
	...
	<TD><INPUT TYPE=HIDDEN SIZE=6 MAXLENGTH=6 NAME='ChargeFreightCost' VALUE=" . $_SESSION['CreditItems']->FreightCost . ">".$_SESSION['CreditItems']->FreightCost."</TD>";
...
foreach ($_SESSION['CreditItems']->FreightTaxes as $FreightTaxLine) {
	...
	echo  '<INPUT TYPE=HIDDEN NAME=FreightTaxRate' . $FreightTaxLine->TaxCalculationOrder . ' MAXLENGTH=4 SIZE=4 VALUE=' . $FreightTaxLine->TaxRate * 100 . '>'.$FreightTaxLine->TaxRate * 100;
...
// bowikaxu realhost March 2008 - set date for stockmoves inserts
$DefaultDispatchDate_Stock = Date($_SESSION['DefaultDateFormat']);

$DefaultDispatchDate = $_SESSION['CreditItems']->DeliveryDate;
//$DefaultDispatchDate = str_replace($_SESSION['CreditItems']->DeliveryDate,'-','/');
$DefaultDispatchDate = ConvertSQLDate($DefaultDispatchDate);

$OkToProcess = true;
// bowikaxu realhost - june 27 2007 - poner la cuenta de clientes
//$_POST['WriteOffGLCode'] = $_SESSION['CompanyRecord']['debtorsact'];
...
/*Now Get the next credit note number - function in SQL_CommonFunctions*/
	...
	// bowikaxu realhost - March 2008 - get stock movement period
	$PeriodNo_Stock = GetPeriod($DefaultDispatchDate_Stock, $db);
...
/*Start an SQL transaction */
	...
	// bowikaxu realhost March 2008 - format the date for the insert
	$DefaultDispatchDate_Stock = FormatDateForSQL($DefaultDispatchDate_Stock);
...
/*Now need to update the invoice DebtorTrans record for the amount to be allocated and if the invoice is now settled*/
	...
	$SQL = "UPDATE debtortrans SET
		...
		$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('El Status de la Factura no puede ser modificado');
		$DbgMsg = _('The following SQL to update the invoice allocation was used');
		$Result = DB_query($SQL,$db,$ErrMsg,DbgMsg,true);
/*Now insert the Cancelacion into the DebtorTrans table with the allocations as calculated above*/
/*
 * rleal
 * Jul 2010
 ...
 */
	$SQL = "INSERT INTO debtortrans (transno,
		...
		VALUES (". $CreditNo . ",
			...
			'" . $DefaultDispatchDate . "',
			" . $PeriodNo . ", concat('Fac-" . $_SESSION['ProcessingCredit'] . "-Usr-".$_SESSION['UserID']."',' ',now()),
...
/*Could do with setting up a more flexible freight posting schema that looks at the sales type and area of the customer branch to determine where to post the freight recovery */
		...
		//rleal Jun 17 2011 se agrega un round al amount del asiento
		foreach ( $TaxTotals as $TaxAuthID => $TaxAmount){	
			if ($TaxAmount !=0 ){
				$SQL = "INSERT INTO gltrans (
					...
					VALUES (
						...
						'" . $_SESSION['CreditItems']->DebtorNo . "', 
						" . round(($TaxAmount/$_SESSION['CurrencyRate']),2) . "
					)";
 ...
 * rleal
 * Jul 2010
 * Se agrega la fecha de hoy a la referencia
 */
	$SQL = "UPDATE debtortrans SET
		reference = concat(reference, 'NC-".$CreditNo."-Usr-".$_SESSION['UserID']."',' ',now())
		, invtext='".$_POST['CreditText']."' WHERE transno = " . $_SESSION['ProcessingCredit'] . "
		AND type=10";

		$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('La Referencia de la Factura no puede ser modificado');
		$DbgMsg = _('The following SQL to update the invoice allocation was used');
		$Result = DB_query($SQL,$db,$ErrMsg,DbgMsg,true);
		
	//TODO bowikaxu - insert el usuario de cancelacion a rh_usertrans.
    ...
    if($_SESSION['CFDIVersion']==22){
        try {
            $sqlQuery = "select serie,folio from rh_cfd__cfd where fk_transno = " . $_SESSION['ProcessingCredit'] . " and id_systypes = 10 limit 1";
        ...
        $sqlQuery = "SELECT rh_cfd__cfd.uuid,pfx,pfxpass FROM rh_cfd__cfd JOIN rh_cfd__locations__systypes__ws_csd ON    rh_cfd__locations__systypes__ws_csd.serie = rh_cfd__cfd.serie AND rh_cfd__locations__systypes__ws_csd.`id_systypes`=rh_cfd__cfd.`id_systypes`
	...
	echo _('Cancelacion Numero') . ' ' . $CreditNo . ' ' . _('procesada');
	if ($_SESSION['InvoicePortraitFormat']==0){
		//echo "<BR><A HREF='". $rootpath . "/PrintCustTrans.php?" . SID . "&FromTransNo=" . $CreditNo . "&InvOrCredit=Credit&PrintPDF=True'>" . _('Imprimir Cancelacion') . '</A>';
        echo "<BR><A target='_blank' HREF='". $rootpath . "/rh_printFE.php?" . SID . "&transno=" . $_SESSION['ProcessingCredit'] . "&isCfdCancelado=true' >" . _('Imprimir Cancelacion') . '</A>';
	} else {
...
/*end of process credit note */
	...
	echo '<TR><TD>' . _('Accion') . "</TD><TD><SELECT NAME=CreditType>";

	if (!isset($_POST['CreditType']) OR $_POST['CreditType']=="Return"){
		echo "<OPTION SELECTED VALUE='Return'>" . _('Regresar Articulos');
...
/*if the credit note is a return of goods then need to know which location to receive them into */
		...
		echo '<TR><TD>' . _('Regresar Articulos a') . '</TD><TD><SELECT NAME=Location>';
		...
		// bowikaxu realhost Feb 2008 - Acepto de Terminos
		echo '<TR><TD>' . _('Accept Cancel Terms') . '</TD><TD><SELECT NAME=AcceptTerms onchange="acceptterms(this.value)">';
	...
	// DISABLED=true
	echo '<TR><TD>' . _('Razones de Cancelacion') . '</TD><TD><TEXTAREA placeholder="DESCRIPCION ERRONEA; SE REFACTURA" NAME=CreditText COLS=31 ROWS=5>' . $_POST['CreditText'] . '</TEXTAREA></TD></TR>';
	echo '</TABLE><CENTER><INPUT TYPE=SUBMIT NAME=Update Value=' . _('Actualizar') . '><P>';
	echo "<INPUT TYPE=SUBMIT NAME='ProcessCredit' DISABLED Value='" . _('Procesar Cancelacion') ."'></CENTER>";
