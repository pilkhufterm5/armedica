...
if (isset($_POST['SearchCust']) AND $_SESSION['RequireCustomerSelection']==1 AND in_array(2,$_SESSION['AllowedPageSecurityTokens'])){
	...
	If (($_POST['Keywords']=='') AND ($_POST['CustCode']=='')  AND ($_POST['CustPhone']=='')) {
		...
		//insert wildcard characters in spaces
			...
			// bowikaxu realhost - may 2007 - search only in debtorsmaster.name the searchstring
...
// everything below here only do if a customer is selected
	...
	if(isset($_POST['CopyInvoice'])){
		
		$i=1;
		$sql ='SELECT stockmoves.stockid,
				   		stockmaster.description, 
						stockmoves.qty as quantity, 
						stockmoves.discountpercent, ((1 - stockmoves.discountpercent) * stockmoves.price * debtortrans.rate * stockmoves.qty) AS fxnet,
					...
					AND stockmoves.transno=' . $_POST['Invoice'] . '
					AND debtortrans.transno=' . $_POST['Invoice'] .'
...
//		echo "<FORM ACTION='".$_SERVER['PHP_SELF'] . '?' .SID."' METHOD=POST>";
		...
		while($part = DB_fetch_array($res)){
			
			//echo '<INPUT TYPE="text" name="part_' . $i . '" value="'.$part['stockid'].'">';
			//echo '<INPUT TYPE="text" name="qty_' . $i . '" value="'.(-1*$part['quantity']).'"><BR>';
			$cpyitem = 'part_'.$i;
			$cpyqty = 'qty_'.$i;
			$_POST[$cpyitem]=$part['stockid'];
			
			$_POST[$cpyqty]=(-1*$part['quantity']);
						
			$i++;
		}
		$_POST['QuickEntry']=_('Quick Entry');
		//echo "<INPUT type='SUBMIT' NAME='QuickEntry' value='" . _('Quick Entry') . "'></FORM>";
		...
		$sql = "SELECT debtortrans.transno,
			...
			locations.rh_serie
			FROM debtortrans, rh_invoicesreference, locations
			WHERE
			debtortrans.debtorno = '".$_SESSION['Items']->DebtorNo."'
			AND debtortrans.type=10
			AND rh_invoicesreference.intinvoice = debtortrans.transno
			AND locations.loccode = rh_invoicesreference.loccode
			ORDER BY debtortrans.transno DESC";
		
		echo "<SELECT NAME='Invoice'>";	
		$res = DB_query($sql,$db);
		while($inv = DB_fetch_array($res)){
		
			echo "<OPTION VALUE='".$inv['transno']."'> Factura: ".$inv['rh_serie'].$inv['extinvoice'].'-('.$inv['transno'].') $:'.$inv['total']." Fecha: ".$inv['trandate']."</OPTION>";
		
		}
		echo "</SELECT><INPUT TYPE='submit' NAME='CopyInvoice' VALUE='Copiar Factura'></FORM></CENTER>";
		// bowikaxu - fin de agregar articulos de la transaccion		
	}
	...
	if(isset($_POST['PriceLess'])){
		...
		foreach($_SESSION['Items']->LineItems as $OrderLine){
			...
			if($OrderLine->Price < $OrderLine->StandardCost){ // el precio es menor
					...
					echo "<FORM ACTION='" . $_SERVER['PHP_SELF'] . '?' . $SID . "' METHOD=POST>";
					echo "<BR><CENTER><INPUT TYPE=SUBMIT NAME='DeliveryDetails' VALUE='" . _('SI') . "'>";
					...
					exit;
						...
						echo "<H1><STRONG>"._('El precio del articulo ').$OrderLine->StockID._(' es menor que su costo');
						echo "<BR>"._('usted tiene autorizacion para hacer la operacion, Perola opcion se encuentra deshabilitada');
...
/* This is where the order as selected should be displayed  reflecting any deletions or insertions*/
		...
		echo '<BR><INPUT TYPE=SUBMIT NAME="Recalculate" Value="' . _('Recalcular y/o Agregar Factura') . '">
                <INPUT TYPE=SUBMIT NAME="DeliveryDetails" VALUE="' . _('Introducir Detalles y Comfirmar Orden') . '"><HR>';

	} # end of if lines
	
	// bowikaxu - Realhost - March 2007
	// obtener numero de factura y agregar articulos
