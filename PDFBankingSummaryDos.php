...
$SQL = "SELECT (rh_titular.folio) as FolioAfil,
    ...
    FROM debtortrans
        ...
        LEFT JOIN rh_reverseo on custallocns.transid_allocto=rh_reverseo.transid_reversto
...
include ('includes/PDFBankingSummaryPageHeaderDos.inc');
$Subtotal = 0;
$Impuesto_total = 0;
while ($myrow = DB_fetch_array($CustRecs)) {
    ...
    $y2 = 0;
    $LeftOvers = $pdf->addTextWrap(40, $YPos, 40, $FontSize, $myrow['FolioAfil'], 'center');
    ...
    $LeftOvers = $pdf->addTextWrap(180, $YPos, 45, $FontSize, ConvertSQLDate($myrow['daterevers']), 'left');
    if ($LeftOvers != '') {
        $LeftOvers = $pdf->addTextWrap(120, $YPos - $FontSize, 45, $FontSize, $LeftOvers, 'center');
        $y2 = 1;
    }
    $LeftOvers = $pdf->addTextWrap(230, $YPos, 60, $FontSize, $myrow['cobrador'], 'center');
    $LeftOvers = $pdf->addTextWrap(320, $YPos, 150, $FontSize, $myrow['name'], 'left');
    if ($LeftOvers != '') {
        $y2 = 1;
    ...
    $LeftOvers = $pdf->addTextWrap(500, $YPos, 110, $FontSize, $myrow['reference'], 'left');
    if ($LeftOvers != '') {
        $y2 = 1;
        $LeftOvers = $pdf->addTextWrap(500, $YPos - $FontSize, 110, $FontSize, $LeftOvers, 'left');
    }
