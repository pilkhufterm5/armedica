...
<?php
	if ($_REQUEST['OrderNumber']=='' OR !$_REQUEST['OrderNumber']){
		...
		echo _('Order number') . ": <INPUT type=text name='OrderNumber' MAXLENGTH =8 SIZE=9>&nbsp " . _('From Stock Location') . ":<SELECT id='StockLocation' name='StockLocation'> ";
		...
		echo '<SELECT id="Quotations" NAME="Quotations">';
      ...
      <TABLE>
      	<TR>
      		<TD><FONT SIZE=1>" . _('Select a stock category') . ":</FONT>
      			<SELECT id='StockCat' NAME='StockCat'>";
...
//end if stock search results to show
		...
		if (isset($_REQUEST['SelectedCustomer'])) {
			...
			if (isset($_REQUEST['SelectedStockItem'])) {
				...
				$SQL = "SELECT salesorders.orderno,
					...
					AND salesorders.fromstkloc_virtual = '". $_POST['StockLocation'] . "'
					ORDER BY salesorders.orderno";
			...
			if (isset($_REQUEST['SelectedStockItem'])) {
				...
				// bowikaxu realhost - may 2007 - rh_status, salesorderdetails.quantity-salesorderdetails.qtyinvoiced
				$SQL = "SELECT salesorders.orderno,
						debtorsmaster.debtorno,
						debtorsmaster.name,
						custbranch.brname,
						...
						salesorders.deliverydate,
					  salesorders.printedpackingslip,
						SUM(salesorderdetails.unitprice*(salesorderdetails.quantity-salesorderdetails.qtyinvoiced)*(1-salesorderdetails.discountpercent)) AS ordervalue
					FROM salesorders,
						salesorderdetails,
						debtorsmaster,
						custbranch
	...
	/*Se Agrega Columna Metodo Pago y el Tipo */
	if ($_POST['Quotations']=='Orders_Only'){
		$tableheader = "<TR>
				...
				<TD class='tableheader'>" . _('Metodo Pago') . "</TD>
				<TD class='tableheader'>" . _('Frecuencia') . "</TD>
	...
	while ($myrow=DB_fetch_array($SalesOrdersResult)) {
		...
		if ($_SESSION['PackNoteFormat']==1){ /*Laser printed A4 default */
			...
			$PrintDispatchNote = $rootpath . '/rh_PrintCustOrder.php?' . SID . '&TransNo=' . $myrow['orderno'];
        ...
        /*Obtengo Datos del Afiliado*/
       	$_2GetAfilData = "SELECT cobranza.folio, cob.nombre as cobrador, pm.paymentname, fp.frecuencia
        ...
        /*Obtengo Datos del rh_facturacion*/
       	$_2GetFacData = "SELECT tipo FROM rh_facturacion WHERE orderno = '{$myrow['orderno']}'";
        $_GetFacData=DB_query($_2GetFacData,$db);
        $GetFacData = DB_fetch_assoc($_GetFacData);
		...
		if ($_POST['Quotations']=='Orders_Only'){
			printf("<td><A HREF='%s'>%s</A></td>
				...
				$GetAfilData['paymentname'],
				$GetAfilData['frecuencia'],
				$GetFacData['tipo'],
