...
*
*/
If (isset($_POST['PrintExcel']) AND isset($_POST['ReportOrClose'])){
...
/*First off do the Inventory Comparison file stuff */
	...
	$objPHPExcel->getActiveSheet()->    setCellValue('A1',  _('Stock Check Comparison at ') . Date($_SESSION['DefaultDateFormat']));
	...
	$objPHPExcel->getActiveSheet()->    setCellValue('A2', $_SESSION['CompanyRecord']['coyname'])
									  ...
									  ->setCellValue('A3', _('Stock Check Comparison at ') . ' ' . $LocationName );
									  
	$objPHPExcel->getActiveSheet()								  
									  ->setCellValue('A4', _('Item'))
									  ->setCellValue('B4', _('QOH'))
									  ->setCellValue('C4', _('Counted'))
  									  ->setCellValue('D4', _('Reference'))
									  ->setCellValue('E4', _('Adjustment'));
	...
	While ($CheckItemRow = DB_fetch_array($CheckedItems,$db)){
		...
		if ($Location!=$CheckItemRow['loccode']){
			$objPHPExcel->getActiveSheet()->    setCellValue('A'.$i, $CheckItemRow['loccode'] . ' - ' . $CheckItemRow['locationname']);
			$i++;
		}
		$Location = $CheckItemRow['loccode'];

		if ($Category!=$CheckItemRow['categoryid']){
			$objPHPExcel->getActiveSheet()->    setCellValue('A'.$i, $CheckItemRow['categoryid'] . ' - ' . $CheckItemRow['categorydescription']);
			$i++;
		}
		...
		$objPHPExcel->getActiveSheet()								  
									  ->setCellValue('A'.$i, $CheckItemRow['stockid'])
									  ->setCellValue('B'.$i, $CheckItemRow['description'])
									  ->setCellValue('C'.$i, $CheckItemRow['qoh']);
		...
		if (DB_num_rows($Counts)==0){
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, _('No counts entered'));
  									  
			if ($_POST['ZeroCounts']=='Adjust'){
				
				$objPHPExcel->getActiveSheet()->setCellValue('E'.$i, -($CheckItemRow['qoh']));
			
			}
		} else {
			$TotalCount =0;
			while ($CountRow=DB_fetch_array($Counts,$db)){
				$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, $CountRow['qtycounted'])
											  ->setCellValue('E'.$i, $CountRow['reference']);						
				$TotalCount += $CountRow['qtycounted'];
				
				$i++;
			} // end of loop printing count information
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$i+1, _('Total for') . ': ' . $CheckItemRow['stockid'])
										 ->setCellValue('D'.$i+1, $TotalCount)		
										 ->setCellValue('E'.$i+1, $TotalCount-$CheckItemRow['qoh']);		
...
/*First off do the Inventory Comparison file stuff */
	...
	echo '</TABLE><INPUT TYPE=Submit Name="PrintExcel" Value="' . _('Imprimir Excel'). '"><INPUT TYPE=Submit Name="PrintPDF" Value="' . _('Print PDF'). '"></CENTER>';
