...
function mysql_report($query,$dump=false,$attr=array()){
    ...
    //$this->Output();
    //$_SESSION['reports_dir']
	include('includes/htmlMimeMail.php');
    $fp = fopen( "companies/reynagarza_erp_002_test/reports" . "/InventoryNegReport.pdf","wb");
	fwrite ($fp, $this->Output());
	fclose ($fp);
	$Recipients = array('"Carlos Reyna" <bowikaxu@gmail.com>');	
	$mail = new htmlMimeMail();
	$attachment = $mail->getFile( "companies/reynagarza_erp_002_test/reports" . '/InventoryNegReport.pdf');
	$mail->setText(_('Reporte de Inventarios Negativos'));
	//$mail->setHeader('html',"Content-type: text/html; charset=iso-8859-1\r\n");
	$mail->setSubject(_('Inventarios Negativos'));
	$mail->addAttachment($attachment, 'InventoryNegReport.pdf', 'application/pdf');
	$mail->setFrom('Inventarios Negativos' . "<" . 'info@reynagarza.com' . ">");
...
$pdf->AliasNbPages();
$pdf->connect('localhost','usr_reyna2_test','OLKS5410oki','reynagarza_erp_002_test');
$attr=array('titleFontSize'=>14,'titleText'=>'Reporte Inventarios Negativos');
$pdf->mysql_report("SELECT stockcategory.categorydescription AS CATEGORIA, 
			stockmaster.stockid AS CODIGO, 
			stockmaster.description AS DESCRIPCION, 
			locstock.quantity AS CANTIDAD_ACTUAL, 
			locstock.loccode AS ALMACEN,
			stockmaster.materialcost + stockmaster.labourcost + stockmaster.overheadcost AS COSTO_UNITARIO, 
			locstock.quantity *(stockmaster.materialcost + stockmaster.labourcost + stockmaster.overheadcost) AS TOTAL
		FROM stockmaster, 
			stockcategory, 
			locstock 
		...
		ORDER BY locstock.loccode,
			...
			stockmaster.stockid",false,$attr);
?>