			...
			while ($myrow=db_fetch_array($LineItemsResult)) {
				...
				if(count($_SESSION['Items']->LineItems) >= $ItmMax['rh_maxitminv']){
					...
					echo "<H2>Imposible agregar el articulo ".$myrow['stkcode']." se llego al limite de </H2>";
                                        ...
                                        //Jaime @oneLine @modify se quita alerta de precio iguales a 0
					if($myrow['unitprice']<=0 && false){
...
$sql = "SELECT rh_serie, extinvoices FROM rh_locations WHERE loccode = '".$_SESSION['Items']->Location."'";
$res = mysql_query($sql);
...
echo"<table><tr><td class='tableheader'>"._('Direccion Carta Porte').":<br>
...
echo '<FORM onSubmit="return isSerieSelected()" ACTION="' . $_SERVER['PHP_SELF'] . '?' . SID . '" METHOD=POST>';
...
echo '<TR>
	<TD COLSPAN=8 ALIGN=RIGHT>' . _('Total Carta Porte'). '</TD>
...
if (isset($_POST['ProcessInvoice']) && $_POST['ProcessInvoice'] != ""){
	...
	$InvoiceNo = GetNextTransNo(20002, $db);
...
// bowikaxu realhost - 9 july 2008 - save item description
    ...
    //Carta Porte
    $idXsd = 2;
    $xmlXsd = "<clausulaCartaPorte>\nCONDICIONES DEL CONTRATO DE TRANSPORTE QUE AMPARA ESTA CARTA DE PORTE\n\nPRIMERA.- Para los efectos del presente contrato de transporte se\ndenomina “Porteador” al transportista y “Remitente” al usuario que\ncontrate el servicio.\n\nSEGUNDA.- El “Remitente” es responsable de que la información\nproporcionada al “Porteador” sea veraz y que la documentación que\nentregue para efectos del transporte sea la correcta.\n\nTERCERA.- El “Remitente” debe declarar al “Porteador”el tipo de\nmercancía o efectos de que se trate, peso, medidas y/o número de la\ncarga que entrega para su transporte y, en su caso, el valor de la\nmisma. La carga que se entregue a granel será pesada por el “Porteador”\nen el primer punto donde haya báscula apropiada o, en su defecto,\naforada en metros cúbicos con la conformidad del “Remitente\n\n\nCUARTA.- Para efectos del transporte, el “Remitente”\ndeberá entregar al “Porteador” los documentos que las leyes\ny reglamentos exijan para llevar a cabo el servicio, en caso de no\ncumplirse con estos requisitos el “Porteador” está obligado a rehusar el\ntransporte de las mercancías.\n\nQUINTA.- Si por sospecha de falsedad en la declaración del contenido de\nun bulto el “Porteador” deseare proceder a su reconocimiento, podrá\nhacerlo ante testigos y con asistencia del “Remitente” o del\nconsignatario. Si este último no concurriere, se solicitará la presencia\nde un inspector de la Secretaría de Comunicaciones y Transportes, y se\nlevantará el acta correspondiente. El “Porteador” tendrá en todo caso la\nobligación de dejar los bultos en el estado en que se encotraban antes\ndel reconocimiento.\n\nSEXTA.- El “Porteador” deberá recoger y entregar la carga precisamente\nen los domicilios que señale el “Remitente”, ajustándose a los términos\ny condiciones convenidos. El “Porteador” sólo está obligado a llevar la\ncarga al domicilio del consignatario para su entrega una sola vez. Si\nésta no fuera recibida, se dejará aviso de que la mercancía queda a\ndisposición del interesado en las bodegas que indique el “Porteador”.\n\nSEPTIMA.- Si la carga no fuere retirada dentro de 30 días siguientes a\naquel en que hubiera sido puesta a disposición del consignatario, el\n“Porteador” podrá solicitar la venta en pública subasta con arreglo a lo\nque dispone el código de Comercio.\n\nOCTAVA.- El “Porteador” y el “Remitente” negociáran libremente el precio\ndel servicio, tomando en cuenta su tipo, características de los\nembarques, volumen, regularidad, clase de carga y sistema de pago.\n\n\nNOVENA.- Si el “Remitente” desea que el “Porteador” asuma la\nresponsabilidad por el valor de las mercancías o efectos que él declare\ny que cubra toda clase de riesgos, inclusive los derivados de caso\nfortuito o de fuerza mayor, las partes deberán convenir un cargo\nadicional, equivalente al valor de la prima del seguro que contrate, el\ncual se deberá expresar en la carta de porte.\n\nDECIMA.- Cuando el importe del flete no incluya el cargo adicional, la\nresponsabilidad del “Porteador” queda expresamente limitada a la\ncantidad equivalente a 15 días del salario mínimo vigente en el Distrito\nFederal por tonelada o cuando se trate de embarques cuyo peso sea mayor\nde 200 kg. pero menor de 1000 kg.; y a 4 días de salario mínimo por\nremesa cuando se trate de embarques con peso hasta de 200 kg.\n\nDECIMA PRIMERA.- El precio del transporte deberá pagarse en origen,\nsalvo convenio entre las partes de pago en destino. Cuando el transporte\nse hubiere concertado “Fletes por Cobrar”, la entrega de las mercancías\no efectos se hará contra el pago del flete y el “Porteador” tendrá\nderecho a retenerlos mientras no se le cubra el precio convenido.\n\nDECIMA SEGUNDA.- Si al momento de la entrega resultare algún faltante o\navería, el consignatario deberá hacerla constar en ese acto en la carta\nde porte y formular su reclamación por escrito al “Porteador” dentro de\nlas 24 horas siguientes.\n\nDECIMA TERCERA.- El “Porteador” queda eximido de la obligación de\nrecibir mercancías o efectos para su transporte, en los siguientes casos:\n\na) Cuando se trate de carga que por naturaleza, peso, volumen, embalaje\ndefectuoso o cualquier otra circunstancia no pueda transportarse sin\ndestruirse o sin causar daño a los demás artículos o al material\nrodante, salvo que la empresa de que se trate tenga el equipo adecuado.\n\nb) Las mercancías cuyo transporte haya sido prohibido por disposiciones\nlegales o reglamentarias. Cuando tales disposiciones no prohíban\nprecisamente el transporte de determinadas mercancías, pero sí ordenen\nla presentación de ciertos documentos para que puedan ser transportadas,\nel “Remitente” estará obligado a entregar al “Porteador” los documentos\ncorrespondientes.\n\nDECIMA CUARTA.- Los casos no previstos en las presentes condiciones y\nlas quejas derivadas de su aplicación se someterán por vía\nadministrativa a la Secretaría de Comunicaciones y Transportes.\n</clausulaCartaPorte>";
    ...
    echo _('Carta Porte'). ' '. $datosSat['serie'].$datosSat['folio'] .' '. _('processed'). '<BR>';
    ...
    echo '<A target="_blank" HREF="'.$rootpath."/rh_printFE.php?transno=$InvoiceNo&isCartaPorte=true" . SID . '">'._('Imprimir Carta Porte en PDF').'</A><BR>';
    ...
    echo '<A target="_blank" HREF="'.$rootpath."/rh_j_downloadFacturaElectronicaXML.php?downloadPath=XMLFacturacionElectronica/facturasElectronicas/" . $datosSat['noCertificado'] . "/" . $datosSat['serie'] . $datosSat['folio'] . '-' . $datosSat['transno'] .'.xml'. SID . '">'._('Descargar Carta Porte en formato XML').'</A><BR>';
	...
	/*end of process invoice */
        ...
        echo '<TABLE><TR>
		<TD>' ._('Tipo de Comprobante'). ':</TD>
		<TD>Ingreso: <input ' . ($_POST['cartaPorteTipoDeComprobante']=='ingreso'?'checked="true"':'') . ' type="radio" name="cartaPorteTipoDeComprobante" value="ingreso" id="cartaPorteTipoDeComprobanteIngreso" /><br/>Traslado:<input '.($_POST['cartaPorteTipoDeComprobante']=='traslado'?'checked="true"':'').' type="radio" name="cartaPorteTipoDeComprobante" value="traslado" id="cartaPorteTipoDeComprobanteTraslado"/></TD>
	</TR>';
	echo '<TR>
		<TD>' ._('Fecha de Carta Porte'). ':</TD>
...
****************************************************************************************************************************/
    ...
    $idLocation = $_SESSION['Items']->Location;
    $sql = "select id_ws_csd, serie from rh_cfd__locations__systypes__ws_csd where id_locations = '$idLocation' and !isnull(id_ws_csd) and id_systypes = 20002";
    $result = DB_query($sql,$db,'','',false,false);
	...
	echo '<INPUT TYPE=SUBMIT NAME="ProcessInvoice" Value="'._('Procesar Carta Porte').' '._('from').' '.$_SESSION['Items']->Location.'"</CENTER>';
...
<script type="text/javascript">
    function isSerieSelected(){
        //verificamos que se haya escogido si sera ingreso o traslado
        var elementTipoDeComprobanteIngreso = document.getElementById('cartaPorteTipoDeComprobanteIngreso')
        var elementTipoDeComprobanteTraslado = document.getElementById('cartaPorteTipoDeComprobanteTraslado')
        if(!elementTipoDeComprobanteIngreso.checked && !elementTipoDeComprobanteTraslado.checked){
            alert('Debe de escoger si la Carta Porte sera Ingreso o Traslado')
            return false
        }
        //\verificamos que se haya escogido si sera ingreso o traslado
...
try{
    if($serie == null)
        throw new Exception('No ha seleccionado una serie para este tipo de comprobante');
    $ws = getWs();
    $foliosLeft = $ws->foliosLeft(array('serie' => $serie))->return;
    $cfdsLeft = $ws->userCfdsLeft()->return;
    echo "<label>Le quedan $foliosLeft folios a la serie $serie</label>";
    echo '<br/>';
    echo "<label>CFDs restantes: $cfdsLeft</label>";
}
catch(Exception $e){
    prnMsg($e->getMessage(), 'error');
    include('includes/footer.inc');
