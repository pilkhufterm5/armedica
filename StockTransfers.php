...
//	if (strlen($_POST['StockText'])>0) {
	...
	if(DB_num_rows($result)==1){
		...
		if(isset($_GET['StockID'])&&!isset($_GET['Description']))
		$NewTransfer =true;
		$_GET['StockID']=$myrow['stockid'];
		$_GET['Description']=$myrow['description'];
		
	}else
	{
		echo '<center><table><tr><th>'._('Stock Code').'</th><th>'._('Stock Description').'</th></tr>';
...
$term=$_GET;
unset($term['EditControlled'],$term['DELETEALL'],$term['LineNo'],$term['Delete']);
foreach ($term as $item=>$val)
	$term[$item]="$item=$val";
$term=implode('&',$term);
...
/*All inputs must be sensible so make the stock movement records and update the locations stocks */
		...
		if (DB_num_rows($Result)==1){
			...
			$QtyOnHandPrior = 0;
                 ...
                 * rleal
                 * Feb 4, 2011
                 * Evitar que se transfieran articulos si no se tiene existencia.
                 */
                echo "loc:".$QtyOnHandPrior."<br>";
                echo "cant:".$_SESSION['Transfer']->TransferItem[0]->Quantity."<br>";

                if ($QtyOnHandPrior < $_SESSION['Transfer']->TransferItem[0]->Quantity ) {
...
/*Insert the StockSerialMovements and update the StockSerialItems  for controlled items*/
		...
		if ($_SESSION['Transfer']->TransferItem[0]->Controlled ==1){
			foreach($_SESSION['Transfer']->TransferItem[0]->SerialItems as $Item){
				$Expiracion='0000-00-00 00:00:00';
				$SQL="select * from stockserialitems where stockid='".
						$_SESSION['Transfer']->TransferItem[0]->StockID
						."' ".
						" and serialno='".$Item->BundleRef."'".
						"and expirationdate<>'0000-00-00 00:00:00'";
				if($res=DB_query($SQL,$db))
					if($Row=DB_fetch_assoc($res)){
					$Expiracion=$Row['expirationdate'];
				}
			...
			The StockSerialMoves as well */
				...
				$SerialItemExistsRow = DB_fetch_row($Result);
					/* Realhost 2013-05-30 Rafael Rojas
					 * Se actualiza la expiracion si es que en el destino no esta
					*/
					$SQLx = "UPDATE stockserialitems SET
					...
					AND serialno='" . $Item->BundleRef . "'
					and expirationdate='0000-00-00 00:00:00'";
					DB_query($SQLx, $db, $ErrMsg, $DbgMsg, false, false);
...
/*Insert the StockSerialMovements and update the StockSerialItems  for controlled items*/
		...
		if ($_SESSION['Transfer']->TransferItem[0]->Controlled ==1){
			foreach($_SESSION['Transfer']->TransferItem[0]->SerialItems as $Item){
			/* Realhost 2013-05-29 Rafael Rojas
				 * Fecha de expiracion
				*/
				$Expiracion='0000-00-00 00:00:00';
				$SQL="select * from stockserialitems where stockid='".
...
foreach($_SESSION['rh_permitionlocation'] as $key=>$value){
	if ($_POST['StockLocationFrom']==$key){
...
if ($_SESSION['Transfer']->TransferItem[0]->Controlled==1){
	echo '<TD ALIGN=RIGHT><INPUT TYPE=HIDDEN NAME="Quantity" VALUE=' . $_SESSION['Transfer']->TransferItem[0]->Quantity . '><A HREF="' . $rootpath .'/StockTransferControlled.php?' . $term . '">' . $_SESSION['Transfer']->TransferItem[0]->Quantity . '</A></TD></TR>';
} else {
