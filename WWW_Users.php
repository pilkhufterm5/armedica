...
if (isset($_POST['submit'])) {
        ...
        $InputError = 0;
    if (!in_array($_POST['DefaultLocation'],$_POST['PermitionLocation'])){
      array_push($_POST['PermitionLocation'],$_POST['DefaultLocation']);
    }
...
/*SelectedUser could also exist if submit had not been clicked this code would not run in this case cos submit is false of course  see the delete code below*/
        ...
        if ($InputError!=1){
                ...
                unset($_POST['rh_updatecost']);
        unset($_POST['PermitionLocation']);
                unset($SelectedUser);
        }
...
if (isset($SelectedUser)) {
        ...
        $_POST['rh_updatecost'] = $myrow['rh_updatecost'];
        // agregado por Daniel Villarreal el 20 de Abril del 2016, para restringir el campo costo total de rh titular en modulo afiliaciones
        $_POST['or_costo_afil'] = $myrow['or_costo_afil'];
        // Termina
        // agregado por Daniel Villarreal el 27 de Julio del 2016, para poder cancelar facturas y modificar el tipo de plan
...
$locCode=explode(',',$_POST['PermitionLocation']);
echo '<TR><TD>' . _('Almacenes permitidos') . ":</TD>
        <TD><select name='PermitionLocation[]' size='5' multiple='multiple' style='width:100%;'>";
$sql = "SELECT loccode, locationname FROM locations";
$result = DB_query($sql,$db);
while ($myrow=DB_fetch_array($result)){
        if (in_array($myrow['loccode'],$locCode)){
                echo "<OPTION SELECTED Value='" . $myrow['loccode'] . "'>" . $myrow['locationname'];
        } else {
                echo "<OPTION Value='" . $myrow['loccode'] . "'>" . $myrow['locationname'];
        }
...
echo '<TR><TD>' . _('Update Cost') . ":</TD><TD><SELECT name='rh_updatecost'>";
if ($_POST['rh_updatecost']==0){
        echo '<OPTION SELECTED VALUE=0>' . _('No');
        echo '<OPTION VALUE=1>' . _('Yes');
} else {
        echo '<OPTION SELECTED VALUE=1>' . _('Yes');
...
?>
<!-- 
AGREGADO PARA RESTRINGIR LA MODIFICACION DEL CAMPO COSTO TOTAL EN RH TITULAR A TRAVES DEL MODULO AFILIACIONES
POR DANIEL VILLARREAL EL 20 DE ABRIL DEL 2016
-->
<tr>
    <td>Modificar Costo Afil: </td>
    <td>
        <select name="or_costo_afil">
            <?php
            if ($_POST['or_costo_afil']==0){
                echo '<OPTION SELECTED VALUE=0>' . _('No');
                echo '<OPTION VALUE=1>' . _('Yes');
            } else {
                echo '<OPTION SELECTED VALUE=1>' . _('Yes');
...
-->
<!-- 
AGREGADO PARA CANCELAR UNA FACTURA Y MODIFICAR EL TIPO DE MEMBRESIA
POR DANIEL VILLARREAL EL 27 DE JULIO DEL 2016
-->
<input type="hidden" value="0" name="or_cancelar_factura">
<tr>
    <td>Modificar tipo Membresia: </td>
    <td>
        <select name="or_cambio_membresia">
            <?php
            if ($_POST['or_cambio_membresia']==0){
                echo '<OPTION SELECTED VALUE=0>' . _('No');
                echo '<OPTION VALUE=1>' . _('Yes');
            } else {
                echo '<OPTION SELECTED VALUE=1>' . _('Yes');
...
<tr>
        ...
        <td>
                <select name="or_cancelar_factura">
                        <?php
                        if ($_POST['or_cancelar_factura']==0){
                        echo '<OPTION SELECTED VALUE=0>' . _('No');
                        echo '<OPTION VALUE=1>' . _('Yes');
                        } else {
                        echo '<OPTION SELECTED VALUE=1>' . _('Yes');
...
<tr>
    <td>Administrar Afiliaciones: </td>
    <td>
        <select name="rh_adminafil">
            <?php
            if ($_POST['rh_adminafil']==0){
                echo '<OPTION SELECTED VALUE=0>' . _('No');
                echo '<OPTION VALUE=1>' . _('Yes');
            } else {
                echo '<OPTION SELECTED VALUE=1>' . _('Yes');
...
<tr>
    <td>Simulacion Aumentos de Precio: </td>
    <td>
        <select name="simulacion_aum_prec">
            <?php
            if ($_POST['simulacion_aum_prec']==0){
                echo '<OPTION SELECTED VALUE=0>' . _('No');
                echo '<OPTION VALUE=1>' . _('Yes');
            } else {
                echo '<OPTION SELECTED VALUE=1>' . _('Yes');
...
if(isset($SelectedUser)&&(isset($_SESSION['Permisos'])) &&($_SESSION['Permisos']>0)){
        ?>
        <tr><td style="vertical-align:top"><?=_('Restricci&oacute;n de p&aacute;ginas')?>:</td><td>
                  <a href="rh_permisosIndividuales.php?userid=<?=$SelectedUser?>">
                      <input type="button" value="<?=_('Editar Permisos Individuales')?>" />
                  </a>
           </td>
        </tr>
...
echo "</TABLE><CENTER><input type='Submit' name='submit' value='" . _('Enter Information') . "'></CENTER></FORM>";
