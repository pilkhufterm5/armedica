...
function get_category($category_id) {
	...
	// bowikaxu realhost august 07- if the category id == null then set it to 0 so it get all the categories
		...
		if (DB_num_rows($result1) >0 ) {
			...
			OR $_SESSION['AllowSalesOfZeroCostItems']==true) {
				...
				if(DB_num_rows($result) >0 ) {
					...
					if($price >= 0) {
						// bowikaxu - calculate discount quite -->  / 100
						...
						// bowikaxu - send also de discount rate
						...
						'tax' => number_format($tax_rate, 2, '.', '')); // bowikaxu - send the discount
					}
				}
			}
		}
...
*/
function create_order($products, $subtotal, $tax, $total, $customer_id, $sale_num, $rh_user, $rh_station, $rh_payment, $rh_time) {
	global $db;

	$_SESSION['Items'] = new Cart;
	$today = Date("Y-m-d H:i");

	// bowikaxu - get the date in sql format
	$rh_time = substr($rh_time,0,10);
	$fdate = explode("-",$rh_time);
	$rh_time_sql = $fdate[2]."/".$fdate[1]."/".$fdate[0];
	
	//$rh_time_sql = FormatDateForSQL($rh_time_sql);
	...
	$_SESSION['Items']->Orig_OrderDate = $rh_time;
	...
	for($i = 0; $i < count($products); $i++) {
		// check quantity
		//SELECT SUM(locstock.quantity) AS quantity
		// bowikaxu
		$result = DB_query("SELECT ".$products[$i]['Quantity']." AS quantity
...
// $products[$i]['Quantity']
	...
	// bowikaxu - change type name
	$InvoiceNo = GetNextTransNo(20000, $db);
	$PeriodNo = GetPeriod($rh_time_sql, $db);
	...
	$SQL = "INSERT INTO debtortrans (
		...
		VALUES (
			...
			'synPOS # ".$sale_num." Station:".$rh_station."',
			" . $_SESSION['Items']->ShipVia . ",
			''
		)";
	...
	/* bowikaxu realhost - insert a rh_possales */
	if($rh_payment == 'Cash'){
		$sql2 = "INSERT INTO rh_possales (trans, ip, user, cash, total, station, salesnum) VALUES 
			(".$DebtorTransID.", 
			'synPOS','".$rh_user."',".($_SESSION['Items']->total+$TaxTotal).", ".($_SESSION['Items']->total+$TaxTotal).", '".$rh_station."', ".$sale_num.")";
	}else if ($rh_payment == 'Check'){
		$sql2 = "INSERT INTO rh_possales (trans, ip, user, cheque, total, station, salesnum) VALUES 
			(".$DebtorTransID.", 
			'synPOS','".$rh_user."',".($_SESSION['Items']->total+$TaxTotal).", ".($_SESSION['Items']->total+$TaxTotal).", '".$rh_station."', ".$sale_num.")";
	}else {
		$sql2 = "INSERT INTO rh_possales (trans, ip, user, credcard, total, station, salesnum) VALUES 
			(".$DebtorTransID.", 
			'synPOS','".$rh_user."',".($_SESSION['Items']->total+$TaxTotal).", ".($_SESSION['Items']->total+$TaxTotal).", '".$rh_station."', ".$sale_num.")";
	}
	...
	foreach ($_SESSION['Items']->LineItems as $OrderLine) {
		...
		// bowikaxu - insert a rh_remdetails
...
function cancel_order($order_id) {
	...
	if (DB_num_rows($GetInvHdrResult)==1) {
		...
		$LineItemsSQL = "SELECT stockmoves.stkmoveno,
					...
					stockmoves.qty as qty2,
					
					rh_remdetails.qty AS quantity,
	...
	$SQL = "SELECT (ovamount+ovgst+ovfreight-ovdiscount-alloc) as baltoallocate
		FROM debtortrans
		WHERE transno=" . $_SESSION['ProcessingCredit'] . " AND type=20000";
	$Result = DB_query($SQL,$db);
	$myrow = DB_fetch_row($Result);
...
/**
*
*	bowikaxu - cancelar remision
*
*@param String $order_id The remision id to be cancelled
*/
function cancel_remls
($order_id){
...
/*read in all the guff from the selected invoice into the Items cart	*/
	...
	$InvoiceHeaderSQL = "SELECT DISTINCT
			...
			WHERE debtortrans.transno = " . $_GET['InvoiceNumber'] . "
				...
				AND rh_invoiceshipment.Shipment = debtortrans.transno
				AND stockmoves.type=20000";
...
/*Now insert the Cancelacion into the DebtorTrans table with the allocations as calculated above*/
	...
	$SQL = "INSERT INTO debtortrans (transno,
		...
		VALUES (". $CreditNo . ",
			...
			'" . $_SESSION['CreditItems']->Branch . "',
			'" . $DefaultDispatchDate . "',
			" . $PeriodNo . ", 'Rem-" . $_SESSION['ProcessingCredit'] . "-Usr-".$_SESSION['UserID']."',
			'" . $_SESSION['CreditItems']->DefaultSalesType . "',
			" . $_SESSION['CreditItems']->OrderNo . ",
			" . -($_SESSION['CreditItems']->total) . ",
			" . -$TaxTotal . ", " . -$_SESSION['CreditItems']->FreightCost . ",
					   ...
					   if ($Component_MBFlag=='M' OR $Component_MBFlag=='B'){
					   	...
					   	$SQL = "INSERT INTO stockmoves (
							...
							VALUES ('" . $AssParts['component'] . "',
								...
								'" . $_SESSION['CreditItems']->Branch . "',
								" . $PeriodNo . ",
								'" . _('Ex Rem') . ': ' .  $_SESSION['ProcessingCredit'] . ' ' . _('Assembly') . ': ' . $OrderLine->StockID . "',
								" . $AssParts['quantity'] * $OrderLine->QtyDispatched . ",
								'" . $OrderLine->StandardCost . "',
								0,
								" . ($QtyOnHandPrior + ($AssParts['quantity'] * $OrderLine->QtyDispatched)) . "
...
/* Insert stock movements for the stock coming back in - with unit cost */
					...
					$SQL = "INSERT INTO stockmoves (
						...
						VALUES ('" . $OrderLine->StockID . "',
							...
							" . $LocalCurrencyPrice . ",
							" . $PeriodNo . ",
							'" . _('Ex Rem') .' - ' . $_SESSION['ProcessingCredit'] . "',
							" . $OrderLine->QtyDispatched . ",
							" . $OrderLine->DiscountPercent . ",
							'" . $OrderLine->StandardCost . "',
							" .  ($QtyOnHandPrior + $OrderLine->QtyDispatched) . ",
...
/*Post credit note transaction to GL credit debtors, debit freight re-charged and debit sales */
		if (($_SESSION['CreditItems']->total + $_SESSION['CreditItems']->FreightCost) !=0) {
			$SQL = "INSERT INTO gltrans (type,
							typeno,
							trandate,
							periodno,
...
function get_customer($id) {
	...
	if (DB_num_rows($result) > 0) { // exists. update
		...
		'city' => $myrow['braddress5'],
		'state' => $customers['braddress6'],
		'zip' => $myrow['braddress4'],
		'country' => $myrow['braddress3'],
		'phone' => $myrow['phoneno'],
		'fax' => $myrow['faxno'],
		'email' => $myrow['email']
		);
	...
	if(isset($data['GetCategoryRequest'])) {
		...
		if(is_array($category)) {
			for($i = 0; $i < count($category['product']); $i++) {
				// product
				// bowikaxu - <Discount>".$category['product'][$i]['discount']."</Discount>
				$response .= "<Product>
                        		...
                        		<Discount>".$category['product'][$i]['discount']."</Discount>
                      			</Product>";
			}
		...
		// bowikaxu - get the SaleNum
		//$SaleNum = $data['AddOrderRequest']['SaleNum'];
		$order_id = create_order($products, $data['AddOrderRequest']['SubTotal'], $data['AddOrderRequest']['Tax'], 
		$data['AddOrderRequest']['Total'], $data['AddOrderRequest']['CustomerId'],$data['AddOrderRequest']['Salenum'],
		$data['AddOrderRequest']['rh_user'],$data['AddOrderRequest']['rh_station'], $data['AddOrderRequest']['rh_payment'],
		$data['AddOrderRequest']['rh_time']);
		...
		// bowikaxu - change cancel invoice to cancel remision
		//cancel_order($data['CancelOrderRequest']['OrderId'])
		if(cancel_rem($data['CancelOrderRequest']['OrderId'])) {
