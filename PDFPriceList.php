...
If (isset($_POST['PrintPDF'])
	...
	if ($_POST['CustomerSpecials']==_('Customer Special Prices Only')){
		...
		//rleal jul 21 2011 se agrega stockmaster.taxcatid
		$SQL = "SELECT prices.typeabbrev,
				prices.stockid,
				stockmaster.description,
				prices.currabrev,
	...
	While ($PriceList = DB_fetch_array($PricesResult,$db)){
		...
		//rleal jul 21 2011 se agrega el codigo del impuesto
		$LeftOvers = $pdf->addTextWrap($Left_Margin,$YPos,60,$FontSize,$PriceList['taxcatid']);
		$LeftOvers = $pdf->addTextWrap($Left_Margin+10,$YPos,60,$FontSize,$PriceList['stockid']);					$LeftOvers = $pdf->addTextWrap(120,$YPos,260,$FontSize,$PriceList['description']);
		...
		$DisplayUnitPrice = number_format($PriceList['price'],4);
