...
If (isset($_POST['PrintPDF'])
	...
	if ($_POST['All_Or_Overdues']=='All'){
		...
		$SQL = "SELECT debtorsmaster.debtorno, 
				...
				FROM debtorsmaster,
					...
					debtortrans 
				WHERE 
					debtortrans.type = 20000
					AND debtortrans.transno IN (SELECT Shipment FROM rh_invoiceshipment WHERE Facturado = 0)
	...
	While ($AgedAnalysis = DB_fetch_array($CustomerResult,$db)){
		...
		//$Factur = $AgedAnalysis['Factur'];
		// bowikaxu discriminar las remisiones ya facturadas

//		$Order_ = $AgedAnalysis['order_'];
//		$Tipo = $AgedAnalysis['type'];
		//print_r($AgedAnalysis);		
//		if($Tipo == 20000){
/*			
		$QueryRem = "SELECT * from rh_invoiceshipment
					where Invoice = ".$Order_.
		 		" AND Facturado = 0";
		
		$Resultado = DB_query($QueryRem,$db);
*/	
		 //while ($NoFact = DB_fetch_array($Resultado,$db) && ($Tipo = 20000)){
		...
		if ($_POST['DetailedReport']=='Yes'){
		   ...
		   $pdf->line($Page_Width-$Right_Margin, $YPos+10,$Left_Margin, $YPos+10);
				...
				$sql = "SELECT systypes.typename, 
				   ...
				   WHERE 
						debtortrans.type <> 20000
				   		AND systypes.typeid = debtortrans.type 
				   		AND debtorsmaster.paymentterms = paymentterms.termsindicator 
		   				AND debtorsmaster.debtorno = debtortrans.debtorno
