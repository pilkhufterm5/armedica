...
while ($myrow = DB_fetch_array($Result))
{
	...
	if ( DB_num_rows($Result2) == 0)
	{
		...
		echo "<font color=RED>No Sales Order</font>";
	}
	...
	{
		echo "<br>"._('Invoice ')." . $myrow['transno'] . " : ";
		echo "<font color=RED>Has no Tax Entry</font>";
	}

	$SQL4 = 'SELECT typeno FROM gltrans WHERE type = 10 AND typeno = ' . $myrow['transno'];
...
while ($myrow = DB_fetch_array($Result))
{
	$SQL2 = 'SELECT id, transno, trandate FROM debtortrans WHERE type = 10 AND transno = ' . $myrow['typeno'];
	$Result2 = DB_query($SQL2,$db);
...
$SQL = 'SELECT typeno, amount FROM gltrans WHERE type = 12 AND account = ' . $_SESSION['CompanyRecord']['debtorsact'];
$Result = DB_query($SQL,$db);

while ($myrow = DB_fetch_array($Result))
{
	$SQL2 = 'SELECT SUM((ovamount+ovgst)/rate) FROM debtortrans WHERE type = 12 AND transno = ' . $myrow['typeno'];
	$Result2 = DB_query($SQL2,$db);
...
while ($myrow = DB_fetch_array($Result))
{
	$SQL2 = 'SELECT transno, order_, trandate FROM debtortrans WHERE type = 10 AND order_ = ' . $myrow['orderno'];
	$Result2 = DB_query($SQL2,$db);
...
while ($myrow = DB_fetch_array($Result))
{
	...
	$sumsql = "SELECT SUM( qtyinvoiced * unitprice ) AS InvoiceTotal FROM salesorderdetails
				WHERE orderno = " . $myrow['orderno'];
	$sumresult = DB_query($sumsql,$db);

	if ( $sumrow = DB_fetch_array($sumresult) )
	{
		$invSQL = "SELECT transno, type, trandate, settled, rate, ovamount, ovgst
		...
		while( $invrow = DB_fetch_array($invResult) )
		{
			...
			if ( $invrow['type'] != 11 )
			{
					...
					{
						echo "<font color=red>"._(' Debtors trans ') . $invrow['ovamount'] . _(' differ from salesorderdetails ') . $sumrow['InvoiceTotal'] . "</font>";
					}
			}
		}
	}
...
while ($myrow = DB_fetch_array($Result))
{
	$SQL2 = 'SELECT transno, order_, trandate FROM debtortrans WHERE type BETWEEN 10 AND 11 AND transno = ' . $myrow['transno'];
	$Result2 = DB_query($SQL2,$db);
