...
/*end of Process Goods Received Reversal entry */
	...
	if (isset($_POST['ShowGRNS'])){
		...
		$sql = "SELECT grns.grnno,
				...
				grns.qtyrecd,
				grns.quantityinv,
				grns.qtyrecd-grns.quantityinv AS qtytoreverse,
                rh_factura_oc.transno,
                supptrans.suppreference,
                purchorders.orderno
			FROM grns join rh_factura_oc on grns.grnno = rh_factura_oc.grnno and grns.grnbatch=rh_factura_oc.grnbatch
                join supptrans on  rh_factura_oc.transno = supptrans.transno and rh_factura_oc.type = supptrans.type ,
                purchorderdetails, purchorders
			WHERE grns.podetailitem=purchorderdetails.podetailitem
		AND purchorders.orderno = purchorderdetails.orderno
        and grns.supplierid = '" . $_POST['SupplierID'] . "'
		...
		if (DB_num_rows($result) ==0){
			...
			while ($myrow=DB_fetch_array($result)) {
				...
				printf("<TD>%s</TD>
					...
					$myrow['grnno'],
                    $myrow['orderno'],
                    $myrow['suppreference'].' ('.$myrow['transno'].')',
					$myrow['itemcode'],
