...
if (isset($StockID)&& (!isset($_POST['submit']))) {
...
<script LANGUAGE="JavaScript">
	...
	$(function(){
		setTimeout(function(){
			$('[name=id_agrupador],[name^=rh_familia]').select2();
			$('[name=StockID]').focusout(function(){
				stockid=$(this).val();
				if(stockid.indexOf('_')){
					idAgrupador=parseInt(stockid.split('_')[0]);
					$('[name=id_agrupador]').find(':selected').removeAttr('selected');
					$('[name=id_agrupador]').find('[value='+idAgrupador+']').attr('selected','selected');
					$('[name=id_agrupador]').select2();
				}
			});
		},50);
	});
...
if (isset($_POST['submit'])||isset($_POST['submit2'])) {
	...
	if (!isset($_POST['Description']) or strlen($_POST['Description']) > 200 OR strlen($_POST['Description'])==0) {
		$InputError = 1;
	...
	if ($InputError !=1){
		...
		if (!isset($_POST['New']) and !isset($New)) { /*so its an existing one */
			...
			*/
			...
			// bowikaxu realhost 15 july 2008 - verify chances between flags
			if ($OldMBFlag != $_POST['MBFlag']){
				if (($OldMBFlag == 'M' OR $OldMBFlag=='B') AND ($_POST['MBFlag']=='E' OR $_POST['MBFlag']=='A' OR $_POST['MBFlag']=='K' OR $_POST['MBFlag']=='D')){ /*then need to check that there is no stock holding first */
					if ($stkqtychk[0]!=0){
						$InputError=1;
				...
				if ($_POST['MBFlag']=='K' OR $_POST['MBFlag']=='A' OR $_POST['MBFlag']=='E' OR $_POST['MBFlag']=='D') {
				...
				// bowikaxu realhost 15 july 2008 - ensamblado costo por componente
				if (($OldMBFlag=='M' OR $OldMBFlag =='K' OR $OldMBFlag=='A' OR $OldMBFlag=='E' OR $MBFlag=='C') AND ($_POST['MBFlag']=='B' OR $_POST['MBFlag']=='D')) {
				...
				// bowikaxu realhost 15 july 2008 - ensamblado costo por componente
				if (($OldMBFlag=='M' OR $OldMBFlag =='B' OR $OldMBFlag=='D') AND ($_POST['MBFlag']=='E' OR $_POST['MBFlag']=='A' OR $_POST['MBFlag']=='K' OR $_POST['MBFlag']=='C')) {
			...
			if ($InputError == 0){
				...
				$sql = "UPDATE stockmaster
						SET longdescription='" . $_POST['LongDescription'] . "',
							...
							lowestlevel = ".$_POST['LowestLevel'].",
							rh_lowestprod = ".$_POST['rh_lowestprod'].",
							decimalplaces=" . $_POST['DecimalPlaces'] . ",
							rh_marca = " . $_POST['rh_marca'] . ",
							rh_sales_factor=".$_POST['factorVenta'].",
							rh_sustanciaactiva=".$_POST['rh_sustanciaactiva'].",
							rh_cantidadporunidad=".$_POST['rh_cantidadporunidad'].",
							id_agrupador='".DB_escape_string($_POST['id_agrupador'])."',
							is_farmacia=".$_POST['is_farmacia'].",
							is_cortesia = ".$_POST['is_cortesia']."
					WHERE stockid='$StockID'";
                //rh_cantidadporunidad
				$ErrMsg = _('The stock item could not be updated because');
				$DbgMsg = _('The SQL that was used to update the stock item and failed was');
				$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);

                $sql="select * from rh_especie_stock where stockid='".$StockID."'";
                $rs=DB_query($sql,$db);
                if(DB_num_rows($rs)>0){
                    $sql="update rh_especie_stock set idEspecie ='".$_POST['rh_especie']."' where stockid='".$StockID."'";
                    $ErrMsg = _('Error 0x0001 actualizar especie');
				    $DbgMsg = _('Eror: 0x0001 Error de SQL');
				    $rs=DB_query($sql,$db,$ErrMsg,$DbgMsg);
                }else{
                ...
                $sql="select * from rh_gamma_stock where stockid='".$StockID."'";
                $rs=DB_query($sql,$db);
                if(DB_num_rows($rs)>0){
                    $sql="update rh_gamma_stock set idGamma ='".$_POST['rh_gamma']."' where stockid='".$StockID."'";
				    $ErrMsg = _('Error 0x0001 actualizar gamma');
				    $DbgMsg = _('Eror: 0x0001 Error de SQL');
				    $rs=DB_query($sql,$db,$ErrMsg,$DbgMsg);
                }else{
                    $sql="insert into rh_gamma_stock (idGamma,stockid) values('".$_POST['rh_gamma']."','".$StockID."');";
				    $ErrMsg = _('Error 0x0001 actualizar gamma');
				    $DbgMsg = _('Eror: 0x0001 Error de SQL');
				    DB_query($sql,$db,$ErrMsg,$DbgMsg);
                }
                $ErrMsg = _('Error 0x0001 actualizar familia');
				$DbgMsg = _('Eror: 0x0001 Error de SQL');
				...
				foreach($_REQUEST['rh_familia'] as $clave=>$familia)
					if($familia!=''){
					$sql="insert into rh_familia_stock (categoria,clave,stockid) values('".DB_escape_string($clave)."','".DB_escape_string($familia)."','".DB_escape_string($StockID)."');";
					DB_query($sql,$db,$ErrMsg,$DbgMsg);
				}
            ...
            //echo "SELECT stockid FROM stockmaster WHERE stockid='" . $StockID ."'";
			...
			$sql = "INSERT INTO stockmaster (
						...
						VALUES ('$StockID',
							...
							" . $_POST['rh_marca']. ",
							".$_POST['factorVenta'].",
                         ".$_POST['rh_sustanciaactiva'].",
						 '".DB_escape_string($_POST['id_agrupador'])."',
						  ".$_POST['is_farmacia'].",
						  '".$_POST['is_cortesia']."'
							)";
	...
	prnMsg( 'articulo agregado correctamente', 'success');
				...
				foreach($_REQUEST['rh_familia'] as $clave=>$familia)
					...
					if (DB_error_no($db)==0) {
						...
						unset($_POST['LowestLevel']);
						unset($_POST['rh_lowestprod']);
						unset($_POST['factorVenta']);
						unset($StockID);

						unset($_POST['rh_marca']);
                        unset($_POST['rh_gamma']);
                        unset($_POST['rh_especie']);//rh_sustanciaactiva: rh_cantidadporunidad
                        unset($_POST['rh_sustanciaactiva']);
                        unset($_POST['rh_cantidadporunidad']);
                        unset($_POST['id_agrupador']);
                        unset($_POST['is_farmacia']);
                        unset($_POST['is_cortesia']);
...
// PREVENT DELETES IF DEPENDENT RECORDS IN 'StockMoves'
	...
	if ($CancelDelete==0) {
		$result = DB_query('BEGIN', $db);
			...
			//rleal Mayo 14 2011 se agrega el cost history
			$sql = "DELETE FROM rh_costhistory WHERE stockid='$StockID'";
			$result=DB_query($sql,$db,_('Could not delete the bill of material because'),'',true);

			$sql="DELETE FROM rh_especie_stock WHERE stockid='$StockID'";
			$result=DB_query($sql,$db, _('Could not delete the item record'),'',true);
 			$sql="DELETE FROM rh_gamma_stock WHERE stockid='$StockID'";
			$result=DB_query($sql,$db, _('Could not delete the item record'),'',true);
	
			$sql="DELETE FROM rh_familia_stock WHERE stockid='$StockID'";
			$result=DB_query($sql,$db, _('Could not delete the item record'),'',true);
		...
		unset($_POST['rh_marca']);
        unset($_POST['rh_especie']);
        unset($_POST['rh_gamma']);
        unset($_POST['factorVenta']);//rh_cantidadporunidad
        unset($_POST['rh_sustanciaactiva']);
        unset($_POST['rh_cantidadporunidad']);
...
/*If the page was called without $StockID passed to page then assume a new stock item is to be entered show a form with a part Code field other wise the form showing the fields with the existing entries against the part will show for editing with only a hidden StockID field. New is set to flag that the page may have called itself and still be entering a new part, in which case the page needs to know not to go looking up details for an existing part*/
	...
	$sql = "SELECT stockmaster.stockid,
           ...
           is_farmacia,
           is_cortesia
		FROM stockmaster left join rh_especie_stock on rh_especie_stock.stockid = stockmaster.stockid
    ...
    $_POST['LowestLevel'] = $myrow['lowestlevel'];
    $_POST['rh_lowestprod'] = $myrow['rh_lowestprod'];
	$_POST['factorVenta']=$myrow['rh_sales_factor'];
	$_POST['rh_marca'] = $myrow['rh_marca'];
    $_POST['rh_especie'] = $myrow['idEspecie'];
    $_POST['rh_gamma'] = $myrow['idGamma']; //rh_sustanciaactiva
    $_POST['rh_sustanciaactiva'] = $myrow['rh_sustanciaactiva'];//rh_cantidadporunidad
    $_POST['rh_cantidadporunidad'] = $myrow['rh_cantidadporunidad'];
    $_POST['id_agrupador'] = $myrow['id_agrupador'];
    $_POST['is_farmacia'] = $myrow['is_farmacia'];
    $_POST['is_cortesia'] = $myrow['is_cortesia'];
    
	//Tabla de familia gamma N
	$_REQUEST['rh_familia']=array();
	$SQL="select * from rh_familia_stock where stockid='".DB_escape_string($StockID)."'";
	$res=DB_query($SQL,$db);
	while ($fila=DB_fetch_assoc($res)){
		$_REQUEST['rh_familia'][$fila['categoria']]=$fila['clave'];
	}
	...
	//echo '<TR><TD>' . _('Item Code') . ':</TD><TD>'.$StockID.'</TD></TR>';
    echo '<TR><TD>'. _('Item Code'). ':</TD><TD><INPUT ' . (in_array('StockID',$Errors) ?  'class="inputerror"' : '' ) .'  TYPE="TEXT" NAME="StockID" SIZE=21 MAXLENGTH=20 value='.$StockID.'></TD></TR>'. "\n";
...
echo '<TR><TD>' . _('Part Description') . ' (' . _('short') . '):</TD><TD><input ' . (in_array('Description',$Errors) ?  'class="inputerror"' : '' ) .' type="Text" name="Description" SIZE=52 MAXLENGTH=200 value="' . htmlentities($Description,ENT_QUOTES,_('UTF-8')) . '"></TD></TR>'."\n";
...
echo "	<TR><TD>" . _('Cortesia ') . ":</TD>
			<TD><input type='checkbox' name='is_cortesia'  " .(($_POST['is_cortesia']==1)? "checked='checked' '" :''). " ></TD>
		</TR>";

echo "	<TR><TD>" . _('Pertence a Farmacia') . ":</TD>
			<TD><input type='checkbox' name='is_farmacia'  " .(($_POST['is_farmacia'] ==1)? "checked='checked' '" :''). " ></TD>
		</TR>";
//Agregado
echo '<TR>'
    . '<TD>' . _('ID Agrupador') . ':</TD>'
    . '<TD>';

	$SQL="select * from rh_stock_grupo order by nombre ASC ";
	$res=DB_query($SQL,$db);
	echo '<select name=id_agrupador >';
	echo '<option value="">Ninguno</option>';
	while($FilaAg=DB_fetch_assoc($res)){
		echo '<option value="'.$FilaAg['clave'].'" ';
		if($_POST['id_agrupador']==$FilaAg['clave'])
			echo ' selected=selected ';
		echo '>';
		echo htmlentities('( '.$FilaAg['clave'].' )'.$FilaAg['nombre']);
		echo '</option>';
	}
	echo '</select>';
     echo '</TD></TR>';

echo '<tr><td>' . _('Sustancia Activa') . ':</td><td><select name="rh_sustanciaactiva">'; //<option value="677">SU TEST</option>
echo '<OPTION VALUE="0">No Aplica</option>';
$sql = 'SELECT * FROM rh_sustanciaactiva ORDER BY nombre ASC';
$ErrMsg = _('The stock categories could not be retrieved because');
$DbgMsg = _('The SQL used to retrieve stock categories and failed was');
$result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
while ($myrow=DB_fetch_array($result)){
    if ($myrow['id'] == $_POST['rh_sustanciaactiva']){
        echo '<OPTION SELECTED VALUE="'. $myrow['id'] . '">' . $myrow['nombre'];
    } else {
        echo '<OPTION VALUE="'. $myrow['id'] . '">' . $myrow['nombre'];
    }
...
//********************Agregado para Gamma y especie *****************************
//********************31/03/2011*************************************************
if($_SESSION['Especie']==1){
    echo '<tr><td>' . _($_SESSION['EspecieName']) . ':</td><td><select name="rh_especie">';
    $sql = 'SELECT * FROM rh_especie ORDER BY id ASC';
    $ErrMsg = _('Error 0X001: Error de SQL');
    $DbgMsg = _('Error 0X001: Error de SQL');
    $result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
    while ($myrow=DB_fetch_array($result)){
	    if ($myrow['id'] == $_POST['rh_especie']){
  ...
  echo '<input type="hidden" name="rh_especie" value="0" />';
}
if($_SESSION['Gamma']==1){
    echo '<tr><td>' . _($_SESSION['GammaName']) . ':</td><td><select name="rh_gamma">';
    $sql = 'SELECT * FROM rh_gamma ORDER BY descripcion ASC';
    $ErrMsg = _('Error 0X001: Error de SQL');
    $DbgMsg = _('Error 0X001: Error de SQL');
    $result = DB_query($sql,$db,$ErrMsg,$DbgMsg);
    while ($myrow=DB_fetch_array($result)){
	    if ($myrow['id'] == $_POST['rh_gamma']){
		    echo '<OPTION SELECTED VALUE="'. $myrow['id'] . '">' . $myrow['descripcion'];
	    } else {
		    echo '<OPTION VALUE="'. $myrow['id'] . '">' . $myrow['descripcion'];
    	}
...
echo '</select></td></tr>';
  ...
  echo '<input type="hidden" name="rh_gamma" value="0" />';
}

$SQL="select * from rh_familia group by categoria asc";
$resCate=DB_query($SQL,$db);

while ($Categoria=DB_fetch_assoc($resCate)){
echo '<TR>'
    . '<TD>' . _('Categoria').' '.htmlentities($Categoria['categoria']) . ':</TD>'
    . '<TD>';

	$SQL="select * from rh_familia where categoria='".DB_escape_string($Categoria['categoria'])."' order by nombre ASC ";
	$res=DB_query($SQL,$db);
	echo '<select name=rh_familia['.htmlentities($Categoria['categoria']).'] >';
	echo '<option value="">'._('Sin Categoria').' '.htmlentities($Categoria['categoria']) .'</option>';
	while($FilaAg=DB_fetch_assoc($res)){
		echo '<option value="'.$FilaAg['clave'].'" ';
		if($_REQUEST['rh_familia'][$Categoria['categoria']]==$FilaAg['clave'])
			echo ' selected=selected ';
		echo '>';
		echo htmlentities('( '.$FilaAg['clave'].' )'.$FilaAg['nombre']);
		echo '</option>';
...
// bowikaxu realhost - 18 july 2008 - lowest production quantity
...
echo '<TR><TD>' . _('Lowest Level') . ':</TD><TD><input type="Text" name="LowestLevel" SIZE=12 MAXLENGTH=10 Value="' . $_POST['LowestLevel'] . '"></TD></TR>';

// bowikaxu realhost - 18 july 2008 - lowest production quantity
echo '<TR><TD>' . _('Production') . ' ' . _('Lowest Level') . ':</TD><TD><input type="Text" name="rh_lowestprod" SIZE=12 MAXLENGTH=10 Value="' . $_POST['rh_lowestprod'] . '"></TD></TR>';
...
echo '</SELECT></TD></TR>';
echo '<TR>
        <TD>' . _('Cantidad por Unidad de Medida') . ':</TD>
        <TD><input ' . (in_array('rh_cantidadporunidad',$Errors) ?  'class="inputerror"' : '' ) .'   type="Text" name="rh_cantidadporunidad" SIZE=12 MAXLENGTH=10 value="' . $_POST['rh_cantidadporunidad'] . '"></TD>
      </TR>';
...
//iJPe 2010-04-16 Se quito este tipo de articulo
// bowikaxu realhost 15 july 2008 - Ensamblado costo por componente
//if ($_POST['MBFlag']=='E'){
//	echo '<OPTION SELECTED VALUE="E">' . _('Assembly').' '._('Costo por Componente');
//} else {
//	echo '<OPTION VALUE="E">' . _('Assembly').' '._('Costo por Componente');
//}
// fin bowikaxu
if ($_POST['MBFlag']=='K'){
...
//iJPe 2010-04-16 Se quito este tipo de articulo
//// bowikaxu realhost Feb 2008 - Articulos de Tipo Manufacturado Costo por Componente
//if ($_POST['MBFlag']=='C'){
//	echo '<OPTION SELECTED VALUE="C">' . _('Manufactured').' '._('Costo por Componente');
//} else {
//	echo '<OPTION VALUE="C">' . _('Manufactured').' '._('Costo por Componente');
//}
// bowikaxu end Costo por Componente
echo '</SELECT></TD></TR>';
...
echo '<input type="hidden" name="factorVenta" value="1" />';
//*********************************************************************************************************
//***             						FACTOR DE VENTA
//*********************************************************************************************************
/*echo '<TR><TD>' . _('Factor de Venta') . ':</TD><TD><SELECT NAME="factorVenta">';
$sql = 'SELECT id, name FROM rh_sales_factors ORDER BY id';
$result = DB_query($sql, $db);

if (!isset($_POST['factorVenta'])){
	$_POST['factorVenta'] = 1;
}

while ($myrow = DB_fetch_array($result)) {
	if ($_POST['factorVenta'] == $myrow['id']){
		echo '<OPTION SELECTED VALUE=' . $myrow['id'] . '>' . $myrow['name'];
	} else {
...
if (isset($New)) {
	...
	echo '<input type="submit" name="submit2" value="' . _('Update') . '">';
	echo '<input type="submit" name="UpdateCategories" style="visibility:hidden;width:1px" value="' . _('Categories') . '">';
	echo '<P>';
