...
/**
 * REALHOST 2008
 * $LastChangedDate: 2008-09-12 09:01:37 -0500 (Fri, 12 Sep 2008) $
 * $Rev: 399 $
 */
...
//Realhost
//17-Ago-2009
//Comente esta linea para que no se imprima la informacion de la sesion.
//print_r($_SESSION);
if (!isset($_GET['OrderNumber']) && !isset($_SESSION['ProcessingOrder'])) {
	/* This page can only be called with an order number for invoicing*/
	echo '<CENTER><A HREF="' . $rootpath . '/rh_SelectSalesOrder_Shipment_NC.php?' . SID . '">' . _('Seleccionar otro pedido para crear nota de cargo'). '</A></CENTER>';
	echo '<P><BR><BR>';
...
//Realhost
//17-Ago-2009
//Quite el echo para que no se imprima el query.
	 /*echo*/ $OrderHeaderSQL = 'SELECT salesorders.orderno,
	...
	$GetOrdHdrResult = DB_query($OrderHeaderSQL,$db,$ErrMsg,$DbgMsg,true);
	...
	if (DB_num_rows($GetOrdHdrResult)==1) {
		...
		if($_GET['type']=='1'){ // remision grande
			$sql = "SELECT rh_maxitmrem as Max FROM locations WHERE loccode ='".$_SESSION['Items']->Location."'";
		}else { // remision punto de venta
			$sql = "SELECT rh_maxitmpos as Max FROM locations WHERE loccode ='".$_SESSION['Items']->Location."'";
		}
		...
		if (db_num_rows($LineItemsResult)>0) {
			...
			while ($myrow=db_fetch_array($LineItemsResult)) {
				...
				// bowikaxu realhost - 9 july 2007 - check the max. items in a remision
				if(count($_SESSION['Items']->LineItems) >= $ItmMax['Max']){
					echo "<H2>Imposible agregar el articulo ".$myrow['stkcode']." se llego al limite de articulos</H2>";
				}else {
			...
			echo '<CENTER><A HREF="'. $rootpath. '/rh_SelectSalesOrder_Shipment_NC.php?' . SID . '">' ._('Seleccionar un pedido diferente para crear nota de cargo') .'</A></CENTER>';
			echo '<P>';
			prnMsg( _('There are no ordered items with a quantity left to deliver. There is nothing left to shipment'));
			include('includes/footer.inc');
			exit;
	...
	*/
	...
	foreach ($_SESSION['Items']->LineItems as $Itm) {
		...
		//iJPe
		//realhost		2010-01-11
		//Modificacion pedida por mangueras, aunque no sirve de nada este porcentaje de IVA
...
//Realhost
//17-Ago-2009
//Quite la linea para que no se imprima la informacion de la session.
	//print_r($_SESSION['Items']);

echo '<CENTER><FONT SIZE=4><B><U>' . $_SESSION['Items']->CustomerName . '</U></B></FONT><FONT SIZE=3> - ' .
_('Shipment amounts stated in') . ' ' . $_SESSION['Items']->DefaultCurrency . '</CENTER>';
...
//<TD class="tableheader">' . _('Tax %') . '</TD>
//<TD class="tableheader">' . _('Tax') . '<BR>' . _('Amount') . '</TD>
//'<TD class="tableheader">' . _('Total') . '<BR>' . _('Sin Tax') . '</TD>
'</TR>';
...
//$LineTotalSINIVA = $LnItm->QtyDispatched * $LnItm->Price ;
	...
	if ($LnItm->Controlled==1){
		...
		//echo '<TD ALIGN=RIGHT><input type=hidden name="' . $LnItm->LineNumber . '_QtyDispatched"  value="' . $LnItm->QtyDispatched . '"><a href="' . $rootpath .'/rh_ConfirmDispatchControlled_Shipment.php?' . SID . '&LineNo='. $LnItm->LineNumber.'">' .$LnItm->QtyDispatched . '</a></TD>';

	} else {
		// bowikaxu - corregir escoger menos cantidad del pedido a remisionar
...
//03-Ago-2009
	...
	$DisplayLineNetTotalSINIVA = number_format($LineTotalSINIVA,2);
...
//<TD ALIGN=RIGHT>'.$DisplayLineNetTotalSINIVA.'</TD>';
...
$_SESSION['Items']->total = $_SESSION['Items']->total;
...
if (isset($_POST['ProcessInvoice']) && $_POST['ProcessInvoice'] != ""){
	...
	if (DB_num_rows($Result) != count($_SESSION['Items']->LineItems)){
		...
		echo '<P>';
		prnMsg( _('This order has been changed or shipment since this delivery was started to be confirmed') . '. ' . _('Processing halted') . '. ' . _('To enter and confirm this dispatch') . '/' . _('shipment the order must be re-selected and re-read again to update the changes made by the other user'), 'error');
		echo '<BR>';

		echo '<CENTER><A HREF="'. $rootpath.'/rh_SelectSalesOrder_Shipment_NC.php?' . SID . '">'. _('Seleccionar un pedido para crear nota de cargo'). '</A></CENTER>';
	...
	while ($myrow = DB_fetch_array($Result)) {
		...
		if ($_SESSION['Items']->LineItems[$myrow['orderlineno']]->Quantity != $myrow['quantity'] OR $_SESSION['Items']->LineItems[$myrow['orderlineno']]->QtyInv != $myrow['qtyinvoiced']) {
			...
			$myrow['quantity'] . ' '. _('and an shipment qty of'). ' ' . $myrow['qtyinvoiced'] . ' '.
			_('the session shows quantity of'). ' ' . $_SESSION['Items']->LineItems[$myrow['orderlineno']]->Quantity .
			' ' . _('and quantity shipment of'). ' ' . $_SESSION['Items']->LineItems[$myrow['orderlineno']]->QtyInv;

			prnMsg( _('This order has been changed or shipment since this delivery was started to be confirmed') . ' ' . _('Processing halted.') . ' ' . _('To enter and confirm this dispatch, it must be re-selected and re-read again to update the changes made by the other user'), 'error');
			echo '<BR>';
	...
	$SQL = "BEGIN";
	$Result = DB_query($SQL,$db,'ERROR: Imposible comenzar transacciones con al base de datos','FAllo el QUERY: BEGIN',true);
	...
	$SQL = "UPDATE salesorders SET comments = CONCAT(salesorders.comments,' Rem ','" . $InvoiceNo . "') WHERE orderno= " . $_SESSION['ProcessingOrder'];
	...
	*/
	...
	foreach ($_SESSION['Items']->LineItems as $OrderLine) {
		...
		if ($OrderLine->QtyDispatched !=0 AND $OrderLine->QtyDispatched!="" AND $OrderLine->QtyDispatched) {
			...
			// andres amaya - insert a rh_remdetails
                         ...
                         * 31-Agosto-2009
                         *
                         * Se agrego el campo line para insertar en el regitro el numero de linea del
                         * producto en el pedido, para poder visualizar los productos en el orden en
	...
	$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('No se pudo obtener el valor de la ultima nota de cargo');
	$DbgMsg = _('The following SQL to insert the GLTrans record was used');
	...
	$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('No se pudieron insertar los datos de la nota de cargo');
	...
	$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('No se pudo actualizar la cantidad de Notas de Cargo en systypes');
	$DbgMsg = _('Error al actualizar systypes');
	$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);

	$SQL='COMMIT';
	$Result = DB_query($SQL,$db,'Fallo Insertar todos los cambios','Fallo el Query: COMMIT',true);

	unset($_SESSION['Items']->LineItems);
	unset($_SESSION['Items']);
	unset($_SESSION['ProcessingOrder']);

	echo "<br><br>".$_SESSIONE['Items']->total."<br><br>";
	echo _('Shipment number'). ' '. $InvoiceNo .' '. _('processed'). '<BR>';
	// bowikaxu realhost - print links depending on sale
	echo '<A HREF="'.$rootpath.'/rh_PDFRemGde.php?' . SID . 'FromTransNo='.$InvoiceNo.'&InvOrCredit=Invoice&PrintPDF=True&nc=1">'. _('Imprimir Nota de Cargo'). '</A><BR>';
	//echo '<A HREF="'.$rootpath.'/rh_TicketPOS.php?' . SID . 'FromTransNo='.$InvoiceNo.'&InvOrCredit=Invoice&PrintPDF=True">'. _('Imprimir Remision Chica'). '</A><BR>';

	echo '<A HREF="'.$rootpath.'/rh_SelectSalesOrder_Shipment_NC.php?' . SID . '">'. _('Seleccionar otro pedido para crear nota de cargo'). '</A><BR>';
	...
	// bowikaxu - tipo de remision
	echo "<TR>
		<TD>" . _('Tipo de Nota de Cargo'). ":</TD>
		<TD><SELECT NAME='RemType'>
		
			<OPTION SELECTED VALUE='0'>"._('Nota de Cargo Normal')."</OPTION>	
			<OPTION VALUE='1'>"._('Muestra')."</OPTION>
		
		</TD>
	</TR>";
	// fin de tipo de remision
	echo '<TR>
	...
	echo '<INPUT TYPE=SUBMIT NAME="ProcessInvoice" Value="'._('Process Shipment').'"</CENTER>';
