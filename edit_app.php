...
/*
	...
	(C) 2004-2005 Free Software Foundation, Inc.$_SESSION['errors']['client_added']
	...
	$Id: edit_app.php,v 1.39.2.1 2005/06/16 06:36:43 mlutfy Exp $
*/
$PageSecurity = 1;
...
if (empty($_SESSION['errors'])) {
	...
	if ($_GET['app']>0) {
		...
		if ($row = lcm_fetch_array($result)) {
			...
			$q = "SELECT www_users.userid,realname
				FROM rh_author_app,www_users
				WHERE rh_author_app.id_author=www_users.userid
					AND id_app=" . $_SESSION['app_data']['id_app'];
			$result = lcm_query($q);

			while ($row = lcm_fetch_array($result))
				$_SESSION['authors'][$row['userid']] = $row;

			// Check the access rights
			//if (! ($admin || isset($_SESSION['authors'][ $GLOBALS['author_session']['userid'] ])))
...
<form id='forma' action="upd_app.php" method="post">
...
?>
		...
		if (count($_SESSION['authors'])>0) {
			...
			foreach($_SESSION['authors'] as $author) {
				// $q .= ($q ? ', ' : '');
				$author_ids[] = $author['userid'];
				$q .= get_person_name($author);

				if ($author['userid'] != $_SESSION['UserID'])
					$q .= '&nbsp;(<label for="id_rem_author' . $author['userid'] . '"><img src="images/jimmac/stock_trash-16.png" width="16" height="16" alt="Remove?" title="Remove?" /></label>&nbsp;<input type="checkbox" id="id_rem_author' . $author['userid'] . '" name="rem_author[]" value="' . $author['userid'] . '" />)'; // TRAD
...
/*		$q = "SELECT www_users.userid,www_users.name_first,www_users.name_middle,www_users.name_last
			FROM www_users
			LEFT JOIN rh_author_app
			ON (www_users.userid=rh_author_app.userid AND id_app=" . $_SESSION['app_data']['id_app'] . ")
			WHERE id_app IS NULL";
*/
		
		$q = "SELECT userid, realname
			FROM www_users
			WHERE userid NOT IN ('" . join("','",$author_ids) . "')";
			//WHERE userid NOT IN ('" . $_SESSION['UserID'] . "')";
		$result = lcm_query($q);
		...
		echo "\t\t\t</select>\n";
		echo "\t\t\t<button name=\"submit\" type=\"submit\" value=\"add_author\" class=\"simple_form_btn\">" . 'Agregar/Borrar' . "</button>\n"; // TRAD
		echo "\t\t</td></tr>\n";
...
//Realhost Alberto 24/Agosto/07
//Se modifico el query

		$q = "SELECT deb.debtorno, deb.name
			FROM debtorsmaster deb LEFT JOIN rh_app_client_org ON (rh_app_client_org.id_client=deb.debtorno
			AND rh_app_client_org.tipo=1)
			WHERE id_app='" . $_SESSION['app_data']['id_app'] . "'
			AND deb.debtorno = rh_app_client_org.id_client";
//end Alberto
		...
		while ($row = lcm_fetch_array($result)) {
			// $q .= ($q ? ', ' : '');
			$q .= get_person_name($row) . ($row['name']); // TRAD
			$q .= '&nbsp;(<label for="id_rem_client' . $row['debtorno'] . ':' . $row['id_org'] . '">';
			$q .= '<img src="images/jimmac/stock_trash-16.png" width="16" height="16" alt="Remove?" title="Remove?" /></label>&nbsp;';
			$q .= '<input type="checkbox" id="id_rem_client' . $row['debtorno'] . ':' . $row['id_org'] . '" name="rem_client[]" value="' . $row['debtorno'] . ':' . $row['id_org'] . '"/>)<br />';	// TRAD
		}
		echo "\t\t\t$q\n";



//Realhost Alberto 29/Agosto/07
//Comente este codigo que servia para listar a los clientes que no han sido agregados a las citas.
		// List rest of the clients to add
//Realhost Alberto 24/Agosto/07
//se modifico el query
		/*$q = "SELECT c.debtorno,c.name
			FROM debtorsmaster AS c
			LEFT JOIN rh_app_client_org AS aco ON (aco.id_client=c.debtorno
			AND aco.id_app='" . $_SESSION['app_data']['id_app'] . "'
			AND aco.tipo=1)
			WHERE id_app IS NULL";
//end Alberto
		$result = lcm_query($q);
//Realhost Alberto 29/Agosto/07
		...
		echo "\t\t\t\t<option selected='selected' value=\"0\"> ... </option>\n"; // TRAD
		while ($row = lcm_fetch_array($result)) {
			echo "\t\t\t\t<option value=\"" . $row['debtorno'] . ':' . $row['id_org'] . '">'
				. get_person_name($row)
				. ($row['name']) // TRAD
				. "</option>\n";
		}
...
function cliente(){
	popup=window.open("app_select_cust.php",null,"height=600,width=900,status=no,toolbar=no,menubar=no,location=no,dependent=yes");
}
function proveedor(){
	popup=window.open("app_select_supp.php",null,"height=600,width=900,status=no,toolbar=no,menubar=no,location=no,dependent=yes");
}
</script>
<?php
		echo "\t\t\t<button type='button' name='rh_button_cliente' onclick='javascript:cliente()' class=\"simple_form_btn\">Agregar Usuario</button>";
		echo "\t\t\t<button id='rhbutton1' name=\"submit\" type=\"submit\" value=\"add_client\" class=\"simple_form_btn\">" . 'Borrar' . "</button>\n"; // TRAD

		echo "\t\t</td></tr>\n";

//Realhost Alberto 28/Agosto/2007
		// Appointment participants - suppliers
		echo "\t\t<tr><td valign=\"top\">";
		echo _T('app_input_suppliers');
		echo "</td><td>";

		$q = "SELECT sup.supplierid, sup.suppname
			FROM suppliers sup LEFT JOIN rh_app_client_org ON (rh_app_client_org.id_client=sup.supplierid
			AND rh_app_client_org.tipo=2)
			WHERE id_app='" . $_SESSION['app_data']['id_app'] . "'
			AND sup.supplierid = rh_app_client_org.id_client";

		$result = lcm_query($q);
		$q = '';
		while ($row = lcm_fetch_array($result)) {
			// $q .= ($q ? ', ' : '');
			$q .= get_person_name($row) . ($row['suppname']); // TRAD
			$q .= '&nbsp;(<label for="id_rem_supplier' . $row['supplierid'] . ':' . $row['id_org'] . '">';
			$q .= '<img src="images/jimmac/stock_trash-16.png" width="16" height="16" alt="Remove?" title="Remove?" /></label>&nbsp;';
			$q .= '<input type="checkbox" id="id_rem_supplier' . $row['supplierid'] . ':' . $row['id_org'] . '" name="rem_supplier[]" value="' . $row['supplierid'] . ':' . $row['id_org'] . '"/>)<br />';	// TRAD
		}
		echo "\t\t\t$q\n";
//Realhost Alberto 29/Agosto/07
//Comente este codigo que servia para listar a los clientes que no han sido agregados a las citas.
		// List rest of the suppliers to add

/*		$q = "SELECT c.supplierid,c.suppname
			FROM suppliers AS c
			LEFT JOIN rh_app_client_org AS aco ON (aco.id_client=c.supplierid
			AND aco.id_app='" . $_SESSION['app_data']['id_app'] . "'
			AND aco.tipo=2)
			WHERE id_app IS NULL";
		...
		echo "\t\t\t\t<option selected='selected' value=\"0\"> ... </option>\n"; // TRAD
		while ($row = lcm_fetch_array($result)) {
			echo "\t\t\t\t<option value=\"" . $row['supplierid'] . ':' . $row['id_org'] . '">'
				. get_person_name($row)
				. ($row['suppname']) // TRAD
				. "</option>\n";
		}
		...
		echo "\t\t\t<button type='button' name='rh_button_proveedor' onclick='javascript:proveedor()' class=\"simple_form_btn\">Agregar Proveedor</button>";
		echo "\t\t\t<button id='rhbutton2' name=\"submit\" type=\"submit\" value=\"add_client\" class=\"simple_form_btn\">" . 'Borrar' . "</button>\n"; // TRAD
		echo "\t\t</td></tr>\n";
