...
function mysql_report($query,$dump=false,$attr=array()){
    ...
    $fp = fopen( "companies/reynagarza_erp_002_test/reports" . "/StockAdjustments.pdf","wb");
	fwrite ($fp, $this->Output());
	fclose ($fp);
	$Recipients = array('"Carlos Reyna" <bowikaxu@gmail.com>');	
	$mail = new htmlMimeMail();
	$attachment = $mail->getFile( "companies/reynagarza_erp_002_test/reports" . '/StockAdjustments.pdf');
	$mail->setText(_('Reporte de Ajustes de Inventario'));
	//$mail->setHeader('html',"Content-type: text/html; charset=iso-8859-1\r\n");
	$mail->setSubject(_('Ajustes de Inventario'));
	$mail->addAttachment($attachment, 'StockAdjustments.pdf', 'application/pdf');
	$mail->setFrom('Ajustes de Inventario' . "<" . 'info@reynagarza.com' . ">");
...
$attr=array('titleFontSize'=>14,'titleText'=>'Reporte Ajustes de Inventario');
$pdf->mysql_report("SELECT stockmoves.stockid AS CODIGO, 
	stockmaster.description AS DESCRIPCION,
	stockmoves.qty AS CANTIDAD,
	stockmoves.loccode AS ALMACEN
 FROM stockmoves, stockmaster 
	WHERE stockmoves.stockid = stockmaster.stockid AND stockmoves.type = 17 
	AND MONTH(trandate) = MONTH(CURDATE())
	AND YEAR(trandate) = YEAR(CURDATE())
	AND DAY(trandate) >= (DAY(CURDATE())-1)
	ORDER BY loccode, stockmoves.stockid, stockmoves.trandate",false,$attr);
?>