...
function ordenar(q){
	...
	var url = "rh_sustanciaactiva_ope.php";
...
function edita(id){
	...
	xmlhttp.onreadystatechange = function(){
		if(xmlhttp.readyState == 4){//cuando la respuesta llegue
			...
			document.forma2.nombre.value = mi_array[1];
		}
	}
	xmlhttp.open("post", url, true);
	xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
...
function graba(){
	...
	if(error == 1){
		xmlhttp = create_ajaxOb();
		var url = "rh_sustanciaactiva_ope.php";
		var posts = "rh_modo="+modo+"&id="+id+"&nombre="+document.forma2.nombre.value+"&pagina="+document.forma.actulpagina.value+"&ordenado="+document.forma.ordenado.value+"&ad="+document.forma.ad.value+"&tpagina="+document.forma.totalpagina.value+"&buscacion="+document.forma.buscacion.value;
		xmlhttp.onreadystatechange = function(){
			if(xmlhttp.readyState == 4){//cuando la respuesta llegue
				respuesta = xmlhttp.responseText;
				mi_array = new Array();
...
function borra(id){
	document.getElementById('msn').innerHTML = "";
	if(confirm('Esta seguro de borrar esta Sustancia Activa?')){
		xmlhttp = create_ajaxOb();
		var url = "rh_sustanciaactiva_ope.php";
