...
if($_POST['rh_modo'] == 'PAGINA'){
	...
	$sqlcount = "SELECT COUNT(descripcion) AS paginas FROM rh_especie WHERE descripcion LIKE '%".$_POST['buscacion']."%'";
	...
	$sql = "SELECT * FROM rh_especie WHERE descripcion LIKE '%".$_POST['buscacion']."%' ORDER BY ".$_POST['ordenado']." ".$_POST['ad']." LIMIT ".(($_POST['pagina']-1)*20).",20";
...
if($_POST['rh_modo'] == 'EDITA'){
	...
	include('includes/session.inc');
	include('includes/SQL_CommonFunctions.inc');
	$sql = "SELECT * FROM rh_especie WHERE id = ".$_POST['id']."";
	$result = DB_query($sql,$db);
...
if($_POST['rh_modo'] == 'GRABA'){
	...
	$checkSql = "SELECT count(*) FROM rh_especie WHERE codigo = '".str_replace(' ','',strtoupper($_POST['codigo']))."'";
	...
	if ($checkrow[0] > 0) {
		...
		$sql = "INSERT INTO rh_especie (codigo,descripcion) VALUES ('".str_replace(' ','',strtoupper($_POST['codigo']))."','".$_POST['nombre']."')";
		$result = DB_query($sql,$db);
	}
	echo "||<FORM name='forma'>
	<input type='hidden' id='actulpagina' value='".$_POST['pagina']."'><input type='hidden' id='totalpagina' value='".$_POST['tpagina']."'><input type='hidden' id='ordenado' value='".$_POST['ordenado']."'><input type='hidden' id='ad' value='".$_POST['ad']."'>
...
if($_POST['rh_modo'] == 'ACTUALIZA'){
	...
	$sql = "UPDATE rh_especie SET descripcion = '".$_POST['nombre']."' WHERE id = '".$_POST['id']."'";
	$result = DB_query($sql,$db);
	echo "||<FORM name='forma'>
	<input type='hidden' id='actulpagina' value='".$_POST['pagina']."'><input type='hidden' id='totalpagina' value='".$_POST['tpagina']."'><input type='hidden' id='ordenado' value='".$_POST['ordenado']."'><input type='hidden' id='ad' value='".$_POST['ad']."'>
	<input type='text' id='buscacion' value='".$_POST['buscacion']."'><INPUT TYPE='button' VALUE='Buscar' onClick=buscararticulo()>
...
if($_POST['rh_modo'] == 'BORRA'){
	...
	$sql= "SELECT COUNT(*) FROM rh_especie_stock WHERE idEspecie = '".$_POST['id']."'";
	...
	if ($myrow[0]>0) {
		...
		$sql2 = "DELETE FROM rh_especie WHERE id = '".$_POST['id']."'";
