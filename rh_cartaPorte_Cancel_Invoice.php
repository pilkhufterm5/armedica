 ...
 */
//Jaime, hardcode de los usuarios que pueden cancelar facturas
...
// bowikaxu realhost - get the ivoice period
	$InvoiceHeaderSQL = "SELECT DISTINCT
			...
			WHERE debtortrans.transno = " . $_GET['InvoiceNumber'] . "
				...
				AND debtortrans.type=20002
				AND debtortrans.alloc=0
				AND stockmoves.type=20002";
...
/*Start an SQL transaction */
	...
	$SQL = "SELECT (ovamount+ovgst+ovfreight-ovdiscount-alloc) as baltoallocate
		FROM debtortrans
		WHERE transno=" . $_SESSION['ProcessingCredit'] . " AND type=20002";
	$Result = DB_query($SQL,$db);
	$myrow = DB_fetch_row($Result);
 ...
 */
	...
	//TODO bowikaxu - insert el usuario de cancelacion a rh_usertrans.
        ...
        try {
            $sqlQuery = "select id_ws_cfd from rh_cfd__cfd where fk_transno = " . $_SESSION['ProcessingCredit'] . " and id_systypes = 20002 limit 1";
	...
	if ($_SESSION['InvoicePortraitFormat']==0){
	    echo "<BR><A target='_blank' HREF='". $rootpath . "/rh_printFE.php?" . SID . "&transno=" . $_SESSION['ProcessingCredit'] . "&isCfdCancelado=true&isCartaPorte=true' >" . _('Imprimir Cancelacion') . '</A>';
...
/*if the credit note is a return of goods then need to know which location to receive them into */
		...
		echo '<TR><TD>' . _('Acepta los terminos de cancelaci&oacute;n') . '</TD><TD><SELECT NAME=AcceptTerms onchange="acceptterms(this.value)">';
	...
	// DISABLED=true
	echo '<TR><TD>' . _('Razones de Cancelacion') . '</TD><TD><TEXTAREA NAME=CreditText COLS=31 ROWS=5>' . $_POST['CreditText'] . '</TEXTAREA></TD></TR>';
