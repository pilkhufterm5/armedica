...
while ($WORow = DB_fetch_array($WOItemsResult)){
	...
	// bowikaxu reahost Feb 2008 - save the order item
	$rh_woitm = $WORow['stockid'];
...
// bowikaxu realhost Feb 2008 - select qtypu to chose if the item is Fixed Qty
$RequirementsResult = DB_query("SELECT worequirements.stockid,
				stockmaster.description,
				stockmaster.decimalplaces,
				worequirements.stdcost,
				worequirements.qtypu AS fixed_requiredqty,
				...
				SUM(worequirements.stdcost*worequirements.qtypu) AS fixed_expectedcost
...
while ($RequirementsRow = DB_fetch_array($RequirementsResult)){
	...
	// bowikaxu realhost TODO - que hacer al recibir parcialmente una orden de produccion.
	...
	// bowikaxu realhost Feb 2008 - get fixed or variable type
	$sql = "SELECT rh_type FROM bom WHERE component = '".$RequirementsRow['stockid']."' AND parent = '".$rh_woitm."'";
			$rh_typeres = DB_query($sql,$db);
			$rh_type = DB_fetch_array($rh_typeres);
	if($rh_type['rh_type']==0){ // variable
	...
	//$UsageVar =(-$IssueQty)*$RequirementsRow['stdcost'];
		$UsageVar =($RequirementsRow['fixed_requiredqty']-$IssueQty)*$RequirementsRow['stdcost'];
	
		echo '<td colspan="2"></td><td align="right">'  . number_format($RequirementsRow['fixed_requiredqty'],$RequirementsRow['decimalplaces']) . '('._('fixed'). ')' . '</td>
				<td align="right">' . number_format($RequirementsRow['fixed_expectedcost'],2) . '</td>
				<td></td>
				<td align="right">' . number_format($IssueQty,$RequirementsRow['decimalplaces']) . '</td>
				<td align="right">' . number_format($IssueCost,2) . '</td>
				<td align="right">' . number_format($UsageVar,2) . '</td>
		...
		if ($TotalStdValueRecd==0){
 		...
 		if ($_SESSION['WeightedAverageCosting']==1){ 
			...
			*/
				...
				if ($ProportionOnHand < 1){
					...
					$SQL = "INSERT INTO rh_wogl (wo,
							...
							VALUES (".$_POST['WO'].",
								...
								" . $WOCloseNo . ",
								" . (-$TotalVariance*$ShareProportion*(1-$ProportionOnHand)) . ",
								'" . DB_escape_string($_POST['WO'] . ' - ' . $WORow['stockid'] . ' ' . _('share of variance')) . "')";
			...
			// bowikaxu realhost March 2008 - dividir el precio entre la cantidad
				...
				// bowikaxu realhost March 2008 - Insert a la Relacion WoGl
					$SQL = "INSERT INTO rh_wogl (wo,
							...
							VALUES (".$_POST['WO'].",
								...
								" . $WOCloseNo . ",
								" . ($TotalCostVar*$ShareProportion) . ",
								'" . DB_escape_string($_POST['WO'] . ' - ' . $WORow['stockid'] . ' ' . _('share of cost variance')) . "')";
					$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' .  _('The GL rh_wogl');
					$DbgMsg = _('The following SQL to insert the GLTrans record was used');
					$Result = DB_query($SQL,$db,$ErrMsg,$DbgMsg,true);
