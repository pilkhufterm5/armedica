...
/*Now check that the new code doesn't already exist */
	 ...
	 * rleal
	 * May 13, 2011
	 * Se agregan otros updates 
	 */
	
	echo '<BR>' . _('Changing the rh_costhistory table records');
	$sql = "UPDATE rh_costhistory SET stockid='" . $_POST['NewStockID'] . "' WHERE stockid='" . $_POST['OldStockID'] . "'";
	$ErrMsg =  _('The SQL to update rh_costhistory records failed');
	$result = DB_query($sql,$db,$ErrMsg,$DbgMsg,true);
	echo ' ... ' . _('completed');
	
	echo '<BR>' . _('Changing the rh_especie_stock table records');
	$sql = "UPDATE rh_especie_stock SET stockid='" . $_POST['NewStockID'] . "' WHERE stockid='" . $_POST['OldStockID'] . "'";
	$ErrMsg =  _('The SQL to update rh_especie_stock records failed');
	$result = DB_query($sql,$db,$ErrMsg,$DbgMsg,true);
	echo ' ... ' . _('completed');
	
	echo '<BR>' . _('Changing the rh_gamma_stock table records');
	$sql = "UPDATE rh_gamma_stock SET stockid='" . $_POST['NewStockID'] . "' WHERE stockid='" . $_POST['OldStockID'] . "'";
	$ErrMsg =  _('The SQL to update rh_gamma_stock records failed');
	$result = DB_query($sql,$db,$ErrMsg,$DbgMsg,true);
	echo ' ... ' . _('completed');
