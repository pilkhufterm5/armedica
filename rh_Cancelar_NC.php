...
$title = _('Cancelar una Nota de Cargo');
...
// bowikaxu realhost - get the ivoice period
	$InvoiceHeaderSQL = "SELECT DISTINCT
			...
			WHERE debtortrans.transno = " . $_GET['InvoiceNumber'] . "
				...
				AND debtortrans.type=20001
				AND debtortrans.alloc=0
				AND stockmoves.type=20001";
...
/*Start an SQL transaction */
	...
	$SQL = "SELECT (ovamount+ovgst+ovfreight-ovdiscount-alloc) as baltoallocate
		FROM debtortrans
		WHERE transno=" . $_SESSION['ProcessingCredit'] . " AND type=20001";
	$Result = DB_query($SQL,$db);
	$myrow = DB_fetch_row($Result);
...
/*Now need to update the invoice DebtorTrans record for the amount to be allocated and if the invoice is now settled*/
	...
	$SQL = "UPDATE debtortrans SET
		...
		$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('El Status de la Nota de Cargo no puede ser modificado');
		$DbgMsg = _('The following SQL to update the invoice allocation was used');
		$Result = DB_query($SQL,$db,$ErrMsg,DbgMsg,true);
/*Now insert the Cancelacion into the DebtorTrans table with the allocations as calculated above*/
	...
	$SQL = "INSERT INTO debtortrans (transno,
		...
		VALUES (". $CreditNo . ",
			...
			'" . $DefaultDispatchDate . "',
			" . $PeriodNo . ", 'Nota de Cargo-" . $_SESSION['ProcessingCredit'] . "-Usr-".$_SESSION['UserID']."',
...
/*Could do with setting up a more flexible freight posting schema that looks at the sales type and area of the customer branch to determine where to post the freight recovery */
	...
	$SQL = "UPDATE debtortrans SET
		reference = concat(reference, 'NC-".$CreditNo."-Usr-".$_SESSION['UserID']."')
		WHERE transno = " . $_SESSION['ProcessingCredit'] . "
		AND type=20001";

		$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('La Referencia de la Nota de Cargo no puede ser modificado');
		$DbgMsg = _('The following SQL to update the invoice allocation was used');
		$Result = DB_query($SQL,$db,$ErrMsg,DbgMsg,true);
