...
$PageSecurity = 6;
 ...
 /*Show a form to allow input of criteria for TB to show */
	...
	include_once 'php-ofc-library/open_flash_chart_object.php';
 	...
 	$g = new graph();
 	$g->title($GraphTitle, '{font-size: 12px;}' );
 	$g->set_x_legend(_('Month'));
	...
	if ($_POST['GraphValue']=='Net'){	
		...
		$g->set_y_legend(_('Sales Value'), 12, '#736AFF' );
	} elseif ($_POST['GraphValue']=='GP'){	
		//$graph->SetYTitle(_('Gross Profit'));
		$g->set_y_legend(_('Gross Profit'), 12, '#736AFF' );
	} else {
		//$graph->SetYTitle(_('Quantity'));
		$g->set_y_legend(_('Quantity'), 12, '#736AFF' );
	}
	...
	$bar_red->key( _('Budget'), 10 );
	
	$bar_blue = new bar_3d( 70, '#3334AD' );
	$bar_blue->key( _('Actual'), 10 );
	...
	while ($myrow = DB_fetch_array($SalesResult)){
		...
		$bar_red->data[] = $myrow['budget'];
		$bar_blue->data[] = $myrow['sales'];
		$x[] = MonthAndYearFromSQLDate($myrow['lastdate_in_period']);
		$i++;
	}
	
	//$graph->SetDataValues($GraphArray);
	//$g->set_data($GraphArray);
	$g->data_sets[] = $bar_red;
	$g->data_sets[] = $bar_blue;
	$g->set_x_labels($x);
	$g->set_x_label_style(10,'#000000',2,-1);
	$g->set_y_max(max($bar_blue->data));
	/*
	$graph->SetDataColors(
	...
	*/  
	...
	//$g->set_height( 200 );
	//echo $g->render();
	$handle = fopen("tmp/SalesReport.txt","w");
	fwrite($handle,$g->render());
	open_flash_chart_object( 950, 450, "tmp/SalesReport.txt",false); //
	include('includes/footer.inc');
 }
 ?>