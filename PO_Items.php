			        ...
			        if ($AlreadyOnThisOrder!=1 and $Quantity>0){
			        	...
			        	if($ItemCode!=''){
					        $purchdatasql="SELECT COUNT(supplierno)
									FROM purchdata
									WHERE purchdata.supplierno = '" . $_SESSION['PO'.$identifier]->SupplierID . "'
									AND purchdata.stockid='". DB_escape_string($ItemCode) . "'";
					        ...
					        if ($myrow[0]>0) {
						        $sql = "SELECT stockmaster.description,
						            ...
						            WHERE chartmaster.accountcode = stockcategory.stockact
							            ...
							            AND stockmaster.stockid = '". DB_escape_string($ItemCode) . "'
							            AND purchdata.effectivefrom =
								    (SELECT max(effectivefrom)
									        FROM purchdata
									    WHERE purchdata.stockid='". DB_escape_string($ItemCode) . "'
									        AND purchdata.supplierno='" . $_SESSION['PO'.$identifier]->SupplierID . "')";
					        } else {
						        $sql="SELECT stockmaster.description,
							                stockmaster.stockid,
				        	...
				        	$sql="SELECT 	'{$Descripcion}' description,
						                ...
						                WHERE chartmaster.accountcode = stockcategory.stockact
							                AND stockcategory.categoryid = stockmaster.categoryid
							                AND stockmaster.id_agrupador = '". DB_escape_string($id_agrupador) . "' limit 1";
				        }
...
if (!isset($_GET['Edit'])) {
  ...
  echo '<br>Estructura de importacion:<br><strong>stockid|Cantidad|Precio|impuesto|Fecha|Descripcion Agrupador</strong>';
  echo "</TD></TR>";
  echo "</table><br>
