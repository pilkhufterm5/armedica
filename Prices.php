...
if(isset($_REQUEST['utilidadsobre']) && is_double((double) trim($_REQUEST['utilidadsobre']) )  && $_REQUEST['utilidadsobre'] != ''){
		if($_REQUEST['utilidadsobre'] < 1 && $_REQUEST['utilidadsobre'] > 0){
			
			$porcentaje = $_REQUEST['utilidadsobre'] + 1 ;
		}elseif($_REQUEST['utilidadsobre'] >= 1){
			$porcentaje = $_REQUEST['utilidadsobre'] / 100 + 1;
			
		}
		
		//buscamos el costo del articulos.
		...
		WHERE stockmaster.stockid='{$_REQUEST['Item']}'
		...
		$costo = $myrow['materialcost']+$myrow['labourcost']+$myrow['overheadcost'];
		
		$_POST['Price'] = $costo * $porcentaje;
	...
	if ($InputError !=1){
		...
		// bowikaxu realhost - get old price
			$OldPrice = $_POST['OldPrice'];
			// bowikaxu realhost - historial del precio
			$sqlprice = "INSERT INTO rh_pricehistory (stockid,
					typeabbrev, currabrev, debtorno, price, lastprice, branchcode, trandate) VALUES (
					'".$Item."',
					'".$_POST['TypeAbbrev']."',
					'".$_POST['CurrAbrev']."',
					'',
		...
		// bowikaxu realhost january 2008 - insert price history
		if($_SESSION['PriceHistory']==1){
				DB_query($sqlprice,$db);
			}
	}
	prnMsg($msg);
...
if ($InputError ==0){
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		if (in_array(5,$_SESSION['AllowedPageSecurityTokens'])) {
			...
			// realhost - april 2007 - show 4 decimals instead of 2
			printf("<td>%s</td>
			        <td>%s</td>
	...
	if (isset($_GET['Edit'])){
		...
		// bowikaxu realhost - save old price
		echo '<INPUT TYPE=HIDDEN NAME="OldPrice" VALUE="' . $_GET['Price'] .'">';
	...
	<tr style="visibility:hidden; display:none;">
		<td> <?php echo _('% Utilidad Sobre Costo')?>:</td>
		<td><input type="text" name="utilidadsobre" /></td>
	</tr>
	</TABLE>
