<script type="text/javascript">
    ...
    function FacturarTimbrar(){
         ...
         var EmpresaPadre = $('#EmpresaPadre').val();
         
        if (confirm('¿Desea Facturar y Timbrar todas los Folios de la busqueda?')) {

            var jqxhr = $.ajax({
                url: "<?php echo $this->createUrl("facturacion/FacturaemisionMasiva1Concepto"); ?>",
                type: "POST",
                dataType : "json",
                timeout : (0),
                data: {
...
<div class="container-fluid">
    <form method="POST" action="<?php echo $this->createUrl("facturacion/emisionmasiva1concepto/"); ?>" >
        <h3>Emisión Masiva de Empresas con sucursales, solo 1 concepto</h3>
...
<!--                     <label style="margin-top: 10px;" >Tipo Factura:</label>
            ...
            <tr id="FProgramadas">
                ...
                <td>
                     <label style="margin-top: 10px;" >
                        Seleccione empresa padre:
                    </label>
                    <select name="EmpresaPadre" id="EmpresaPadre" class="required">
                        <?php
                        foreach($ListaEmpresasPadre as $rowempresas){
                            if($rowempresas['tipopersona']=='FISICA')
                            {
                                $NombreEmpresa  = $rowempresas['name'].' '. $rowempresas['apellidos'];
                            }
                            else
                            {
                                $NombreEmpresa  = $rowempresas['name'];
                            }
                            if(isset($_POST['EmpresaPadre']) and $_POST['EmpresaPadre']==$rowempresas['folio']){
                                echo "<option selected='selected' value='".$rowempresas['folio']."'>".$rowempresas['folio']." ".$NombreEmpresa."</option>";
                            }else{
                                echo "<option  value='".$rowempresas['folio']."'>".$rowempresas['folio']." ".$NombreEmpresa."</option>";
                            }
                            ?>
                        <?php } ?>                  
                    </select>
             ...
             foreach($DebtorData as $Data){ ?>
                <tr id="<?=$Data['id']?>" >
                    ...
                    <td style="text-align:center;">
                        <input type="checkbox" id="TimbrarFactura<?=$Data['id']?>" name="TimbrarFactura[]" value="<?=$Data['folio']?>" title="Generar Factura Si/NO"  class="success TimbrarFactura" checked />
                    </td>
                    <td id="actions<?=$Data['id']?>" style ="text-align:center;">

                    </td>
                </tr>
            <?php } // end  foreach($DebtorData as $Data)
            }//end  if(count($DebtorData)>0) 
            ?>
        </tbody>
    </table>
</div>
