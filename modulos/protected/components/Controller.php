...
class Controller extends CController
{
    ...
    public function init() {
        ...
        // FB::INFO($ServidoresdeCorreos,'____________$ServidoresdeCorreos');
    ...
    /**
     * @Todo
     * Obtiene el Tipo de Folio ('Cliente', 'Socio')
     * @author erasto@realhost.com.mx
     */
    public function GetTipoFolio($Folio) {

        $Tipo = Yii::app()->db->createCommand()
        ->select(' tipo_membresia ')->from('rh_foliosasignados')
        ->where('folio = :folio', array(':folio' => $Folio))->queryAll();
        return $Tipo[0]['tipo_membresia'];
    }
    ...
    /*Envia Email*/
    public function EnviarMail($from = 'gerardo.delangel@armedica.com.mx', $To = '', $Subject = '', $Mensaje = '', $adjuntos = array() , $BCC = '', $repplyTo = '', $AddCC = '') {

        global $db, $ServidoresdeCorreos;
...
/*
    *   METODO PARA VERIFICAR SI LE TOCA O NO FACTURAR
    *   POR DANIEL VILLARREAL EL 25 DE FEBRERO DEL 2016
    *   CambiarMesFacturacion: 1 = sumar 1 mes y 0 = fecha actual
    * Frecuencia: 1 = Mensual, 2 = Semestral, 3 = anual y 4 = anual insen
    *   DiaEmision: 
    */
    public function VerificarOrden($Frecuencia,$DiaEmision,$Date,$CambiarMesFacturacion,$folio,$amount,$pagoadelantado=0)
    {   
        // Obtenemos que tipo de frecuencia es
        
        $ObtenerFechaCorte = Yii::app()->db->createCommand()
      ->select("MAX(rh_facturacion.fecha_corte) as fecha_corte, rh_titular.fecha_ingreso, rh_titular.debtorno")
      ->from("rh_titular")
      ->leftJoin("rh_facturacion", "rh_titular.debtorno = rh_facturacion.debtorno")
      ->where("rh_titular.folio = :folio", array(':folio' => $folio))
      ->queryAll();
      if($ObtenerFechaCorte['0']['fecha_corte']!='')
      {
          $FechaNuevoCorte = $ObtenerFechaCorte['0']['fecha_corte'];
      }else{
          $FechaNuevoCorte = $ObtenerFechaCorte['0']['fecha_ingreso'];
      }
      // 0 = AÑO, 1 = MES Y 2 = DIA
      $FechaNuevoCorteArray = explode("-", $FechaNuevoCorte);
     
      $debtorno = $ObtenerFechaCorte['0']['debtorno'];
      // 0 = AÑO, 1 = MES Y 2 = DIA
      $FechaACompararArray = explode("-", $FechaAComparar);
      $FechaNuevoPedido = $FechaACompararArray[0].'-'.$FechaNuevoCorteArray[1].'-'.$DiaEmision;
      if($Frecuencia==1)
      { // El pedido sera el año de corte, el mes de corte y el dia de emision.
        $FechaNuevoPedido = $FechaACompararArray[0].'-'.$FechaACompararArray[1].'-'.$DiaEmision;
      }

      // Le sumamos 1 mes en base a mes actual o mes siguiente
      if($CambiarMesFacturacion==1){
          $FechaAComparar = strtotime ("+1 Months", strtotime ($Date));
          $FechaAComparar = date ( 'Y-m-d' , $FechaAComparar);
      }else{
          $FechaAComparar = $Date;
      }
      /*==============================
      TERMINA
      ================================*/
      // Si es pago adelantado, obtenemos la fecha que nos envian y comparamos
      if($pagoadelantado!=0 and $pagoadelantado!='')
      {
        //2018-02-01
        $FechaAComparar = $pagoadelantado;
      }
        
        // 0 = AÑO, 1 = MES Y 2 = DIA
        $FechaACompararArray = explode("-", $FechaAComparar);
        // Obtengo el año del nuevo pedido
        // 2018-
        $FechaNuevoPedido = $FechaACompararArray[0].'-'.$FechaNuevoCorteArray[1].'-'.$DiaEmision;
        if($Frecuencia==1) // frecuencia mensual
        {
            $FechaNuevoPedido = $FechaACompararArray[0].'-'.$FechaACompararArray[1].'-'.$DiaEmision;    
        }
    
        // Verificamos si exte un registro en rh facturacion uniedo debtortrans       
        $VerifyOrder2 = Yii::app()->db->createCommand()
        ->select("*")
        ->from("rh_facturacion")
        ->join("debtortrans", "debtortrans.id = rh_facturacion.debtortrans_id")
        ->where("rh_facturacion.debtorno = :debtorno AND rh_facturacion.fecha_corte = :fecha_corte AND debtortrans.rh_status != 'E' ", 
            array(':debtorno' => $debtorno,':fecha_corte' => $FechaAComparar))
        ->queryAll();
        ...
        ->from("rh_facturacion")
        ->join("debtortrans", "debtortrans.id = rh_facturacion.debtortrans_id")
        ->where("rh_facturacion.debtorno = :debtorno AND rh_facturacion.fecha_corte = :fecha_corte
          AND debtortrans.ovamount = :amount AND debtortrans.rh_status != 'E'", array(':debtorno' => $debtorno,':fecha_corte' => $FechaAComparar, ':amount' => $amount))
        ->queryAll();
        */
      // Se elimino la comprobacion de  AND salesorderdetails.unitprice = :amount debido a que en pagos adelantados no guardan el total unitario, , ':amount' => $amount.
        
      /*
      echo 'date:'.$Date.'<br>';
        echo 'folio:'.$folio.'<br>';
        echo 'amount:'.$amount.'<br>';
        echo 'debtorno:'.$debtorno.'<br>';
      print_r($VerifyOrder2);
      echo '<br>';
      echo $FechaAComparar;
      echo '<br>';
      echo $FechaNuevoPedido;      
        */

      if(empty($VerifyOrder2) and $FechaAComparar == $FechaNuevoPedido)
      {
        
        return array('respuesta'=>true,'fechapedido'=>$FechaNuevoPedido);
      }
      else
      {
        return array('respuesta'=>false,'FechaAComparar'=>$FechaAComparar,'FechaNuevoPedido'=>$FechaNuevoPedido);
      }
