...
// cliente
$pdf->addText(50, 280, $FontSize, $myrow['name'].' - '.$myrow['debtorno'] . ' ' . _('Sucursal') . ' ' . $myrow['branchcode']);
//$pdf->addText($Page_Width-268, $YPos-39, $FontSize, _('Date'));
$pdf->addText(428, 288, $FontSize, ConvertDate($myrow['trandate']));
...
//if ($myrow['invaddrbranch']==0){
	...
	$pdf->addText(50, 265, $FontSize, $myrow['address1'].' '.$myrow['address2']);
	$pdf->addText(50, 250, $FontSize, $myrow['address5'].' '.$myrow['address4']);
	$pdf->addText(50, 235, $FontSize, $myrow['taxref'].' - Vendedor(a): '.$myrow['salesmanname']);
