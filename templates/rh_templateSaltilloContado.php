...
$rh_fontsize=$rh_fontsize+2;
		while ($myrow2=DB_fetch_array($result)){
			...
			if ($rh_tipo==10)
			{
				...
				//Codigo
				$pdf->addTextWrap(45,$YPos,81,$rh_fontsize-2.5,$myrow2['stockid'], 'left');
			}
			else
			{
				//total
			...
			for($i = 0; $i < count($rh_arraydesc1); $i=$i + 1)
			{
				if(strlen($rh_arraydesc1[$i]) >= 115)
				{
					...
					while($nuevo >= 105)
					{
						...
						$pdf->addTextWrap(30,156-$i*10-1+32,340,$rh_fontsize-2,$nueva_cadena, 'left');
						$pdf->addTextWrap(30,156-$i*10-12+32,340,$rh_fontsize-2,$nueva_cadena2, 'left');
						$rh_sumar = 12;
					}
				}
				else
				$pdf->addTextWrap(30,156-$i*10-$rh_sumar+32,340,$rh_fontsize-2,$rh_arraydesc1[$i], 'left');
		...
		if ($rh_tipo==10)
		{
			...
			$pdf->y = $Page_Height - 129 - 30 - 32 - 12 + 8 + 4;
			$pdf->x = 45;
			$pdf->SetFontSize($rh_fontsize-3);
			$pdf->MultiCell(432,$line_height,strtoupper($ConLetra),0,'J',0,15);

			//subtotal
			$pdf->addTextWrap(504-8,97-51+60+40+10+11+8,81,$rh_fontsize+1,$DisplaySubTot, 'right');
	/*
			//iva %
	...
	*/
			//iva
			$pdf->addTextWrap(504-8,75-51+60+40+10+11+8,81,$rh_fontsize+1,$DisplayTax, 'right');

			//total
			$pdf->addTextWrap(504-8,51-51+60+40+10+11+8,81,$rh_fontsize+1,$DisplayTotal, 'right');
		}
		else
		{
			//total letra
