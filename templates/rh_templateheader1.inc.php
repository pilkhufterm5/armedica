...
if($PrintPDF=='Imprimir PDF (imagen)' && $rh_tipo!=20001){
	...
	//no factura	
	if ($rh_tipo!=20001){	
		$pdf->addTextWrap(481.5-15,500,72,12,$extInvoice.$extCN, 'right');
	}else{
		$pdf->addTextWrap(481.5-15,500,72,12,$extInvoice.$ExtRes['extinvoice'], 'right');
	}
}
	//color normal
...
if ($rh_tipo==10)
{
	...
	//fecha expedicion
	$pdf->addTextWrap(478-25,475.5+4,30,$rh_fontsize,$myrow['dd'], 'right');
	$pdf->addTextWrap(514-25,475.5+4,30,$rh_fontsize,$myrow['mm'], 'right');
	$pdf->addTextWrap(550-25,475.5+4,33,$rh_fontsize,$myrow['yy'], 'right');

	//lugar expedicion
	//$pdf->addTextWrap(468,652.5,108,7,$myrow['locationname'], 'right');

	//terminos
	$pdf->addTextWrap(481.5-25,412.5+4,108,$rh_fontsize-1,$myrow['terms'], 'center');

	//no cliente
	$pdf->addTextWrap(395-25,428.5+4,72,$rh_fontsize+1,$myrow['debtorno'], 'left');

	//cliente
	$pdf->addTextWrap(81-25,469+4,360,$rh_fontsize-1,$myrow['name'].' '.$myrow['name2'], 'left');
	...
	if (strlen($rh_unirDir) > 50)
	{
		...
		//direccion
		$pdf->addTextWrap(81-25,457+4,360,$rh_fontsize-1.5,$rh_strtmpI, 'left');//linea 1
		$pdf->addTextWrap(81-25,449+4,360,$rh_fontsize-1.5,$rh_strtmpF, 'left');//linea 2
		$pdf->addTextWrap(81-25,442+4,360,$rh_fontsize-1.5,$myrow['address4'].' '.$myrow['address5'].' C.P. '.$myrow['address6'], 'left');//linea 3
	}
	else
	{
		//direccion
		$pdf->addTextWrap(81-25,455+4,360,$rh_fontsize-1,$rh_unirDir, 'left');//linea 1
		$pdf->addTextWrap(81-25,444+4,360,$rh_fontsize-1,$myrow['address4'].' '.$myrow['address5'].' C.P. '.$myrow['address6'], 'left');//linea 2
	}
	...
	//rfc
	$pdf->addTextWrap(90-25,430.5+4,144,$rh_fontsize-1,$myrow['taxref'], 'left');

	//Vendedor
	$pdf->addTextWrap(409.5-25,418+4,54,$rh_fontsize-1,$myrow['salesmanname'], 'left');
